...
try:
    ...
    from src.adaptation.validator import PredictionValidator, ValidationRecord, AccuracyMetrics
    from src.adaptation.tracker import AccuracyTracker, RealTimeMetrics, AccuracyAlert, AlertSeverity
    from src.adaptation.drift_detector import ConceptDriftDetector, DriftMetrics, DriftSeverity
    from src.adaptation.retrainer import AdaptiveRetrainer, RetrainingRequest, RetrainingTrigger
    from src.adaptation.optimizer import ModelOptimizer, OptimizationConfig, OptimizationStrategy
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'system_validation': {
                ...
                } if self.import_validation else None,
                ...
                } if self.configuration_validation else None,
                ...
                } if self.error_handling_validation else None
            }
        }
...
class Sprint4SystemValidator:
    ...
    async def run_complete_validation(self) -> Sprint4ValidationReport:
        ...
        try:
            ...
            self.report.passed_tests = sum([
                ...
                sum([
                    1 if self.report.import_validation and self.report.import_validation.passed else 0,
                    1 if self.report.configuration_validation and self.report.configuration_validation.passed else 0,
                    1 if self.report.error_handling_validation and self.report.error_handling_validation.passed else 0
                ])
            ])
    ...
    async def _validate_component_initialization(self) -> None:
        ...
        self.report.component_coverage = {
            'PredictionValidator': any(t.test_name.startswith('PredictionValidator') and t.passed for t in self.report.component_tests),
            'AccuracyTracker': any(t.test_name.startswith('AccuracyTracker') and t.passed for t in self.report.component_tests),
            'ConceptDriftDetector': any(t.test_name.startswith('ConceptDriftDetector') and t.passed for t in self.report.component_tests),
            'AdaptiveRetrainer': any(t.test_name.startswith('AdaptiveRetrainer') and t.passed for t in self.report.component_tests),
            'ModelOptimizer': any(t.test_name.startswith('ModelOptimizer') and t.passed for t in self.report.component_tests)
        }
    ...
    async def _test_prediction_validator(self) -> None:
        ...
        try:
            ...
            self.report.component_tests.append(ValidationResult(
    ...
    async def _validate_system_integration(self) -> None:
        ...
        self.report.integration_coverage = {
            'TrackingManager': any(t.test_name.startswith('TrackingManager') and t.passed for t in self.report.integration_tests),
            'Dashboard': any(t.test_name.startswith('Dashboard') and t.passed for t in self.report.integration_tests),
            'EndToEnd': any(t.test_name.startswith('EndToEnd') and t.passed for t in self.report.integration_tests)
        }
    ...
    async def _test_tracking_manager_integration(self) -> None:
        ...
        try:
            ...
            details = {
                ...
                'total_predictions': stats.get('total_predictions_recorded', 0) if stats else 0
            }
            
            self.report.integration_tests.append(ValidationResult(
    ...
    async def _test_dashboard_integration(self) -> None:
        ...
        try:
            ...
            details = {
                ...
                'tracking_manager_connected': self.dashboard.tracking_manager is not None,
                'config_applied': True,
    ...
    async def _test_end_to_end_workflow(self) -> None:
        ...
        try:
            ...
            # Check final state
            stats = self.tracking_manager.get_system_stats()
            validator_metrics = await self.tracking_manager.validator.get_accuracy_metrics()
            ...
            details = {
                ...
                'total_predictions_in_system': stats.get('total_predictions_recorded', 0) if stats else 0,
                'validated_predictions': validator_metrics.validated_predictions if validator_metrics else 0,
                'accuracy_rate': validator_metrics.accuracy_rate if validator_metrics else 0.0
            }
    ...
    async def _test_prediction_throughput(self) -> None:
        ...
        try:
            ...
            details = {
                ...
                'average_time_per_prediction_ms': (recording_duration / num_predictions) * 1000
            }
            
            # Performance criteria: should handle at least 5 predictions per second
            passed = throughput >= 5.0
            
            self.report.performance_tests.append(ValidationResult(
                test_name="Prediction Throughput",
                passed=passed,
                duration_seconds=time.time() - start_time,
                details=details,
                warnings=[] if passed else [f"Throughput {throughput:.1f} < 5 predictions/second"]
            ))
    ...
    async def _test_memory_stability(self) -> None:
        ...
        try:
            ...
            self.report.performance_tests.append(ValidationResult(
                ...
                duration_seconds=time.time() - start_time,
                details=details,
                warnings=[] if passed else [f"Memory increased by {memory_increase:.1f}MB"]
            ))
    ...
    async def _validate_configuration_system(self) -> None:
        ...
        try:
            ...
            config_valid = (
                custom_config.enabled is True and
                custom_config.monitoring_interval_seconds == 2 and
                custom_config.alert_thresholds['accuracy_warning'] == 75.0 and
                custom_config.drift_psi_threshold == 0.2 and
                custom_config.retraining_accuracy_threshold == 60.0 and
                custom_config.optimization_strategy == "bayesian"
            )
            ...
            self.report.configuration_validation = ValidationResult(
                ...
                duration_seconds=time.time() - start_time,
                details=details,
                error_message=None if config_valid else "Configuration values not applied correctly"
            )
    ...
    async def _validate_error_handling(self) -> None:
        ...
        try:
            ...
            try:
                stats = self.tracking_manager.get_system_stats()
                system_responsive = stats is not None and self.tracking_manager._tracking_active
            except Exception:
            ...
            details = {
                ...
                'error_handling_grade': 'A' if all([error_handled, system_responsive, recovery_successful]) else 'B' if any([error_handled, system_responsive]) else 'C'
            }
            ...
            self.report.error_handling_validation = ValidationResult(
                ...
                duration_seconds=time.time() - start_time,
                details=details,
                error_message=None if passed else "System did not handle errors gracefully"
            )
    ...
    def _generate_completion_summary(self) -> None:
        ...
        print(f"\n📈 OVERALL RESULTS:")
        print(f"   ✅ Overall Success: {'YES' if self.report.overall_success else 'NO'}")
        print(f"   📊 Success Rate: {success_rate:.1f}% ({self.report.passed_tests}/{self.report.total_tests} tests)")
        ...
        for component, status in self.report.component_coverage.items():
            print(f"   {'✅' if status else '❌'} {component}: {'PASS' if status else 'FAIL'}")
        
        print(f"\n🔗 INTEGRATION VALIDATION:")
        for integration, status in self.report.integration_coverage.items():
            print(f"   {'✅' if status else '❌'} {integration}: {'PASS' if status else 'FAIL'}")
        ...
        for name, validation in validations:
            if validation:
                print(f"   {'✅' if validation.passed else '❌'} {name}: {'PASS' if validation.passed else 'FAIL'}")
        
        if self.report.overall_success:
            print(f"\n🎉 SPRINT 4 COMPLETE! ✅")
            print(f"   All Sprint 4 components are successfully integrated and working as a unified self-adaptation system.")
            print(f"   System ready for Sprint 5: Integration & API Development")
        else:
            print(f"\n⚠️  SPRINT 4 INCOMPLETE")
            print(f"   {self.report.failed_tests} test(s) failed. Review validation details and fix issues.")
