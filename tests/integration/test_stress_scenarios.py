    ...
    class SystemMonitor:
        ...
        def get_peak_usage(self):
            ...
            return {
                ...
                'average_cpu_percent': sum(m['cpu_percent'] for m in self.metrics) / len(self.metrics),
                'average_memory_mb': sum(m['memory_mb'] for m in self.metrics) / len(self.metrics)
            }
...
class TestConcurrentRequestStress:
    ...
    async def test_concurrent_api_request_handling(self, stress_test_config, system_monitor):
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            mock_tracking_manager.get_all_rooms.return_value = ['living_room', 'bedroom', 'kitchen']
        ...
        assert resource_metrics['peak_memory_mb'] < stress_test_config["memory_limit_mb"], \
            f"Peak memory {resource_metrics['peak_memory_mb']}MB exceeded limit"
        assert failed_requests < (total_requests * 0.05), \
            f"Too many failed requests: {failed_requests}/{total_requests}"
        
        logger.info(f"Concurrent stress test completed: {successful_requests}/{total_requests} successful")
        logger.info(f"Resource usage - CPU: {resource_metrics['peak_cpu_percent']:.1f}%, "
                   f"Memory: {resource_metrics['peak_memory_mb']:.1f}MB")
    
    @pytest_asyncio.async_test
    async def test_database_connection_pool_stress(self, stress_test_config, system_monitor):
        ...
        with patch('src.data.storage.database.create_async_engine') as mock_engine:
            ...
            concurrent_queries = stress_test_config["concurrent_requests"] * 2  # More DB stress
            tasks = [
            ...
            assert resource_metrics['peak_memory_mb'] < stress_test_config["memory_limit_mb"] * 1.2
            
            logger.info(f"Database stress test: {successful_queries}/{concurrent_queries} successful")
...
class TestDataVolumeStress:
    ...
    @pytest_asyncio.async_test
    async def test_large_event_stream_processing(self, stress_test_config, system_monitor):
        ...
        with patch('src.adaptation.tracking_manager.TrackingManager') as MockTM:
            ...
            assert processed_events >= event_volume * 0.95, \
            ...
            assert resource_metrics['peak_memory_mb'] < stress_test_config["memory_limit_mb"] * 1.5
            
            logger.info(f"Data volume stress test: {processed_events} events processed at "
                       f"{events_per_second:.1f} events/sec")
    
    @pytest_asyncio.async_test
    async def test_memory_usage_under_sustained_load(self, stress_test_config, system_monitor):
        ...
        assert resource_metrics['peak_memory_mb'] < stress_test_config["memory_limit_mb"] * 2
        assert final_memory < initial_memory * 1.2, "Final memory usage too high"
        
        logger.info(f"Memory stress test: Initial {initial_memory:.1f}MB, "
...
class TestMultiComponentStress:
    ...
    @pytest_asyncio.async_test
    async def test_tracking_manager_api_mqtt_integration_stress(self, stress_test_config, system_monitor):
        """Test integrated stress across tracking manager, API server, and MQTT."""
        system_monitor.start()
        
        with patch.multiple(
            'src.adaptation.tracking_manager',
            TrackingManager=Mock()
        ), patch('src.integration.enhanced_mqtt_manager.EnhancedMQTTIntegrationManager') as MockMQTT:
            ...
            async def integrated_stress_operation(operation_id: int):
                ...
                try:
                    ...
                    # 2. API operation
                    async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
                        response = await client.get(f"/api/rooms/{room_id}/metrics", timeout=5.0)
                        api_success = 200 <= response.status_code < 300
                    
                    # 3. MQTT operation
                    await mock_mqtt_manager.publish_prediction(room_id, {
            ...
            assert success_rate >= 0.90, f"Integration success rate {success_rate} too low"
            assert resource_metrics['peak_memory_mb'] < stress_test_config["memory_limit_mb"] * 1.5
            assert failed_operations < (concurrent_operations * 0.1)
            
            # Verify component interactions
            assert mock_tracking_manager.get_room_metrics.call_count >= concurrent_operations * 0.8
            assert mock_mqtt_manager.publish_prediction.call_count >= concurrent_operations * 0.8
            
            logger.info(f"Multi-component stress test: {successful_operations}/{concurrent_operations} successful")
    
    @pytest_asyncio.async_test
    async def test_system_resource_limit_handling(self, stress_test_config, system_monitor):
        ...
        for intensity in range(0, 100, 10):
            ...
            if current_metrics['peak_memory_mb'] > stress_test_config["memory_limit_mb"]:
        ...
        assert resource_metrics['peak_memory_mb'] < stress_test_config["memory_limit_mb"] * 2.0
        
        # Ensure system didn't crash under resource pressure
        assert len(results) >= 20, "Too few operations completed"
        
        logger.info(f"Resource limit test: {successful_operations}/{len(results)} operations succeeded")
        logger.info(f"Peak resource usage - Memory: {resource_metrics['peak_memory_mb']:.1f}MB")
...
class TestFailureRecoveryStress:
    ...
    @pytest_asyncio.async_test
    async def test_database_connection_failure_recovery(self, stress_test_config, system_monitor):
        ...
        async def database_operation_with_failure(operation_id: int):
            ...
            try:
                ...
                logger.warning(f"Database connection failed for operation {operation_id}: {e}")
        ...
        assert successful_operations > operations_count * 0.7, \
            f"Too few successful operations after failure: {successful_operations}/{operations_count}"
        assert failed_operations <= operations_count * 0.3, \
            f"Too many failed operations: {failed_operations}"
        
        logger.info(f"Database failure recovery test: {successful_operations}/{operations_count} recovered")
    
    @pytest_asyncio.async_test
    async def test_mqtt_connection_resilience_stress(self, stress_test_config, system_monitor):
        """Test MQTT connection resilience under stress conditions."""
        system_monitor.start()
        
        with patch('src.integration.enhanced_mqtt_manager.EnhancedMQTTIntegrationManager') as MockMQTT:
            mock_mqtt_manager = AsyncMock(spec=EnhancedMQTTIntegrationManager)
            ...
            async def mqtt_stress_operation(operation_id: int):
                ...
                try:
                    ...
                    logger.warning(f"MQTT connection error for operation {operation_id}: {e}")
                    return False
                except Exception as e:
            ...
            assert successful_mqtt_operations >= operations_count * 0.8, \
                f"Too few successful MQTT operations: {successful_mqtt_operations}/{operations_count}"
            
            logger.info(f"MQTT resilience test: {successful_mqtt_operations}/{operations_count} successful, "
