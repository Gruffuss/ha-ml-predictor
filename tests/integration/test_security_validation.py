...
async def mock_authentication_system():
    ...
    class MockAuthSystem:
        ...
        def create_valid_token(self, user_id: str, permissions: List[str] = None) -> str:
...
class TestAuthenticationSecurity:
    ...
    async def test_authentication_bypass_attempts(self, security_test_config, mock_authentication_system):
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            mock_tracking_manager.get_all_rooms.return_value = ['living_room', 'bedroom']
            ...
            bypass_attempts = [
                ...
                {'headers': {'Authorization': 'InvalidFormat token123'}, 'expected_status': 401},
                
                # Malformed JWT token
                {'headers': {'Authorization': 'Bearer invalid.jwt.token'}, 'expected_status': 401},
                
                # Expired token
                {'headers': {'Authorization': f'Bearer {auth_system.create_expired_token("test_user")}'}, 
                 'expected_status': 401},
                
                # SQL injection in token
                {'headers': {'Authorization': "Bearer '; DROP TABLE users; --"}, 'expected_status': 401},
                
                # XSS in authorization header
                {'headers': {'Authorization': 'Bearer <script>alert("xss")</script>'}, 'expected_status': 401}
            ]
            ...
            logger.info(f"Authentication bypass test completed: {len(bypass_attempts)} attempts blocked")
    
    @pytest_asyncio.async_test
    async def test_token_validation_and_expiration(self, security_test_config, mock_authentication_system):
    ...
    async def test_rate_limiting_security(self, security_test_config):
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
                ...
                for i in range(rate_limit + 20):  # Exceed rate limit
                    try:
                        ...
                        if "429" in str(e) or "rate limit" in str(e).lower():
                            ...
                            logger.warning(f"Request {i} failed with unexpected error: {e}")
                ...
                assert rate_limit_effectiveness > 0.1, \
                    f"Rate limiting not effective enough: {rate_limit_effectiveness}"
                assert successful_requests <= rate_limit * 1.1, \
                    f"Too many successful requests: {successful_requests}"
            
            logger.info(f"Rate limiting test: {rate_limited_count} requests rate limited, "
    ...
    async def test_sql_injection_prevention(self, security_test_config):
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
                ...
                for i, payload in enumerate(sql_payloads):
                    ...
                    try:
                        ...
                        db_error_indicators = [
                            ...
                            'postgresql', 'timescaledb', 'constraint'
                        ]
                ...
                assert sql_injection_blocked >= len(sql_payloads), \
                    f"Only {sql_injection_blocked}/{len(sql_payloads)} SQL injections blocked"
            
            logger.info(f"SQL injection prevention test: {sql_injection_blocked} attacks blocked")
            ...
            async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
                ...
                for i, payload in enumerate(xss_payloads):
                    ...
                    try:
                        ...
                        response = await client.get(
                            f"/api/search?query={encoded_payload}",
                            timeout=5.0
                        )
                ...
                assert xss_attacks_blocked >= len(xss_payloads), \
    ...
    async def test_input_size_and_format_validation(self, security_test_config):
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
                ...
                expected_tests = 1 + 1 + len(invalid_data_tests)  # oversized + malformed + data types
                assert validation_tests_passed >= expected_tests * 0.8, \
                    f"Input validation not comprehensive enough: {validation_tests_passed}/{expected_tests}"
            
            logger.info(f"Input validation test: {validation_tests_passed} validation checks passed")
...
class TestAPISecurityBoundaries:
    ...
    @pytest_asyncio.async_test
    async def test_unauthorized_endpoint_access(self, security_test_config, mock_authentication_system):
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            protected_endpoints = [
                {'method': 'GET', 'path': '/api/admin/status', 'required_perm': 'admin'},
                {'method': 'POST', 'path': '/api/rooms', 'required_perm': 'write'},
                {'method': 'DELETE', 'path': '/api/rooms/living_room', 'required_perm': 'admin'},
                {'method': 'PUT', 'path': '/api/system/config', 'required_perm': 'admin'},
                {'method': 'POST', 'path': '/api/model/retrain', 'required_perm': 'admin'}
            ]
                ...
                for endpoint in protected_endpoints:
                    ...
                    try:
                        if endpoint['method'] == 'GET':
                            ...
                            response = await client.post(endpoint['path'], json={}, timeout=5.0)
                        elif endpoint['method'] == 'PUT':
                            response = await client.put(endpoint['path'], json={}, timeout=5.0)
                        elif endpoint['method'] == 'DELETE':
                            response = await client.delete(endpoint['path'], timeout=5.0)
                        ...
                        logger.warning(f"Unauthorized access test failed for {endpoint['path']}: {e}")
                    ...
                    if endpoint['required_perm'] in ['write', 'admin']:
                        try:
                            ...
                            if endpoint['method'] == 'GET':
                                ...
                                response = await client.post(
                                    endpoint['path'], json={}, headers=headers, timeout=5.0
                                )
                            elif endpoint['method'] == 'PUT':
                                ...
                                response = await client.delete(endpoint['path'], headers=headers, timeout=5.0)
                            ...
                            logger.warning(f"Insufficient permission test failed for {endpoint['path']}: {e}")
                
                # Validate access control effectiveness
                expected_blocks = len(protected_endpoints) * 2  # unauthorized + insufficient perms
                assert unauthorized_access_blocked >= expected_blocks * 0.7, \
                    f"Access controls not effective: {unauthorized_access_blocked}/{expected_blocks}"
            
            logger.info(f"API security boundaries test: {unauthorized_access_blocked} access attempts blocked")
    ...
    async def test_sensitive_data_exposure_prevention(self, security_test_config):
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            sensitive_data_patterns = [
                ...
                'aws_access_key', 'private_key', 'jwt_secret'
            ]
            ...
            async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
                ...
                for endpoint in test_endpoints:
                    try:
                        ...
                        if 200 <= response.status_code < 300:
                            ...
                            for pattern in sensitive_data_patterns:
                                if pattern in response_text:
                                    ...
                                    if f'"{pattern}":' not in response_text and pattern not in ['key', 'token']:
                        ...
                        logger.warning(f"Sensitive data test failed for {endpoint}: {e}")
                ...
                try:
                    ...
                    if response.status_code >= 400:
                        ...
                        error_sensitive_patterns = [
                            'traceback', 'stack trace', '/home/', '/opt/',
                ...
                assert data_exposure_prevented >= len(test_endpoints), \
                    f"Sensitive data exposure prevention insufficient: {data_exposure_prevented}"
            
            logger.info(f"Sensitive data exposure prevention test: {data_exposure_prevented} tests passed")
        ...
        with patch('src.integration.api_server.get_tracking_manager') as mock_tm:
            ...
            async with httpx.AsyncClient(app=app, base_url="http://testserver") as client:
                ...
                for header, expected_value in required_headers.items():
                    if header in response.headers:
                        ...
                        if isinstance(expected_value, list):
                            ...
                            header_valid = any(val in header_value for val in expected_value)
                        else:
                            # Single expected substring
                            header_valid = expected_value.lower() in header_value.lower()
                        
                        if header_valid:
                            security_headers_present += 1
                            logger.info(f"Security header '{header}' properly configured: {header_value}")
                        else:
                            logger.warning(f"Security header '{header}' misconfigured: {header_value}")
                    else:
                ...
                info_disclosure_headers = [
                    ...
                    'X-AspNetMvc-Version', 'X-Rack-Cache'
                ]
                ...
                for header in info_disclosure_headers:
                    if header not in response.headers:
                        ...
                        logger.warning(f"Information disclosure header present: {header}")
                
                # Validate security headers
                header_score = security_headers_present / len(required_headers)
                assert header_score >= 0.6, \
                ...
                assert hiding_score >= 0.8, \
                    f"Too many information disclosure headers: {headers_hidden}/{len(info_disclosure_headers)}"
            
            logger.info(f"Security headers test: {security_headers_present}/{len(required_headers)} present, "
