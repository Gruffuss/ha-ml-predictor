...
class TestCompleteSystemWorkflow:
    ...
    async def test_system_initialization_workflow(self, mock_system_config):
        ...
        with patch('src.core.config.get_config', return_value=config), \
             patch('src.data.ingestion.ha_client.HomeAssistantClient') as mock_ha_client, \
    ...
    async def test_data_ingestion_to_prediction_workflow(self, mock_system_config):
        ...
        with patch('src.core.config.get_config', return_value=config), \
            ...
            with patch('src.adaptation.tracking_manager.TrackingManager') as mock_tracking:
    ...
    async def test_system_startup_shutdown_lifecycle(self, mock_system_config):
        ...
        with patch('src.core.config.get_config', return_value=config), \
             patch('src.data.ingestion.ha_client.HomeAssistantClient') as mock_ha, \
...
class TestSystemIntegration:
    ...
    def mock_components(self):
        ...
        components['database_manager'].health_check = AsyncMock(return_value={
            "status": "healthy"
        })
    ...
    def test_performance_integration_monitoring(self, mock_components):
        ...
        assert mock_components['tracking_manager'].get_current_predictions.call_count == 10
...
class TestWorkflowValidation:
    ...
    def test_adaptation_workflow_integration(self, workflow_mocks):
        ...
        needs_retraining = accuracy_metrics["living_room"]["avg_error_minutes"] > threshold
    ...
    def test_monitoring_workflow_integration(self, workflow_mocks):
        ...
        system_status = {
            ...
            "predictions": {"status": "active", "last_update": datetime.now(timezone.utc)},
