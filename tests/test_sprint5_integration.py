...
def mock_tracking_manager():
    ...
    manager.get_room_prediction = AsyncMock(return_value={
        "room_id": "test_room",
        "prediction_time": datetime.utcnow().isoformat(),
        "next_transition_time": (datetime.utcnow() + timedelta(minutes=30)).isoformat(),
    ...
    manager.get_accuracy_metrics = AsyncMock(return_value={
        "room_id": "test_room",
        "accuracy_rate": 0.87,
    ...
    manager.get_tracking_status = AsyncMock(return_value={
    ...
    # Mock retraining
    manager.trigger_manual_retrain = AsyncMock(return_value={
...
def mock_enhanced_mqtt_manager():
    ...
    # Mock stats
    manager.get_integration_stats = Mock(return_value={
        "mqtt_connected": True,
    ...
    manager.get_connection_info = Mock(return_value={
...
class TestTrackingIntegrationManager:
    ...
    async def integration_manager(
        ...
        with patch('src.integration.tracking_integration.EnhancedMQTTIntegrationManager', 
    ...
    async def test_realtime_callback_management(self, integration_manager):
        ...
        integration_manager.enhanced_mqtt_manager.add_realtime_callback.assert_called_once_with(callback)
        
        # Remove callback
        integration_manager.remove_realtime_callback(callback)
        integration_manager.enhanced_mqtt_manager.remove_realtime_callback.assert_called_once_with(callback)
...
class TestEnhancedMQTTIntegrationManager:
    ...
    async def enhanced_mqtt_manager(self, test_system_config):
        ...
        with patch('src.integration.enhanced_mqtt_manager.MQTTIntegrationManager') as mock_base:
            with patch('src.integration.enhanced_mqtt_manager.RealtimePublishingSystem') as mock_realtime:
...
class TestAPIServerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_api_endpoints_with_tracking_manager(self, mock_tracking_manager, mock_api_config):
        ...
        with patch('src.integration.api_server.get_config') as mock_get_config:
            ...
            with TestClient(app) as client:
                ...
                with patch('src.integration.api_server.get_database_manager') as mock_db:
                    ...
                    mock_db_manager.health_check = AsyncMock(return_value={"database_connected": True})
                    ...
                    with patch('src.integration.api_server.get_mqtt_manager') as mock_mqtt:
                        ...
                        mock_mqtt_manager.get_integration_stats = AsyncMock(
                            return_value=Mock(mqtt_connected=True, predictions_published=10)
                        )
    ...
    @pytest.mark.asyncio
    async def test_api_prediction_endpoint(self, mock_tracking_manager, mock_api_config):
        ...
        with patch('src.integration.api_server.get_config') as mock_get_config:
            ...
            with TestClient(app) as client:
                ...
                mock_tracking_manager.get_room_prediction.assert_called_once_with("test_room")
    ...
    async def test_api_retrain_endpoint(self, mock_tracking_manager, mock_api_config):
        ...
        with patch('src.integration.api_server.get_config') as mock_get_config:
            ...
            with TestClient(app) as client:
                response = client.post("/model/retrain", json={
                    "room_id": "test_room",
                    "force": True,
                    "strategy": "full",
                    "reason": "test_request"
...
class TestSystemIntegrationFlow:
    ...
    async def integrated_system(
        ...
        with patch('src.adaptation.tracking_manager.TrackingManager') as mock_tracking_class:
            with patch('src.integration.tracking_integration.EnhancedMQTTIntegrationManager') as mock_mqtt_class:
                ...
                mock_tracking.get_tracking_status = AsyncMock(return_value={"status": "healthy"})
...
class TestErrorHandlingAndRecovery:
    ...
    async def test_graceful_shutdown_with_errors(
        ...
        with patch('src.integration.tracking_integration.EnhancedMQTTIntegrationManager', 
            ...
            mock_enhanced_mqtt_manager.shutdown.side_effect = Exception("Shutdown error")
...
async def test_integration_helper_functions():
    ...
    with patch('src.adaptation.tracking_manager.TrackingManager') as mock_tracking_class:
        with patch('src.integration.tracking_integration.integrate_tracking_with_realtime_publishing') as mock_integrate:
