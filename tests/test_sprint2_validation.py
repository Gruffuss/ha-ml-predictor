...
def test_sprint2_temporal_extractor_structure():
    ...
    expected_features = [
        'time_since_last_event', 'current_state_duration', 
...
def test_sprint2_sequential_extractor_structure():
    ...
    expected_features = [
        'room_transition_count', 'unique_sensors_triggered',
        'movement_velocity_score', 'human_movement_probability'
    ]
    for feature in expected_features:
        assert feature in feature_names
...
def test_sprint2_contextual_extractor_structure():
    ...
    expected_features = [
        'current_temperature', 'doors_currently_open',
        'total_active_rooms', 'is_winter'
    ]
    for feature in expected_features:
        assert feature in feature_names
...
def test_sprint2_dataframe_creation():
    ...
    feature_dicts = [
        {'temporal_hour_sin': 0.5, 'sequential_room_count': 2.0, 'contextual_temperature': 22.0},
        {'temporal_hour_sin': 0.3, 'sequential_room_count': 1.0, 'contextual_temperature': 23.0},
        {'temporal_hour_sin': 0.7, 'sequential_room_count': 3.0, 'contextual_temperature': 21.0}
    ]
...
async def test_sprint2_end_to_end_feature_pipeline():
    ...
    async with FeatureStore(enable_persistence=False) as store:
        ...
        contextual_features = [k for k in features.keys() if k.startswith('contextual_')]
        ...
        batch_requests = [
            ('living_room', target_time - timedelta(hours=i))
            for i in range(3)
        ]
