...
def e2e_room_configs():
    ...
    return {
        ...
        "kitchen": {
            ...
            "sensors": {
                "presence": {
                    "main": "binary_sensor.kitchen_presence"
                },
...
class MockExternalServices:
    ...
    def add_ha_event(self, entity_id: str, state: str, timestamp: datetime = None):
        ...
        event = HAEvent(
            ...
            timestamp=timestamp,
            attributes={"device_class": "motion", "friendly_name": f"Test {entity_id}"}
        )
...
async def complete_system(
    ...
    with patch('src.data.ingestion.ha_client.HomeAssistantClient') as mock_ha_class:
        with patch('src.integration.mqtt_publisher.MQTTPublisher') as mock_mqtt_class:
            with patch('src.integration.realtime_publisher.RealtimePublishingSystem') as mock_realtime_class:
                ...
                with patch('src.core.config.get_config') as mock_get_config:
                    ...
                    for key, value in e2e_system_config.items():
                        setattr(mock_config, key, Mock(**value) if isinstance(value, dict) else value)
                    ...
                    with patch('src.adaptation.tracking_manager.TrackingManager') as mock_tracking_class:
                        with patch('src.integration.tracking_integration.EnhancedMQTTIntegrationManager') as mock_enhanced_mqtt_class:
                            ...
                            mock_enhanced_mqtt.get_integration_stats = Mock(return_value={})
                            mock_enhanced_mqtt.get_connection_info = Mock(return_value={
...
class TestCompleteSystemWorkflow:
    ...
    async def test_prediction_publishing_workflow(self, complete_system):
        ...
        mock_enhanced_mqtt.publish_prediction.assert_called_once_with("bedroom", prediction_data)
    ...
    async def test_system_health_monitoring_workflow(self, complete_system):
        ...
        with patch('src.integration.api_server.get_database_manager') as mock_db:
            ...
            with patch('src.integration.api_server.get_mqtt_manager') as mock_mqtt_mgr:
                ...
                mock_mqtt_manager.get_integration_stats = AsyncMock(return_value=Mock(
                    mqtt_connected=True,
    ...
    async def test_multi_room_prediction_workflow(self, complete_system):
        ...
        def get_prediction_side_effect(room_id):
            ...
            prediction.update({
                "prediction_time": datetime.utcnow().isoformat(),
                "next_transition_time": (datetime.utcnow() + timedelta(minutes=20)).isoformat(),
        ...
        with TestClient(system["api_app"]) as client:
            ...
            mock_tracking.get_room_prediction.assert_has_calls(expected_calls, any_order=True)
...
class TestMetricsAndMonitoring:
    ...
    async def test_accuracy_metrics_reporting(self, complete_system):
        ...
        with TestClient(system["api_app"]) as client:
            ...
            mock_tracking.get_accuracy_metrics.assert_called_once_with("living_room", 24)
            ...
            with TestClient(system["api_app"]) as client:
                ...
                for i in range(50):  # Make 50 concurrent requests
                    task = asyncio.create_task(
                        asyncio.to_thread(lambda: client.get("/predictions/living_room"))
                    )
                    tasks.append(task)
