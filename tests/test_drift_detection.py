...
from src.adaptation.validator import PredictionValidator, AccuracyMetrics, ValidationRecord
...
class TestDriftMetrics:
    ...
    def test_drift_metrics_initialization(self):
        ...
        assert drift_metrics.drift_severity != DriftSeverity.MINOR  # Should be higher due to degradation
    ...
    def test_to_dict_serialization(self):
        ...
        assert result_dict['feature_analysis']['drifting_features'] == ["feature1", "feature2"]
...
class TestConceptDriftDetector:
    ...
    def mock_prediction_validator(self):
        ...
        baseline_metrics.recent_records = [Mock(error_minutes=8.0), Mock(error_minutes=12.0)]
        ...
        current_metrics.recent_records = [Mock(error_minutes=18.0), Mock(error_minutes=22.0)]
        
        validator.get_accuracy_metrics = AsyncMock(side_effect=[baseline_metrics, current_metrics])
    ...
    async def test_detect_drift_basic(self, drift_detector, mock_prediction_validator):
        ...
        with patch.object(drift_detector, '_get_occupancy_patterns', return_value=None), \
             patch.object(drift_detector, '_get_recent_prediction_errors', return_value=[]):
    ...
    async def test_categorical_feature_drift_test(self, drift_detector):
        ...
        current_data = pd.Series(['A'] * 20 + ['B'] * 30 + ['C'] * 50)  # Different distribution
    ...
    def test_categorical_psi_calculation(self, drift_detector):
        ...
        current_data = pd.Series(['A'] * 40 + ['B'] * 30 + ['C'] * 30)  # Different distribution
    ...
    def test_frequency_pattern_comparison(self, drift_detector):
        ...
        current_patterns = {
            'daily_frequency': {'2024-01-04': 20, '2024-01-05': 25, '2024-01-06': 22}  # Higher frequency
        }
    ...
    def test_statistical_confidence_calculation(self, drift_detector):
        ...
        assert drift_metrics.statistical_confidence > 0.5  # Should be high due to significant tests
...
class TestFeatureDriftDetector:
    ...
    async def test_start_stop_monitoring(self, feature_detector):
        ...
        with patch.object(feature_detector, '_get_recent_feature_data', return_value=None):
            # Start monitoring
    ...
    async def test_insufficient_data_handling(self, feature_detector):
        ...
        feature_data = pd.DataFrame({
            'timestamp': pd.date_range(start=datetime.now() - timedelta(hours=2), periods=5, freq='H'),
            'feature1': [1, 2, 3, 4, 5]
        })
