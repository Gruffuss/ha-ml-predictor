...
class TestXGBoostPredictorInitialization:
    ...
    def test_model_params_structure(self):
        ...
        required_params = [
            ...
            "early_stopping_rounds", "eval_metric", "objective"
        ]
...
class TestXGBoostPredictorTraining:
    ...
    async def test_train_with_single_column_targets(self):
        ...
        targets = pd.DataFrame({
            "random_target": np.random.uniform(300, 3600, 50)
        })
    ...
    async def test_train_error_handling(self):
        ...
        with patch('xgboost.XGBRegressor') as mock_xgb:
            ...
            assert "XGBoost initialization failed" in predictor.training_history[0].error_message
...
class TestXGBoostPredictorPrediction:
    ...
    async def test_predict_time_clipping(self, trained_predictor):
        ...
        trained_predictor.model.predict.return_value = np.array([0, 200000])  # Very low/high
    ...
    async def test_predict_transition_type_logic(self, trained_predictor):
        ...
        results_occupied = await trained_predictor.predict(features, prediction_time, "occupied")
        assert results_occupied[0].transition_type == "occupied_to_vacant"
        
        results_vacant = await trained_predictor.predict(features, prediction_time, "vacant")
    ...
    def test_determine_transition_type_edge_cases(self, trained_predictor):
        ...
        features_work = pd.Series({
            "work_hours_indicator": 0.9,
            "other_feature": 0.5
        })
        result = trained_predictor._determine_transition_type("unknown", features_work, prediction_time)
        assert result == "vacant_to_occupied"
        
        # Test with features containing sleep_hours
        features_sleep = pd.Series({
            "sleep_hours_indicator": 0.9,
            "other_feature": 0.5
        })
        result = trained_predictor._determine_transition_type("unknown", features_sleep, prediction_time)
        ...
        result = trained_predictor._determine_transition_type("unknown", features_normal, early_morning)
        assert result == "occupied_to_vacant"
        
        # Test with no special features - afternoon (3 PM)
        afternoon = prediction_time.replace(hour=15)
        result = trained_predictor._determine_transition_type("unknown", features_normal, afternoon)
        assert result == "vacant_to_occupied"
...
class TestXGBoostPredictorFeatureImportance:
    ...
    def test_get_feature_contributions(self):
        ...
        X = pd.DataFrame([[0.5, -0.2, 1.0, 0.1]], columns=["feature1", "feature2", "feature3", "feature4"])
...
class TestXGBoostPredictorModelInformation:
    ...
    def test_get_model_complexity(self):
        ...
        predictor.feature_importance_ = {
            "f1": 0.3, "f2": 0.02, "f3": 0.25, "f4": 0.005, "f5": 0.4
        }
...
class TestXGBoostPredictorEdgeCases:
    ...
    async def test_train_with_nan_values(self):
        ...
        targets = pd.DataFrame({
            "time_until_transition_seconds": [300, 600, 900, 1200]
        })
    ...
    def test_transition_type_hour_boundaries(self):
        ...
        for hour in [0, 6, 22, 23]:
            ...
            result = predictor._determine_transition_type("unknown", features, test_time)
