...
def ensemble_training_data():
    ...
    features['smooth_pattern'] = np.sin(np.linspace(0, 4*np.pi, n_samples)) + np.random.normal(0, 0.1, n_samples)
    features['smooth_trend'] = np.linspace(0, 1, n_samples) + np.random.normal(0, 0.05, n_samples)
    ...
    targets = (base_time * hour_influence * temp_influence * motion_influence * 
    ...
    occupancy_states = np.where(
        (hours >= 22) | (hours <= 6), 'occupied_to_vacant', 'vacant_to_occupied'
    )
    ...
    targets_df = pd.DataFrame({
        ...
        'target_time': [datetime.utcnow() + timedelta(seconds=i*10) for i in range(n_samples)]
    })
...
class TestEnsembleTraining:
    ...
    async def test_ensemble_training_phases(self, ensemble_validation_data):
        ...
        train_features, train_targets, val_features, val_targets = ensemble_validation_data
        ensemble = OccupancyEnsemble(room_id="test_room")
        ...
        with patch.multiple(
            ...
            for model_name in ensemble.base_models.keys():
                ...
                mock_model.train = AsyncMock(return_value=TrainingResult(
                    success=True,
    ...
    @pytest.mark.asyncio
    async def test_ensemble_cross_validation_meta_features(self, ensemble_validation_data):
        ...
        with patch.multiple(
            ...
            for model_name in ensemble.base_models.keys():
                ...
                mock_instance.train = AsyncMock(return_value=TrainingResult(
                    success=True,
                ...
                if model_name == "lstm":
                    ...
                    with patch('src.models.ensemble.XGBoostPredictor', mock_model_class):
                        pass
                elif model_name == "hmm":
                    with patch('src.models.ensemble.HMMPredictor', mock_model_class):
                        pass
                elif model_name == "gp":
                    with patch('src.models.ensemble.GaussianProcessPredictor', mock_model_class):
                        pass
            
            # Test CV meta-feature generation
            meta_features = await ensemble._train_base_models_cv(small_features, small_targets)
    ...
    async def test_ensemble_meta_learner_training(self, ensemble_validation_data):
        ...
        # Train meta-learner
        await ensemble_rf._train_meta_learner(meta_features, train_targets, train_features)
        ...
        await ensemble_linear._train_meta_learner(meta_features, train_targets, train_features)
    ...
    async def test_ensemble_training_error_handling(self, ensemble_validation_data):
        ...
        with patch.object(ensemble.base_models['xgboost'], 'train', 
                         side_effect=Exception("Base model training failed")):
            
            # Should handle individual model failures gracefully
            result = await ensemble.train(train_features.head(100), train_targets.head(100))
...
class TestEnsemblePrediction:
    ...
    async def test_ensemble_prediction_generation(self, ensemble_validation_data):
        ...
        for prediction in predictions:
            ...
            assert prediction.transition_type in ["vacant_to_occupied", "occupied_to_vacant"]
    ...
    @pytest.mark.asyncio
    async def test_ensemble_confidence_with_gp_uncertainty(self, ensemble_validation_data):
    ...
    @pytest.mark.asyncio
    async def test_ensemble_prediction_combination_methods(self, ensemble_validation_data):
        ...
        await self._setup_trained_ensemble(ensemble_weighted, train_features, train_targets)
    ...
    async def test_ensemble_prediction_error_handling(self, ensemble_validation_data):
        ...
        with patch.object(ensemble.base_models['xgboost'], 'predict',
    ...
    async def _setup_trained_ensemble(self, ensemble, train_features, train_targets):
        ...
        for model_name, model in ensemble.base_models.items():
            ...
            model.train = AsyncMock(return_value=TrainingResult(
                success=True,
                training_time_seconds=60.0,
                model_version="v1.0",
                training_samples=len(train_features)
            ...
            for i in range(len(train_features)):
                ...
                base_predictions.append(PredictionResult(
                    predicted_time=pred_time,
                    transition_type="vacant_to_occupied" if i % 2 == 0 else "occupied_to_vacant",
...
class TestEnsembleIncrementalUpdate:
    ...
    async def test_ensemble_incremental_update(self, ensemble_validation_data):
        ...
        for model_name, model in ensemble.base_models.items():
            model.incremental_update = AsyncMock(return_value=TrainingResult(
                success=True,
                training_time_seconds=30.0,
                model_version="v1.1",
...
class TestEnsembleFeatureImportance:
    ...
    @pytest.mark.asyncio
    async def test_ensemble_feature_importance_combination(self, ensemble_validation_data):
        ...
        mock_importances = {
            ...
            'hmm': {'hour_sin': 0.1, 'prev_state_duration': 0.6, 'state_stability': 0.3},
            'gp': {'smooth_pattern': 0.7, 'smooth_trend': 0.2, 'hour_sin': 0.1}
        }
...
class TestEnsemblePerformance:
    ...
    async def test_ensemble_training_performance(self, ensemble_validation_data):
        ...
        with patch.multiple(
            ...
            result = await ensemble.train(
                perf_features, perf_targets, 
                perf_val_features, perf_val_targets
            )
            ...
            print(f"Ensemble training time: {training_time:.2f}s for {len(perf_features)} samples")
    ...
    async def _setup_trained_ensemble(self, ensemble, train_features, train_targets):
        ...
        ensemble.meta_scaler = MagicMock()
        ensemble.meta_scaler.transform = MagicMock(return_value=np.array([[0, 0, 0, 0]]))
        
        ensemble.model_weights = {'lstm': 0.25, 'xgboost': 0.35, 'hmm': 0.20, 'gp': 0.20}