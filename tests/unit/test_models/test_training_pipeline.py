...
@pytest.fixture
def training_pipeline(basic_training_config, mock_feature_engineering_engine, 
...
def sample_raw_data():
    ...
    return pd.DataFrame({
        ...
        'room_id': 'test_room',
        'sensor_type': 'motion',
        'state': np.random.choice(['on', 'off'], len(dates)),
...
class TestTrainingPipelineInitialization:
    ...
    def test_pipeline_initialization(self, basic_training_config, mock_feature_engineering_engine,
                                   mock_feature_store, mock_database_manager):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert pipeline.feature_engineering_engine == mock_feature_engineering_engine
    ...
    def test_pipeline_with_tracking_manager(self, basic_training_config, mock_feature_engineering_engine,
                                          mock_feature_store, mock_database_manager, mock_tracking_manager):
    ...
    def test_artifacts_directory_creation(self, basic_training_config, mock_feature_engineering_engine,
                                        mock_feature_store, mock_database_manager):
...
class TestDataQualityValidation:
    ...
    @pytest.mark.asyncio
    async def test_data_quality_validation_good_data(self, training_pipeline, sample_raw_data):
        """Test data quality validation with good data."""
        quality_report = await training_pipeline._validate_data_quality(sample_raw_data, "test_room")
    ...
    async def test_data_quality_validation_insufficient_data(self, training_pipeline):
        ...
        quality_report = await training_pipeline._validate_data_quality(small_data, "test_room")
    ...
    async def test_data_quality_validation_missing_columns(self, training_pipeline):
        ...
        quality_report = await training_pipeline._validate_data_quality(incomplete_data, "test_room")
    ...
    async def test_data_quality_validation_temporal_issues(self, training_pipeline):
        ...
        quality_report = await training_pipeline._validate_data_quality(inconsistent_data, "test_room")
    ...
    async def test_data_quality_validation_with_missing_values(self, training_pipeline):
        ...
        quality_report = await training_pipeline._validate_data_quality(data_with_nulls, "test_room")
    ...
    def test_can_proceed_with_quality_issues(self, training_pipeline):
        ...
        assert training_pipeline._can_proceed_with_quality_issues(acceptable_report) is True
        ...
        assert training_pipeline._can_proceed_with_quality_issues(unacceptable_report) is False
...
class TestDataPreparationAndFeatures:
    ...
    async def test_feature_extraction(self, training_pipeline, sample_raw_data):
        ...
        expected_feature_columns = ['temporal_hour', 'temporal_day_of_week', 
        ...
        expected_target_columns = ['time_until_transition_seconds', 'transition_type']
    ...
    async def test_data_splitting(self, training_pipeline, sample_features_and_targets):
        ...
        train_split, val_split, test_split = await training_pipeline._split_training_data(
        ...
        expected_val_size = int(total_samples * training_pipeline.config.validation_split)
        ...
        assert len(train_features) + len(val_features) + len(test_features) == total_samples
...
class TestModelTraining:
    ...
    @pytest.mark.asyncio
    async def test_model_training_ensemble(self, training_pipeline, sample_features_and_targets):
        ...
        train_features, train_targets = train_features.head(250), train_targets.head(250)
        ...
        with patch('src.models.training_pipeline.OccupancyEnsemble') as mock_ensemble_class:
            ...
            mock_ensemble.train = AsyncMock(return_value=TrainingResult(
                success=True,
    ...
    @pytest.mark.asyncio
    async def test_model_training_failure_handling(self, training_pipeline, sample_features_and_targets):
        ...
        with patch('src.models.training_pipeline.OccupancyEnsemble') as mock_ensemble_class:
            ...
            with pytest.raises(ModelTrainingError, match="No models were successfully trained"):
    ...
    @pytest.mark.asyncio
    async def test_model_training_specific_type(self, training_pipeline, sample_features_and_targets):
...
class TestModelValidation:
    ...
    @pytest.mark.asyncio
    async def test_model_validation_success(self, training_pipeline, sample_features_and_targets):
        ...
        for i in range(len(val_features)):
            mock_predictions.append(PredictionResult(
    ...
    @pytest.mark.asyncio
    async def test_model_validation_prediction_failure(self, training_pipeline, sample_features_and_targets):
    ...
    async def test_model_evaluation_and_selection(self, training_pipeline):
        ...
        best_model, evaluation_metrics = await training_pipeline._evaluate_and_select_best_model(
            trained_models=trained_models,
...
class TestFullTrainingWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_train_room_models_success(self, training_pipeline, sample_raw_data, 
                                           sample_features_and_targets):
    ...
    @pytest.mark.asyncio
    async def test_initial_training_multiple_rooms(self, training_pipeline, sample_raw_data,
                                                  sample_features_and_targets):
        ...
        with patch('src.models.training_pipeline.get_config') as mock_get_config:
            ...
            with patch('src.models.training_pipeline.OccupancyEnsemble') as mock_ensemble_class:
                ...
                mock_ensemble.predict = AsyncMock(return_value=[
                    PredictionResult(
                        predicted_time=datetime.utcnow() + timedelta(seconds=1800),
                        transition_type="vacant_to_occupied",
                        confidence_score=0.8
...
class TestPipelineStatisticsAndManagement:
    ...
    def test_pipeline_history_tracking(self, training_pipeline):
        ...
        for i in range(60):  # Add more than default limit of 50
            training_pipeline._pipeline_history.append(TrainingProgress(
    ...
    async def test_model_performance_retrieval(self, training_pipeline):
        ...
        performance = await training_pipeline.get_model_performance("nonexistent_room", "ensemble")
        ...
        performance = await training_pipeline.get_model_performance("test_room", "ensemble")
...
class TestTrainingPipelineErrorHandling:
    ...
    async def test_pipeline_cleanup_on_failure(self, training_pipeline):
        ...
        training_pipeline._query_room_events = AsyncMock(return_value=pd.DataFrame({
            'timestamp': pd.date_range('2024-01-01', periods=100, freq='1H'),
            'room_id': 'test_room',
            'sensor_type': 'motion',
            'state': ['on'] * 100,
