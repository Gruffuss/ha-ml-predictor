...
from src.core.config import SystemConfig, RoomConfig
from src.core.exceptions import FeatureExtractionError, ConfigurationError
...
class TestFeatureEngineeringEngine:
    ...
    @pytest.mark.asyncio
    async def test_extract_features_parallel(self, engine, sample_events, sample_room_states, target_time):
        """Test parallel feature extraction."""
        with patch.object(engine.temporal_extractor, 'extract_features') as mock_temporal, \
             patch.object(engine.sequential_extractor, 'extract_features') as mock_sequential, \
             patch.object(engine.contextual_extractor, 'extract_features') as mock_contextual:
            
            # Mock extractor responses
            mock_temporal.return_value = {"temporal_feature_1": 1.0, "temporal_feature_2": 2.0}
    ...
    @pytest.mark.asyncio
    async def test_extract_features_sequential(self, engine_sequential, sample_events, sample_room_states, target_time):
        """Test sequential feature extraction."""
        with patch.object(engine_sequential.temporal_extractor, 'extract_features') as mock_temporal, \
             patch.object(engine_sequential.sequential_extractor, 'extract_features') as mock_sequential, \
             patch.object(engine_sequential.contextual_extractor, 'extract_features') as mock_contextual:
    ...
    @pytest.mark.asyncio
    async def test_extract_features_specific_types(self, engine, sample_events, sample_room_states, target_time):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_extractor_failure(self, engine, sample_events, target_time):
    ...
    def test_get_feature_names(self, engine):
        ...
        with patch.object(engine.temporal_extractor, 'get_feature_names') as mock_temporal, \
             patch.object(engine.sequential_extractor, 'get_feature_names') as mock_sequential, \
             patch.object(engine.contextual_extractor, 'get_feature_names') as mock_contextual:
            ...
            feature_names = engine.get_feature_names(["temporal", "sequential", "contextual"])
            
            # Should include prefixed feature names and metadata
            expected_features = [
                "temporal_temp_feature_1", "temporal_temp_feature_2",
                "sequential_seq_feature_1", "contextual_context_feature_1",
                "meta_event_count", "meta_room_state_count", 
                "meta_extraction_hour", "meta_extraction_day_of_week",
    ...
    def test_clear_caches(self, engine):
        """Test cache clearing."""
        with patch.object(engine.temporal_extractor, 'clear_cache') as mock_temp_clear, \
             patch.object(engine.sequential_extractor, 'clear_cache') as mock_seq_clear, \
             patch.object(engine.contextual_extractor, 'clear_cache') as mock_context_clear:
    ...
    def test_get_default_features(self, engine):
        ...
        sequential_features = [k for k in defaults.keys() if k.startswith("sequential_")]
        contextual_features = [k for k in defaults.keys() if k.startswith("contextual_")]
    ...
    @pytest.mark.asyncio
    async def test_parallel_vs_sequential_consistency(self, mock_config, sample_events, sample_room_states, target_time):
        """Test that parallel and sequential processing produce identical results."""
        # Create engines with same config
        parallel_engine = FeatureEngineeringEngine(config=mock_config, enable_parallel=True)
        sequential_engine = FeatureEngineeringEngine(config=mock_config, enable_parallel=False)
        
        with patch.object(TemporalFeatureExtractor, 'extract_features') as mock_temporal, \
             patch.object(SequentialFeatureExtractor, 'extract_features') as mock_sequential, \
             patch.object(ContextualFeatureExtractor, 'extract_features') as mock_contextual:
            ...
            parallel_clean = {k: v for k, v in parallel_features.items() if not k.startswith("meta_")}
            sequential_clean = {k: v for k, v in sequential_features.items() if not k.startswith("meta_")}
    ...
    async def test_memory_efficiency(self, engine, target_time):
        ...
        for i in range(100):
            ...
            with patch.object(engine.temporal_extractor, 'extract_features', return_value={"temp": 1.0}), \
                 patch.object(engine.sequential_extractor, 'extract_features', return_value={"seq": 2.0}), \
                 patch.object(engine.contextual_extractor, 'extract_features', return_value={"context": 3.0}):
    ...
    @pytest.mark.asyncio
    async def test_extractor_partial_failure_handling(self, engine, sample_events, target_time):
        ...
        with patch.object(engine.temporal_extractor, 'extract_features') as mock_temporal, \
            ...
            sequential_features = [k for k in features.keys() if k.startswith("sequential_")]
            assert len(sequential_features) == 0
    ...
    async def test_large_feature_set_handling(self, engine):
        ...
        large_sequential = {f"sequential_feature_{i}": float(i + 100) for i in range(50)}
        large_contextual = {f"contextual_feature_{i}": float(i + 200) for i in range(75)}
        
        with patch.object(engine.temporal_extractor, 'extract_features', return_value=large_temporal), \
             patch.object(engine.sequential_extractor, 'extract_features', return_value=large_sequential), \
             patch.object(engine.contextual_extractor, 'extract_features', return_value=large_contextual):
            ...
            temporal_count = len([k for k in features.keys() if k.startswith("temporal_temporal_")])
            sequential_count = len([k for k in features.keys() if k.startswith("sequential_sequential_")])
            contextual_count = len([k for k in features.keys() if k.startswith("contextual_contextual_")])
    ...
    async def test_performance_comparison(self, mock_config, enable_parallel):
        ...
        with patch.object(engine.temporal_extractor, 'extract_features', side_effect=mock_extraction_with_delay), \
             patch.object(engine.sequential_extractor, 'extract_features', side_effect=mock_extraction_with_delay), \
             patch.object(engine.contextual_extractor, 'extract_features', side_effect=mock_extraction_with_delay):
