...
class TestContextualFeatureExtractor:
    ...
    def room_configs(self):
        ...
        for room_id in ["living_room", "kitchen", "bedroom"]:
            ...
            config.get_sensors_by_type = Mock(return_value={
    ...
    def environmental_events(self) -> List[SensorEvent]:
        ...
        for i, light in enumerate(light_values):
            ...
            event.attributes = {"illuminance": float(light), "unit_of_measurement": "lx"}
            events.append(event)
    ...
    def test_extract_features_with_environmental_data(self, extractor, environmental_events, 
                                                     room_states, target_time, room_configs):
        """Test feature extraction with environmental sensor data."""
        features = extractor.extract_features(
            environmental_events, room_states, target_time, room_configs, lookback_hours=2
        )
        ...
        expected_features = [
            "current_temperature", "avg_temperature", "temperature_trend",
            "current_humidity", "avg_humidity", "humidity_trend",
            "current_light", "avg_light", "light_trend",
            "is_comfortable_temp", "is_dim", "is_winter"
        ]
    ...
    def test_environmental_features_temperature(self, extractor, environmental_events, target_time):
        """Test temperature feature extraction."""
        features = extractor._extract_environmental_features(environmental_events, target_time)
    ...
    def test_environmental_features_humidity(self, extractor, environmental_events, target_time):
    ...
    def test_environmental_features_light(self, extractor, environmental_events, target_time):
    ...
    def test_multi_room_features(self, extractor, multi_room_events, room_states, target_time):
    ...
    def test_room_context_features(self, extractor, multi_room_events, room_configs, target_time):
    ...
    def test_room_activity_correlation_calculation(self, extractor):
        ...
        correlation = extractor._calculate_room_activity_correlation(room_events, target_time)
    ...
    def test_threshold_configuration(self, extractor):
        ...
        assert extractor.temp_thresholds["cold"] < extractor.temp_thresholds["comfortable"]
        assert extractor.temp_thresholds["comfortable"] < extractor.temp_thresholds["warm"]
        
        # Check humidity thresholds
        assert extractor.humidity_thresholds["dry"] < extractor.humidity_thresholds["comfortable"]
        assert extractor.humidity_thresholds["comfortable"] < extractor.humidity_thresholds["humid"]
    ...
    def test_different_lookback_windows(self, extractor, environmental_events, room_states, 
                                      target_time, lookback_hours):
        """Test feature extraction with different lookback windows."""
        features = extractor.extract_features(
            environmental_events, room_states, target_time, lookback_hours=lookback_hours
        )
    ...
    def test_performance_large_datasets(self, extractor):
        ...
        features = extractor.extract_features(large_events, large_room_states, target_time)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_extraction(self, extractor, environmental_events, room_states, target_time):
        """Test thread safety of feature extraction."""
        import asyncio
        
        async def extract_features():
            return extractor.extract_features(environmental_events, room_states, target_time)
