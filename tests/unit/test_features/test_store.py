...
from src.features.store import FeatureStore, FeatureCache, FeatureRecord
...
class TestFeatureCache:
    ...
    def test_make_key(self, cache):
        ...
        key3 = cache._make_key(room_id, target_time, 12, feature_types)  # Different lookback
        assert key1 != key3
    ...
    def test_get_expired_item(self, cache, sample_params):
        ...
        with patch('src.features.store.datetime') as mock_datetime:
            mock_datetime.utcnow.return_value = datetime(2024, 1, 16, 16, 0, 0)  # 25 hours later
    ...
    def test_cache_move_to_end(self, cache):
        ...
        features = cache.get("room_0", datetime(2024, 1, 15, 15, 0, 0), 24, ["temporal"])
...
class TestFeatureStore:
    ...
    async def test_get_features_cache_hit(self, store):
        ...
        store.cache.put(
            "living_room", target_time, 24, ["temporal", "sequential", "contextual"],
            expected_features, "test_hash"
        )
    ...
    async def test_get_features_cache_miss_compute(self, store):
        ...
        with patch.object(store, '_compute_features', return_value=expected_features) as mock_compute:
            features = await store.get_features("living_room", target_time)
    ...
    async def test_get_features_force_recompute(self, store):
        ...
        store.cache.put(
            "living_room", target_time, 24, ["temporal", "sequential", "contextual"],
            cached_features, "test_hash"
        )
        
        with patch.object(store, '_compute_features', return_value=computed_features) as mock_compute:
    ...
    async def test_compute_training_data(self, store):
        ...
        with patch.object(store, 'get_batch_features', return_value=mock_features), \
             patch.object(store.feature_engine, 'create_feature_dataframe') as mock_create_df:
            ...
            required_target_columns = ["target_time", "next_transition_time", 
                                     "transition_type", "room_id"]
    ...
    async def test_compute_features(self, store, sample_events, sample_room_states):
        ...
        with patch.object(store, '_get_data_for_features', 
                         return_value=(sample_events, sample_room_states)) as mock_get_data, \
             patch.object(store.feature_engine, 'extract_features', 
                         return_value=expected_features) as mock_extract:
    ...
    def test_get_stats(self, store):
        ...
        with patch.object(store.cache, 'get_stats', return_value={"cache_stat": 1}), \
             patch.object(store.feature_engine, 'get_extraction_stats', return_value={"engine_stat": 2}):
    ...
    def test_clear_cache(self, store):
        """Test cache clearing."""
        with patch.object(store.cache, 'clear') as mock_cache_clear, \
    ...
    async def test_health_check(self, store):
        ...
        with patch.object(store.feature_engine, 'validate_configuration') as mock_validate:
