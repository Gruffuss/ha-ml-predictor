...
from src.data.ingestion.event_processor import MovementPatternClassifier, MovementSequence
...
class TestSequentialFeatureExtractor:
    ...
    def room_configs(self):
        ...
        for room_id in ["living_room", "kitchen", "bedroom"]:
            ...
            config.get_sensors_by_type = Mock(return_value={"door": f"sensor.{room_id}_door"})
    ...
    def test_extract_features_multi_room(self, extractor, multi_room_events, room_configs, target_time):
        ...
        expected_features = [
            "room_transition_count", "unique_rooms_visited", "room_revisit_ratio",
            "avg_event_interval", "movement_velocity_score", "unique_sensors_triggered",
            "active_room_count", "multi_room_sequence_ratio"
        ]
    ...
    def test_extract_features_single_room(self, extractor, single_room_events, room_configs, target_time):
    ...
    def test_movement_classification_features(self, mock_classifier_class, extractor, 
    ...
    def test_create_sequences_for_classification(self, extractor, multi_room_events, room_configs):
    ...
    def test_different_lookback_windows(self, extractor, multi_room_events, target_time, lookback_hours):
    ...
    def test_edge_case_duplicate_sensors(self, extractor):
        ...
        assert features["sensor_revisit_count"] == 4.0  # 5 total - 1 initial = 4 revisits
    ...
    def test_no_classifier_available(self, extractor_no_config, multi_room_events, target_time):
    ...
    def test_time_filtering_accuracy(self, extractor, target_time):
        ...
        features = extractor.extract_features(events, target_time, lookback_hours=lookback_hours)
    ...
    def test_performance_large_sequences(self, extractor):
        ...
        for i in range(5000):  # Large number of events
            event = Mock(spec=SensorEvent)
            event.timestamp = base_time + timedelta(minutes=i * 0.3)  # ~18 second intervals
        ...
        features = extractor.extract_features(large_events, target_time, lookback_hours=24)
    ...
    @pytest.mark.asyncio
    async def test_concurrent_extraction(self, extractor, multi_room_events, target_time):
...
class TestSequentialFeatureExtractorMovementPatterns:
    ...
    def test_human_like_patterns(self, extractor):
        ...
        for i, room in enumerate(rooms):
            event = Mock(spec=SensorEvent)
            event.timestamp = base_time + timedelta(minutes=i * 10)  # 10-minute intervals
            event.room_id = room
            event.state = "on"
    ...
    def test_cat_like_patterns(self, extractor):
        ...
        for i, room in enumerate(rooms):
            event = Mock(spec=SensorEvent)
            event.timestamp = base_time + timedelta(seconds=i * 30)  # 30-second intervals
            event.room_id = room
            event.state = "on"
            event.sensor_type = "motion"
