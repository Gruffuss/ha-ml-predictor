...
class TestRESTAPIServer:
    ...
    async def test_api_key_verification(self, mock_get_tracking_manager, mock_get_config):
    ...
    async def test_background_health_check(self, mock_incident_manager, 
                                          mock_health_monitor, mock_get_config):
    ...
    async def test_health_endpoint_comprehensive(self, mock_db_manager, 
                                               mock_tracking_manager, mock_get_config, 
                                               mock_config, mock_tracking_manager_fixture):
        ...
        mock_db_instance.health_check = AsyncMock(return_value={
            "status": "healthy",
    ...
    @patch('src.integration.api_server.get_config')
    @patch('src.integration.api_server.get_tracking_manager')
    async def test_prediction_endpoints(self, mock_tracking_manager, mock_get_config,
                                      mock_config, mock_tracking_manager_fixture):
    ...
    @patch('src.integration.api_server.get_config')
    @patch('src.integration.api_server.get_tracking_manager')
    async def test_accuracy_metrics_endpoint(self, mock_tracking_manager, mock_get_config,
                                           mock_config, mock_tracking_manager_fixture):
    ...
    @patch('src.integration.api_server.get_config')
    @patch('src.integration.api_server.get_tracking_manager')
    async def test_manual_retrain_endpoint(self, mock_tracking_manager, mock_get_config,
                                         mock_config, mock_tracking_manager_fixture):
...
class TestWebSocketAPI:
    ...
    async def test_websocket_connection_registration(self, websocket_config, mock_websocket):
    ...
    async def test_websocket_subscription_management(self, websocket_config, mock_websocket):
        ...
        # Test unsubscription
        result = await manager.unsubscribe_client(connection_id, "/ws/predictions", "living_room")
        assert result is True
    ...
    async def test_websocket_prediction_publishing(self, websocket_config):
        ...
        prediction_result.predicted_time = datetime.now(timezone.utc) + timedelta(minutes=30)
        ...
        assert result["success"] is True
        assert result["clients_notified"] == 4  # 2 for predictions + 2 for room endpoint
    ...
    def test_websocket_server_stats(self, websocket_config):
        ...
        server.connection_manager.get_connection_stats = Mock(return_value={
    ...
    def test_websocket_error_handling(self):
        ...
        conn_error = WebSocketConnectionError("Connection failed", cause=Exception("Network error"))
    ...
    def test_websocket_endpoint_normalization(self, websocket_config):
        ...
        assert server._normalize_endpoint("/ws/room/living_room") == "/ws/room/living_room"
...
class TestRealtimeEndpoints:
    ...
    def test_stream_rate_limiting(self):
        ...
        # Mock rate limiter
        rate_limiter = Mock()
        rate_limiter.is_allowed.side_effect = [True, True, True, False]  # Allow 3, deny 4th
    ...
    async def test_stream_client_management(self):
        ...
        stale_clients = [
            cid for cid, info in client_registry.items()
            if (current_time - info["last_activity"]).total_seconds() > 1800  # 30 minutes
        ]
...
class TestMonitoringAPI:
    ...
    def mock_health_monitor(self):
        ...
        monitor.get_component_health.return_value = {
            "database": Mock(to_dict=Mock(return_value={
                "component_name": "database",
                "status": "healthy",
                "last_check": datetime.now().isoformat(),
                "message": "Database connection active"
            })),
            "tracking_manager": Mock(to_dict=Mock(return_value={
    ...
    def mock_incident_manager(self):
        ...
        manager.get_active_incidents.return_value = {
            "incident_001": Mock(to_dict=Mock(return_value={
        ...
        manager.get_incident.return_value = Mock(to_dict=Mock(return_value={
            "incident_id": "incident_001",
            "type": "database_connection_lost",
            "severity": "critical",
            "status": "active",
            "created_at": datetime.now().isoformat(),
            "description": "Database connection lost",
            "actions_taken": ["Attempted reconnection", "Notified administrators"]
        }))
    ...
    def test_metrics_data_aggregation(self):
        ...
        raw_metrics = [
            {"timestamp": datetime.now() - timedelta(minutes=5), "cpu_usage": 45.2, "memory_usage": 67.8},
            {"timestamp": datetime.now() - timedelta(minutes=4), "cpu_usage": 47.1, "memory_usage": 68.2},
            {"timestamp": datetime.now() - timedelta(minutes=3), "cpu_usage": 43.9, "memory_usage": 66.5},
            {"timestamp": datetime.now() - timedelta(minutes=2), "cpu_usage": 46.3, "memory_usage": 67.1},
            {"timestamp": datetime.now() - timedelta(minutes=1), "cpu_usage": 44.8, "memory_usage": 67.9}
        ]
    ...
    def test_monitoring_api_error_handling(self):
        ...
        error_response = self._handle_monitoring_error(
            Exception("Health monitor unavailable"),
            "health_check"
        )
        ...
        error_response = self._handle_monitoring_error(
            Exception("Incident manager error"),
            "incident_management"
        )
