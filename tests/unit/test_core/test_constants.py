...
from src.core.constants import (
    ...
    INVALID_STATES, MIN_EVENT_SEPARATION, MAX_SEQUENCE_GAP,
    DEFAULT_CONFIDENCE_THRESHOLD, TEMPORAL_FEATURE_NAMES,
    SEQUENTIAL_FEATURE_NAMES, CONTEXTUAL_FEATURE_NAMES, MQTT_TOPICS,
    DB_TABLES, API_ENDPOINTS, DEFAULT_MODEL_PARAMS,
...
class TestFeatureNames:
    ...
    def test_feature_names_no_cross_category_duplicates(self):
        ...
        total_features = len(TEMPORAL_FEATURE_NAMES) + len(SEQUENTIAL_FEATURE_NAMES) + len(CONTEXTUAL_FEATURE_NAMES)
...
class TestMQTTTopics:
    ...
    def test_mqtt_topics_structure(self):
        ...
        expected_topics = ["predictions", "confidence", "accuracy", "status", "health"]
...
class TestDatabaseTables:
    ...
    def test_db_tables_structure(self):
        ...
        expected_tables = [
            "sensor_events", "predictions", "model_accuracy", 
            "room_states", "feature_store"
        ]
...
class TestModelParameters:
    ...
    def test_lstm_parameters(self):
        ...
        expected_params = ["sequence_length", "hidden_units", "dropout", "learning_rate"]
    ...
    def test_xgboost_parameters(self):
        ...
        expected_params = ["n_estimators", "max_depth", "learning_rate", "subsample"]
        for param in expected_params:
...
class TestMovementPatterns:
    ...
    def test_human_movement_patterns(self):
        ...
        expected_keys = [
            "min_duration_seconds", "max_velocity_ms", 
            "typical_room_sequence_length", "door_interaction_probability"
        ]
    ...
    def test_movement_pattern_differences(self):
        ...
        assert CAT_MOVEMENT_PATTERNS["door_interaction_probability"] < HUMAN_MOVEMENT_PATTERNS["door_interaction_probability"]
...
class TestConstantsIntegration:
    ...
    def test_sensor_state_coverage(self):
        ...
        all_defined_states = set(PRESENCE_STATES + ABSENCE_STATES + DOOR_OPEN_STATES + DOOR_CLOSED_STATES + INVALID_STATES)
        ...
        for state in all_enum_states:
            assert state in all_defined_states, f"State '{state}' not covered by any category"
    ...
    def test_constant_value_consistency(self):
        ...
        assert HUMAN_MOVEMENT_PATTERNS["min_duration_seconds"] >= 10  # At least 10 seconds
        assert CAT_MOVEMENT_PATTERNS["min_duration_seconds"] >= 1   # At least 1 second
        
        # Velocities should be reasonable (m/s)
        assert HUMAN_MOVEMENT_PATTERNS["max_velocity_ms"] <= 5.0  # Reasonable human speed
