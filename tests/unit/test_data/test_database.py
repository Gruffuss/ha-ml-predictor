...
from src.data.storage.database import (
    ...
    close_database_manager, execute_sql_file, check_table_exists,
...
class TestDatabaseManager:
    ...
    async def test_initialize_failure_cleanup(self):
        ...
        with patch.object(manager, '_create_engine', side_effect=Exception("Connection failed")) as mock_create, \
    ...
    async def test_get_session_retry_on_connection_error(self):
        ...
        manager.session_factory = Mock(side_effect=[
    ...
    async def test_get_session_max_retries_exceeded(self):
        ...
        # Always fail
        manager.session_factory = Mock(side_effect=OperationalError("Always fails", None, None))
    ...
    async def test_health_check_unhealthy(self, test_db_manager):
        ...
        test_db_manager.get_session = AsyncMock(side_effect=Exception("Connection failed"))
    ...
    def test_connection_event_handlers(self):
        ...
        with patch('src.data.storage.database.event') as mock_event:
            ...
            assert mock_event.listens_for.call_count >= 4  # connect, checkout, checkin, invalidate
...
class TestDatabaseUtilityFunctions:
    ...
    async def test_get_timescaledb_version_success(self):
        ...
        with patch('src.data.storage.database.get_database_manager', return_value=mock_manager):
            ...
            mock_manager.execute_query.assert_called_once()
            args, kwargs = mock_manager.execute_query.call_args
            assert "SELECT extversion FROM pg_extension WHERE extname = 'timescaledb'" == args[0]
            assert kwargs['fetch_one'] is True
