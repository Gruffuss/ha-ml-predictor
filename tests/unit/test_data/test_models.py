...
class TestSensorEvent:
    ...
    async def test_get_recent_events(self, test_db_session, sample_sensor_events):
        ...
        recent_events = await SensorEvent.get_recent_events(
            test_db_session, "test_room", hours=2
        )
    ...
    @pytest.mark.asyncio
    async def test_get_recent_events_with_sensor_filter(
        ...
        recent_events = await SensorEvent.get_recent_events(
            test_db_session, "test_room", hours=2, sensor_types=["presence"]
        )
...
class TestRoomState:
    ...
    async def test_get_current_state_not_found(self, test_db_session):
        ...
        current_state = await RoomState.get_current_state(
            test_db_session, "nonexistent_room"
        )
    ...
    async def test_get_occupancy_history(self, test_db_session):
        ...
        history = await RoomState.get_occupancy_history(
            test_db_session, "bedroom", hours=24
        )
...
class TestPrediction:
    ...
    async def test_get_pending_validations(self, test_db_session):
        ...
        predictions = [
            ...
            Prediction(
                ...
                - timedelta(minutes=10),  # Already validated
            ),
        ...
        pending = await Prediction.get_pending_validations(
            test_db_session, room_id="living_room", cutoff_hours=2
        )
    ...
    async def test_get_accuracy_metrics(self, test_db_session):
        ...
        metrics = await Prediction.get_accuracy_metrics(
            test_db_session, "office", days=7, model_type="lstm"
        )
...
class TestFeatureStore:
    ...
    async def test_get_latest_features(self, test_db_session):
        ...
        latest = await FeatureStore.get_latest_features(
            test_db_session, "kitchen", max_age_hours=6
        )
...
class TestModelApplicationLevelRelationships:
    ...
    @pytest.mark.asyncio
    async def test_sensor_event_prediction_application_relationship(
    ...
    @pytest.mark.asyncio
    async def test_room_state_prediction_application_relationship(
        self, test_db_session
    ):
    ...
    async def test_create_timescale_hypertables(self, test_db_session):
        ...
        with patch.object(test_db_session, "execute") as mock_execute:
            with patch.object(test_db_session, "commit") as mock_commit:
                ...
                hypertable_calls = [
                    call for call in calls if "create_hypertable" in call
                ]
...
class TestModelIntegration:
    ...
    async def test_complete_prediction_workflow(self, test_db_session):
        ...
        prediction.accuracy_minutes = abs(
            (
                prediction.predicted_transition_time - actual_event.timestamp
            ).total_seconds()
            / 60
        )
    ...
    async def test_feature_store_lifecycle(self, test_db_session):
        ...
        latest = await FeatureStore.get_latest_features(
            test_db_session, "lifecycle_test_room", max_age_hours=6
        )
