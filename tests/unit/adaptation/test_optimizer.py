...
class TestHyperparameterOptimizer:
    ...
    async def test_grid_search_optimization(
        self, mock_model, sample_optimization_data, sample_parameter_specs
    ):
    ...
    async def test_early_stopping(
        ...
        optimizer = HyperparameterOptimizer(
            early_stopping_rounds=5, early_stopping_patience=0.001
        )
        features, targets = sample_optimization_data
...
class TestModelOptimizer:
    ...
    def test_initialization(self):
        ...
        assert (
            ...
            in optimizer.optimization_strategies
        )
        assert (
            OptimizationStrategy.ARCHITECTURE_SEARCH
            in optimizer.optimization_strategies
        )

    @pytest.mark.asyncio
    async def test_model_optimization_workflow(
        ...
        mock_hp_optimizer.optimize_hyperparameters = AsyncMock(
            return_value=mock_hp_result
        )

        with patch.object(
            optimizer,
            "_create_hyperparameter_optimizer",
            return_value=mock_hp_optimizer,
        ):
    ...
    @pytest.mark.asyncio
    async def test_architecture_optimization(
        self, mock_model, sample_optimization_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_feature_selection_optimization(
        self, mock_model, sample_optimization_data
    ):
        ...
        with patch.object(
            optimizer, "_get_feature_importance", return_value=feature_importance
        ):
            result = await optimizer.optimize_model(
                model=mock_model,
                training_features=features,
...
class TestOptimizationIntegration:
        ...
        with patch.object(optimizer, "_optimize_hyperparameters") as mock_hp:
            with patch.object(optimizer, "_optimize_feature_selection") as mock_fs:
                ...
                mock_fs.return_value = OptimizationResult(
                    success=True,
                    best_score=0.88,
                    best_parameters={"selected_features": ["f1", "f2"]},
                )
    ...
    @pytest.mark.asyncio
    async def test_adaptive_optimization_budget(
        self, mock_model, sample_optimization_data
    ):
