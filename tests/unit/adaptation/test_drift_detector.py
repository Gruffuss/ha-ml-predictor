    ...
    baseline_data = pd.DataFrame(
        {
            ...
            "timestamp": pd.date_range(
                start=datetime.now(UTC) - timedelta(days=30), periods=100, freq="H"
            ),
        }
    )
    ...
    current_data = pd.DataFrame(
        {
            ...
            "timestamp": pd.date_range(
                start=datetime.now(UTC) - timedelta(days=7), periods=50, freq="H"
            ),
        }
    )
...
class TestConceptDriftDetector:
    ...
    async def test_detect_drift_basic(self, mock_prediction_validator):
        ...
        detector = ConceptDriftDetector(
            baseline_days=30, current_days=7, min_samples=10
        )

        drift_metrics = await detector.detect_drift(
            room_id="living_room", prediction_validator=mock_prediction_validator
        )
    ...
    @pytest.mark.asyncio
    async def test_detect_drift_with_accuracy_degradation(
        self, mock_prediction_validator
    ):
        """Test drift detection identifies accuracy degradation."""
        detector = ConceptDriftDetector()

        drift_metrics = await detector.detect_drift(
            room_id="bedroom", prediction_validator=mock_prediction_validator
        )
    ...
    async def test_detect_drift_no_validator_error(self):
        ...
        with pytest.raises(Exception):  # Should raise error without validator
            await detector.detect_drift(room_id="test_room", prediction_validator=None)
    ...
    async def test_prediction_drift_analysis(self, mock_prediction_validator):
        ...
        drift_metrics = await detector.detect_drift(
            room_id="kitchen", prediction_validator=mock_prediction_validator
        )
    ...
    async def test_pattern_drift_analysis(self):
        ...
        with patch.object(detector, "_get_occupancy_patterns") as mock_patterns:
            ...
            drift_metrics = await detector.detect_drift(
                room_id="office", prediction_validator=mock_validator
            )
    ...
    async def test_statistical_confidence_calculation(self, mock_prediction_validator):
        ...
        drift_metrics = await detector.detect_drift(
            room_id="confidence_test", prediction_validator=mock_prediction_validator
        )
...
class TestFeatureDriftDetector:
    ...
    async def test_detect_feature_drift(self, sample_feature_data):
        ...
        drift_results = await detector.detect_feature_drift(
            room_id="test_room", feature_data=sample_feature_data
        )
        ...
        small_data = pd.DataFrame(
            {
                ...
                "timestamp": pd.date_range(
                    start=datetime.now(UTC), periods=20, freq="H"
                ),
            }
        )

        drift_results = await detector.detect_feature_drift(
            room_id="small_data_test", feature_data=small_data
        )
