...
from src.core.constants import (
    SensorType, SensorState, EventType, ModelType, PredictionType,
    PRESENCE_STATES, ABSENCE_STATES, DOOR_OPEN_STATES, DOOR_CLOSED_STATES,
    INVALID_STATES, MIN_EVENT_SEPARATION, MAX_SEQUENCE_GAP, DEFAULT_CONFIDENCE_THRESHOLD,
    TEMPORAL_FEATURE_NAMES, SEQUENTIAL_FEATURE_NAMES, CONTEXTUAL_FEATURE_NAMES,
    MQTT_TOPICS, DB_TABLES, API_ENDPOINTS, DEFAULT_MODEL_PARAMS,
    HUMAN_MOVEMENT_PATTERNS, CAT_MOVEMENT_PATTERNS
)

# Import exception classes
from src.core.exceptions import (
    ErrorSeverity, OccupancyPredictionError, ConfigurationError, ConfigFileNotFoundError,
    ConfigValidationError, MissingConfigSectionError, ConfigParsingError,
    HomeAssistantError, HomeAssistantConnectionError, HomeAssistantAuthenticationError,
    HomeAssistantAPIError, EntityNotFoundError, WebSocketError, WebSocketConnectionError,
    WebSocketAuthenticationError, WebSocketValidationError, DatabaseError,
    DatabaseConnectionError, DatabaseQueryError, DatabaseMigrationError,
    DatabaseIntegrityError, FeatureEngineeringError, FeatureExtractionError,
    InsufficientDataError, FeatureValidationError, FeatureStoreError,
    ModelError, ModelTrainingError, ModelPredictionError, InsufficientTrainingDataError,
    ModelNotFoundError, ModelVersionMismatchError, MissingFeatureError,
    ModelValidationError, DataProcessingError, DataCorruptionError,
    IntegrationError, DataValidationError, MQTTError, MQTTConnectionError,
    MQTTPublishError, MQTTSubscriptionError, APIServerError,
    SystemInitializationError, SystemResourceError, SystemError,
    ResourceExhaustionError, ServiceUnavailableError, MaintenanceModeError,
    APIError, APIAuthenticationError, RateLimitExceededError,
    APIAuthorizationError, APISecurityError, APIResourceNotFoundError,
    validate_room_id, validate_entity_id
)
...
class TestSystemConstants:
    ...
    def test_event_type_enum_values(self):
        ...
        # Verify all expected values are present
        expected_values = ["state_change", "prediction", "model_update", "accuracy_update"]
    ...
    def test_model_type_enum_values(self):
        ...
        assert len(list(ModelType)) == 5  # LSTM, XGBOOST, HMM, GAUSSIAN_PROCESS (GP is alias), ENSEMBLE
    ...
    def test_prediction_type_enum_values(self):
        ...
        # Verify all expected values are present
        expected_values = ["next_occupied", "next_vacant", "occupancy_duration", "vacancy_duration"]
    ...
    def test_feature_name_lists(self):
        ...
        expected_temporal = [
            "time_since_last_change", "current_state_duration", "hour_sin", "hour_cos",
            "day_sin", "day_cos", "week_sin", "week_cos", "is_weekend", "is_holiday"
        ]
        assert TEMPORAL_FEATURE_NAMES == expected_temporal
        
        # Test sequential features
        expected_sequential = [
            "room_transition_1gram", "room_transition_2gram", "room_transition_3gram",
            "movement_velocity", "trigger_sequence_pattern", "cross_room_correlation"
        ]
        assert SEQUENTIAL_FEATURE_NAMES == expected_sequential
        
        # Test contextual features
        expected_contextual = [
            "temperature", "humidity", "light_level", "door_state",
            "other_rooms_occupied", "historical_pattern_similarity"
        ]
    ...
    def test_mqtt_topics_structure(self):
        ...
        formatted = MQTT_TOPICS["predictions"].format(topic_prefix="occupancy", room_id="living_room")
    ...
    def test_db_tables_structure(self):
        ...
        expected_keys = {"sensor_events", "predictions", "model_accuracy", "room_states", "feature_store"}
    ...
    def test_api_endpoints_structure(self):
        ...
        expected_keys = {"predictions", "accuracy", "health", "retrain", "rooms", "sensors"}
    ...
    def test_default_model_params_structure(self):
        ...
        expected_model_types = {ModelType.LSTM, ModelType.XGBOOST, ModelType.HMM, ModelType.GAUSSIAN_PROCESS, ModelType.GP, ModelType.ENSEMBLE}
    ...
    def test_human_movement_patterns(self):
        ...
        expected_keys = {"min_duration_seconds", "max_velocity_ms", "typical_room_sequence_length", "door_interaction_probability"}
        ...
        assert isinstance(HUMAN_MOVEMENT_PATTERNS["door_interaction_probability"], float)
    ...
    def test_movement_pattern_differences(self):
        ...
        assert CAT_MOVEMENT_PATTERNS["min_duration_seconds"] < HUMAN_MOVEMENT_PATTERNS["min_duration_seconds"]
        
        # Cats should have higher velocity
        assert CAT_MOVEMENT_PATTERNS["max_velocity_ms"] > HUMAN_MOVEMENT_PATTERNS["max_velocity_ms"]
        
        # Cats should have longer room sequences (more wandering)
        assert CAT_MOVEMENT_PATTERNS["typical_room_sequence_length"] > HUMAN_MOVEMENT_PATTERNS["typical_room_sequence_length"]
        
        # Humans should interact with doors more often
        assert HUMAN_MOVEMENT_PATTERNS["door_interaction_probability"] > CAT_MOVEMENT_PATTERNS["door_interaction_probability"]
    ...
    def test_model_type_parameter_alignment(self):
        ...
        expected_model_types = {ModelType.LSTM, ModelType.XGBOOST, ModelType.HMM,
                                ModelType.GAUSSIAN_PROCESS, ModelType.ENSEMBLE}
    ...
    def test_feature_list_performance(self):
        ...
        for _ in range(1000):
            _ = len(TEMPORAL_FEATURE_NAMES + SEQUENTIAL_FEATURE_NAMES + CONTEXTUAL_FEATURE_NAMES)
...
class TestCustomExceptions:
    ...
    def test_base_exception_string_formatting(self):
        ...
        # Test with context
        error = OccupancyPredictionError("Error with context", context={"room": "living_room", "sensor": "motion"})
        error_str = str(error)
    ...
    def test_config_validation_error(self):
        ...
        error = ConfigValidationError(field="timeout", value=0, expected="positive integer")
    ...
    def test_home_assistant_authentication_error(self):
        ...
        assert error.context["hint"] == "Check if token is valid and has required permissions"
        assert error.severity == ErrorSeverity.HIGH
    ...
    def test_database_query_error(self):
        ...
        error = DatabaseQueryError(query, parameters, cause, error_type, ErrorSeverity.HIGH)
    ...
    def test_insufficient_data_error(self):
        ...
        error = InsufficientDataError(data_type, room_id, required_samples, available_samples)
    ...
    def test_feature_validation_error(self):
        ...
        error = FeatureValidationError(feature_name, validation_error, actual_value, room_id, cause)
        ...
        assert error.context["validation_rule"] == validation_error  # Backward compatibility
    ...
    def test_model_version_mismatch_error(self):
        ...
        error = ModelVersionMismatchError(model_type, room_id, expected_version, actual_version, cause)
    ...
    def test_missing_feature_error(self):
        ...
        error = MissingFeatureError(feature_names, room_id, available_features, operation)
        ...
        assert error.context["missing_features"] == feature_names  # Backward compatibility
        assert error.context["room_id"] == room_id
    ...
    def test_validate_entity_id_invalid(self):
        ...
        for entity_id in invalid_entity_ids:
            ...
            if "expected_format" in error.context.get("sample_data", {}):
                assert error.context["sample_data"]["expected_format"] == "sensor.living_room_motion"
...
class TestExceptionInheritance:
    ...
    def test_base_exception_hierarchy(self):
        ...
        base_exceptions = [
            ConfigurationError, HomeAssistantError, DatabaseError,
            FeatureEngineeringError, ModelError, DataProcessingError,
            IntegrationError, SystemError, APIError
        ]
    ...
    def test_configuration_exceptions_hierarchy(self):
        ...
        config_exceptions = [
            ...
            MissingConfigSectionError, ConfigParsingError
        ]
    ...
    def test_home_assistant_exceptions_hierarchy(self):
        ...
        ha_exceptions = [
            ...
            WebSocketConnectionError, WebSocketAuthenticationError,
    ...
    def test_feature_engineering_exceptions_hierarchy(self):
        ...
        fe_exceptions = [
            ...
            FeatureValidationError, FeatureStoreError, MissingFeatureError
        ]
    ...
    def test_model_exceptions_hierarchy(self):
        ...
        model_exceptions = [
            ModelTrainingError, ModelPredictionError,
            InsufficientTrainingDataError, ModelNotFoundError,
            ModelVersionMismatchError, ModelValidationError
        ]
    ...
    def test_integration_exceptions_hierarchy(self):
        ...
        integration_exceptions = [
            DataValidationError, MQTTError, MQTTConnectionError,
            MQTTPublishError, MQTTSubscriptionError, APIServerError,
    ...
    def test_system_exceptions_hierarchy(self):
        ...
        system_exceptions = [
            SystemInitializationError, SystemResourceError,
            ResourceExhaustionError, ServiceUnavailableError,
            MaintenanceModeError
        ]
        
        for exception_class in system_exceptions:
            if exception_class in [ResourceExhaustionError, ServiceUnavailableError, MaintenanceModeError]:
    ...
    def test_api_exceptions_hierarchy(self):
        ...
        api_exceptions = [
            ...
            APISecurityError, APIResourceNotFoundError
        ]
    ...
    def test_exception_chaining(self):
        ...
        processing_error = FeatureExtractionError("temporal", "bedroom", cause=root_cause)
...
class TestErrorHandling:
    ...
    def test_exception_with_large_context(self):
        ...
        error = DatabaseQueryError(
            ...
            parameters={"large_field": large_data}
        )
    ...
    def test_exception_with_unicode_content(self):
        ...
        unicode_message = "Ошибка валидации данных"  # "Data validation error" in Russian
