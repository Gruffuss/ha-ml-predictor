...
class TestValidationResult:
    ...
    def test_merge_valid_results(self):
        ...
        result1 = ValidationResult(is_valid=True, errors=[], warnings=["warn1"], info=["info1"])
        result2 = ValidationResult(is_valid=True, errors=[], warnings=["warn2"], info=["info2"])
    ...
    def test_merge_invalid_result(self):
        ...
        result2 = ValidationResult(is_valid=False, errors=["error"], warnings=[], info=[])
...
class TestHomeAssistantConfigValidator:
    ...
    def test_validate_valid_config(self):
        ...
        assert result.is_valid is True
        assert any("Home Assistant URL: http://localhost:8123" in info for info in result.info)
    ...
    def test_validate_short_token_warning(self):
        ...
        assert result.is_valid is True
        assert any("token appears to be too short" in warning for warning in result.warnings)
    ...
    def test_validate_timeout_warnings(self):
        ...
        assert result.is_valid is True
        assert any("WebSocket timeout is very low" in warning for warning in result.warnings)
    ...
    def test_validate_high_websocket_timeout_warning(self):
        ...
        assert result.is_valid is True
        assert any("WebSocket timeout is very high" in warning for warning in result.warnings)
    ...
    def test_is_valid_url_invalid_cases(self):
        ...
        for url in invalid_urls:
            assert validator._is_valid_url(url) is False, f"URL should be invalid: {url}"
    ...
    def test_test_connection_auth_failure(self, mock_get):
        ...
        assert result.is_valid is False
        assert any("Authentication failed: Invalid token" in error for error in result.errors)
    ...
    def test_test_connection_http_error(self, mock_get):
        ...
        assert result.is_valid is False
        assert any("API connection failed: HTTP 500" in error for error in result.errors)
    ...
    def test_test_connection_timeout(self, mock_get):
        ...
        assert result.is_valid is False
        assert any("Connection timeout after 5 seconds" in error for error in result.errors)
    ...
    def test_test_connection_connection_error(self, mock_get):
        ...
        assert result.is_valid is False
        assert any("Connection failed: Cannot reach Home Assistant" in error for error in result.errors)
    ...
    def test_test_connection_unexpected_error(self, mock_get):
        ...
        assert result.is_valid is False
        assert any("Unexpected error testing connection: Unexpected error" in error for error in result.errors)
    ...
    def test_test_connection_missing_config(self):
        ...
        assert result.is_valid is False
        assert any("Cannot test connection: URL or token missing" in error for error in result.errors)
...
class TestDatabaseConfigValidator:
    ...
    def test_validate_valid_config(self):
        ...
        assert result.is_valid is True
        assert any("Database connection string configured" in info for info in result.info)
        assert any("Pool settings: size=10, max_overflow=20" in info for info in result.info)
    ...
    def test_validate_timescaledb_warning(self):
        ...
        assert result.is_valid is True
        assert any("TimescaleDB extension is recommended" in warning for warning in result.warnings)
    ...
    def test_validate_pool_size_warnings(self):
        ...
        assert result.is_valid is True
        assert any("Database pool size is very low" in warning for warning in result.warnings)
        assert any("Max overflow is low relative to pool size" in warning for warning in result.warnings)
    ...
    def test_validate_high_pool_size_warning(self):
        ...
        assert result.is_valid is True
        assert any("Database pool size is very high" in warning for warning in result.warnings)
    ...
    def test_test_connection_auth_failure(self, mock_connect):
        ...
        async def mock_connect_func(url, timeout=None):
            raise __import__('asyncpg').exceptions.InvalidAuthorizationSpecificationError()
        ...
        assert result.is_valid is False
        assert any("Authentication failed: Invalid username/password" in error for error in result.errors)
    ...
    def test_test_connection_general_error(self, mock_connect):
        ...
        assert result.is_valid is False
        assert any("Connection failed: Connection failed" in error for error in result.errors)
    ...
    def test_test_connection_missing_config(self):
        ...
        assert result.is_valid is False
        assert any("Cannot test connection: connection string missing" in error for error in result.errors)
    ...
    def test_test_connection_no_timescaledb(self, mock_connect):
        ...
        assert result.is_valid is True
        assert any("TimescaleDB extension not found" in warning for warning in result.warnings)
    ...
    def test_test_connection_timescaledb_check_error(self, mock_connect):
        ...
        assert result.is_valid is True
        assert any("Cannot check TimescaleDB extension" in warning for warning in result.warnings)
    ...
    def test_test_connection_import_error(self, mock_import):
        ...
        assert result.is_valid is True
        assert any("Cannot test database connection: asyncpg not available" in warning for warning in result.warnings)
    ...
    def test_test_connection_unexpected_error(self):
        ...
        with patch('asyncio.run', side_effect=Exception("Unexpected error")):
            ...
            assert result.is_valid is False
            assert any("Unexpected error testing database: Unexpected error" in error for error in result.errors)
...
class TestMQTTConfigValidator:
    ...
    def test_validate_non_standard_port_warning(self):
        ...
        assert result.is_valid is True
        assert any("Non-standard MQTT port: 1234" in warning for warning in result.warnings)
    ...
    def test_validate_standard_ports_no_warning(self):
        ...
        for port in [1883, 8883]:
            ...
            assert result.is_valid is True
            assert not any("Non-standard MQTT port" in warning for warning in result.warnings)
    ...
    def test_validate_topic_prefix_slash_warning(self):
        ...
        for prefix in ["/occupancy", "occupancy/", "/occupancy/"]:
            ...
            assert result.is_valid is True
            assert any("Topic prefix should not start or end with '/'" in warning for warning in result.warnings)
    ...
    def test_validate_invalid_qos_levels(self):
        ...
        assert result.is_valid is False
        assert any("Invalid prediction QoS level: 3" in error for error in result.errors)
    ...
    def test_validate_discovery_enabled_with_prefix(self):
        ...
        assert result.is_valid is True
        assert any("MQTT discovery enabled with prefix: homeassistant" in info for info in result.info)
    ...
    def test_validate_discovery_enabled_no_prefix(self):
        ...
        assert result.is_valid is False
        assert any("Discovery prefix is required when discovery is enabled" in error for error in result.errors)
    ...
    def test_test_connection_missing_config(self):
        ...
        assert result.is_valid is False
        assert any("Cannot test connection: broker missing" in error for error in result.errors)
    ...
    def test_test_connection_import_error(self, mock_import):
        ...
        def patched_test_connection(config):
            result = ValidationResult(is_valid=True, errors=[], warnings=[], info=[])
            result.add_warning("⚠️  Cannot test MQTT connection: paho-mqtt not available")
            return result
...
class TestRoomsConfigValidator:
    ...
    def test_validate_few_sensors_warning(self):
        ...
        assert result.is_valid is True
        assert any("Very few sensors configured" in warning for warning in result.warnings)
    ...
    def test_validate_room_missing_name_warning(self):
        ...
        assert result.is_valid is True
        assert any("Room living_room has no name configured" in warning for warning in result.warnings)
    ...
    def test_validate_room_no_sensors_warning(self):
        ...
        assert result.is_valid is True
        assert any("Room living_room has no sensors configured" in warning for warning in result.warnings)
    ...
    def test_validate_invalid_entity_id_format(self):
        ...
        assert result.is_valid is False
        assert any("Invalid entity ID format: invalid_entity_id" in error for error in result.errors)
    ...
    def test_validate_missing_essential_sensors_warning(self):
        ...
        assert result.is_valid is True
        assert any("missing essential sensor types" in warning for warning in result.warnings)
        assert any("['motion', 'occupancy', 'door']" in warning for warning in result.warnings)
    ...
    def test_is_valid_entity_id_valid_formats(self):
        ...
        for entity_id in valid_entity_ids:
            assert validator._is_valid_entity_id(entity_id) is True, f"Should be valid: {entity_id}"
    ...
    def test_is_valid_entity_id_invalid_formats(self):
        ...
        for entity_id in invalid_entity_ids:
            assert validator._is_valid_entity_id(entity_id) is False, f"Should be invalid: {entity_id}"
    ...
    def test_validate_room_with_invalid_nested_entity_id(self):
        ...
        assert result.is_valid is False
        assert any("Invalid entity ID format: invalid_entity_format" in error for error in result.errors)
    ...
    def test_validate_room_info_message(self):
        ...
        assert any("living_room: 2 sensor types, 3 entities" in info for info in result.info)
...
class TestSystemRequirementsValidator:
    ...
    def test_validate_requirements_with_mocked_conditions(self):
        ...
        with patch('sys.version_info', (3, 8, 0)):
            with patch.object(validator, 'validate') as mock_validate:
                ...
                assert any("Python 3.9+ required" in error for error in validated_result.errors)
    ...
    def test_validate_requirements_disk_error(self):
        ...
        with patch.object(validator, 'validate') as mock_validate:
            ...
            assert validated_result.is_valid is False
            assert any("Insufficient disk space" in error for error in validated_result.errors)
    ...
    def test_validate_requirements_packages_missing(self):
        ...
        with patch.object(validator, 'validate') as mock_validate:
            ...
            assert validated_result.is_valid is False
            assert any("Missing required packages:" in error for error in validated_result.errors)
    ...
    def test_validate_requirements_all_good(self):
        ...
        with patch.object(validator, 'validate') as mock_validate:
            ...
            assert validated_result.is_valid is True
            assert any("All required packages are available" in info for info in validated_result.info)
    ...
    def test_validate_requirements_import_errors(self):
        ...
        with patch.object(validator, 'validate') as mock_validate:
            ...
            assert validated_result.is_valid is True
            assert any("Cannot check memory usage" in warning for warning in validated_result.warnings)
...
class TestConfigurationValidator:
    ...
    def test_validate_configuration_success(self):
        ...
        validator.system_validator.validate = Mock(return_value=ValidationResult(
            is_valid=True, errors=[], warnings=[], info=["Python version: 3.11.0", "✅ All required packages are available"]
        ))
        ...
        assert result.is_valid is True
        assert any("Configuration validation completed successfully" in info for info in result.info)
    ...
    def test_validate_configuration_section_exception(self):
        ...
        validator.ha_validator.validate = Mock(side_effect=Exception("Validation failed"))
        ...
        rooms_config = {"rooms": {"living_room": {"name": "Living Room", "sensors": {"motion": "binary_sensor.motion"}}}}
        
        result = validator.validate_configuration(config, rooms_config)
        
        assert result.is_valid is False
        assert any("Validation failed for Home Assistant: Validation failed" in error for error in result.errors)
    ...
    def test_validate_configuration_test_connections(self, mock_mqtt_test, mock_db_test, mock_ha_test):
        ...
        mock_mqtt_test.return_value = ValidationResult(True, [], [], ["MQTT connection OK"])
        ...
        result = validator.validate_configuration(config, rooms_config, test_connections=True)
    ...
    def test_validate_configuration_connection_test_exceptions(self):
        ...
        validator.ha_validator.test_connection = Mock(side_effect=Exception("Connection test failed"))
        ...
        assert result.is_valid is False
        assert any("Home Assistant connection test failed: Connection test failed" in error for error in result.errors)
    ...
    def test_validate_configuration_strict_mode(self):
        ...
        result = validator.validate_configuration(config, rooms_config, strict_mode=True)
        
        assert result.is_valid is False
        assert any("Strict mode:" in error and "warnings treated as errors" in error for error in result.errors)
    ...
    def test_validate_config_files_success(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.is_valid is True
            assert any(f"Loaded configuration from: {config_file}" in info for info in result.info)
            assert any(f"Loaded rooms configuration from: {rooms_file}" in info for info in result.info)
    ...
    def test_validate_config_files_missing_rooms(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result.is_valid is False
            assert any("Rooms configuration file not found:" in error for error in result.errors)
    ...
    def test_validate_config_files_environment_specific(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            base_config = {
                "home_assistant": {"url": "http://localhost:8123", "token": "base_token"},
                "database": {"connection_string": "postgresql://user:pass@localhost/db"},
                "mqtt": {"broker": "localhost"}
            }
            ...
            env_config = {
                ...
                "database": {"connection_string": "postgresql://test:pass@localhost/testdb"},
                "mqtt": {"broker": "test-broker"}
            }
            
            rooms_data = {"rooms": {"living_room": {"name": "Living Room", "sensors": {"motion": "binary_sensor.motion"}}}}
            ...
            result = validator.validate_config_files(str(config_dir), environment="test")
            
            assert result.is_valid is True
            assert any(f"Loaded configuration from: {env_config_file}" in info for info in result.info)
    ...
    def test_validate_config_files_environment_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = validator.validate_config_files(str(config_dir), environment="nonexistent")
            
            assert result.is_valid is True
            assert any(f"Loaded configuration from: {base_config_file}" in info for info in result.info)
    ...
    def test_validate_config_files_with_connection_tests(self):
        ...
        validator.ha_validator.test_connection = Mock(return_value=ValidationResult(True, [], [], []))
        validator.db_validator.test_connection = Mock(return_value=ValidationResult(True, [], [], []))
        validator.mqtt_validator.test_connection = Mock(return_value=ValidationResult(True, [], [], []))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = validator.validate_config_files(str(config_dir), test_connections=True)
