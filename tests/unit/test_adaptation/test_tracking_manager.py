...
from src.adaptation.retrainer import AdaptiveRetrainer, RetrainingRequest, RetrainingStatus
...
async def tracking_manager(
    ...
    with patch('src.adaptation.tracking_manager.PredictionValidator'), \
...
class TestPredictionRecording:
    ...
    @pytest.mark.asyncio
    async def test_prediction_recording(self, tracking_manager, sample_prediction_result):
    ...
    @pytest.mark.asyncio
    async def test_prediction_mqtt_integration(self, tracking_manager, sample_prediction_result):
        ...
        call_args = tracking_manager.mqtt_integration_manager.publish_prediction.call_args
        assert call_args[1]["prediction_result"] == sample_prediction_result

    @pytest.mark.asyncio
    async def test_prediction_recording_with_disabled_tracking(self, sample_prediction_result):
    ...
    @pytest.mark.asyncio
    async def test_prediction_cache_cleanup(self, tracking_manager, sample_prediction_result):
        ...
        old_predictions = [p for p in room_predictions 
...
class TestRoomStateChangeHandling:
    ...
    async def test_state_change_triggers_retraining_evaluation(self, tracking_manager):
        ...
        with patch.object(tracking_manager.validator, 'get_room_accuracy', 
...
class TestDriftDetectionIntegration:
    ...
    async def test_manual_drift_detection(self, tracking_manager):
        ...
        with patch.object(tracking_manager.drift_detector, 'detect_drift', 
...
class TestRetrainingIntegration:
    ...
    async def test_manual_retraining_request(self, tracking_manager):
        ...
        with patch.object(tracking_manager.adaptive_retrainer, 'request_retraining',
    ...
    async def test_retraining_status_tracking(self, tracking_manager):
        ...
        with patch.object(tracking_manager.adaptive_retrainer, 'get_retraining_status',
    ...
    async def test_retraining_cancellation(self, tracking_manager):
        ...
        with patch.object(tracking_manager.adaptive_retrainer, 'cancel_retraining',
                         return_value=True):
...
class TestSystemStatusAndMetrics:
    ...
    async def test_real_time_metrics_retrieval(self, tracking_manager):
        ...
        with patch.object(tracking_manager.accuracy_tracker, 'get_real_time_metrics',
    ...
    async def test_active_alerts_retrieval(self, tracking_manager):
        ...
        with patch.object(tracking_manager.accuracy_tracker, 'get_active_alerts',
    ...
    async def test_alert_acknowledgment(self, tracking_manager):
        ...
        with patch.object(tracking_manager.accuracy_tracker, 'acknowledge_alert',
                         return_value=True):
            success = await tracking_manager.acknowledge_alert(alert_id, acknowledged_by)
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_prediction_recording_error_handling(self, tracking_manager, sample_prediction_result):
...
class TestPerformanceAndConcurrency:
    ...
    async def test_memory_usage_monitoring(self, tracking_manager):
        ...
        total_cached = sum(len(preds) for preds in tracking_manager._pending_predictions.values())
