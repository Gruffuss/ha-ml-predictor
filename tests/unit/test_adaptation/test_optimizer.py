...
def mock_base_predictor():
    ...
    predictor.get_parameters = Mock(return_value={
        'learning_rate': 0.1,
        'n_estimators': 100,
        'max_depth': 5
    })
...
class TestOptimizationStrategies:
    ...
    @pytest.mark.asyncio
    async def test_bayesian_optimization(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_grid_search_optimization(self, optimizer, mock_base_predictor, synthetic_training_data):
        ...
        with patch.object(optimizer, '_get_parameter_space', return_value=param_space):
            ...
            assert isinstance(result, OptimizationResult)
            assert result.total_evaluations <= optimizer.config.grid_search_max_combinations

    @pytest.mark.asyncio
    async def test_random_search_optimization(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    async def test_performance_adaptive_optimization(
        self, optimizer, mock_base_predictor, synthetic_training_data, performance_context
    ):
...
class TestObjectiveFunctions:
    ...
    @pytest.mark.asyncio
    async def test_accuracy_objective(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_confidence_calibration_objective(self, optimizer, mock_base_predictor, synthetic_training_data):
        ...
        mock_base_predictor.evaluate_confidence_calibration = AsyncMock(return_value=0.85)
    ...
    @pytest.mark.asyncio
    async def test_composite_objective(self, optimizer, mock_base_predictor, synthetic_training_data):
...
class TestOptimizationConstraints:
    ...
    @pytest.mark.asyncio
    async def test_time_constraint_enforcement(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_performance_constraint_validation(self, optimizer, mock_base_predictor, synthetic_training_data):
        ...
        with patch.object(optimizer, '_measure_prediction_latency', return_value=45.0), \
            ...
            with patch.object(optimizer, '_get_parameter_space', return_value=param_space):
                ...
                if result.prediction_latency_ms:
                    assert result.prediction_latency_ms <= optimizer.config.max_prediction_latency_ms
                if result.memory_usage_mb:
                    assert result.memory_usage_mb <= optimizer.config.max_memory_usage_mb

    @pytest.mark.asyncio
    async def test_minimum_improvement_threshold(self, optimizer, mock_base_predictor, synthetic_training_data):
        ...
        baseline_score = 0.85
        with patch.object(optimizer, '_get_baseline_performance', return_value=baseline_score):
            ...
            with patch.object(optimizer, '_get_parameter_space', return_value=param_space):
                ...
                if result.success and result.improvement_over_default >= 0:
                    assert result.improvement_over_default >= optimizer.config.min_improvement_threshold or \
                           result.improvement_over_default < optimizer.config.min_improvement_threshold
...
class TestOptimizationHistory:
    ...
    @pytest.mark.asyncio
    async def test_optimization_history_tracking(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_parameter_caching(self, optimizer, mock_base_predictor, synthetic_training_data):
        ...
        with patch.object(optimizer, '_bayesian_optimization') as mock_bayesian:
            ...
            with patch.object(optimizer, '_get_parameter_space', return_value=param_space):
                ...
                if result.success and result.improvement_over_default > optimizer.config.min_improvement_threshold:
                    assert model_key in optimizer._parameter_cache
                    assert optimizer._parameter_cache[model_key] == result.best_parameters
    ...
    async def test_performance_history_tracking(self, optimizer):
        ...
        assert len(optimizer._performance_history[room_model_key]) == len(performance_values)
        assert list(optimizer._performance_history[room_model_key]) == performance_values
...
class TestOptimizationDecisionLogic:
    ...
    async def test_should_optimize_decision(self, optimizer, performance_context):
        ...
        assert should_optimize == True  # Should optimize due to poor performance
        ...
        should_optimize = optimizer._should_optimize(model_type, room_id, excellent_context)
        # May or may not optimize - depends on other factors

    @pytest.mark.asyncio
    async def test_disabled_optimization(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_no_parameter_space_handling(self, optimizer, mock_base_predictor, synthetic_training_data):
...
class TestOptimizationResults:
    ...
    def test_failed_optimization_result(self):
        ...
        assert result.convergence_achieved == False
...
class TestErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_model_training_error_handling(self, optimizer, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_objective_function_error_handling(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_timeout_handling(self, optimizer, mock_base_predictor, synthetic_training_data):
...
class TestPerformanceOptimization:
    ...
    @pytest.mark.asyncio
    async def test_optimization_performance_metrics(self, optimizer, mock_base_predictor, synthetic_training_data):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_optimizations(self, optimizer, mock_base_predictor, synthetic_training_data):
        ...
        for result in results:
            assert isinstance(result, OptimizationResult) or isinstance(result, Exception)
