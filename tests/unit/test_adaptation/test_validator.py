...
class TestValidationRecord:
    ...
    def test_validation_record_failure(self):
        ...
        assert record.validation_time is not None
        assert record.prediction_metadata["failure_reason"] == "Insufficient data for validation"
    ...
    def test_validation_record_serialization(self):
        ...
        record = ValidationRecord(
            ...
            prediction_interval=(predicted_time - timedelta(minutes=5), predicted_time + timedelta(minutes=5)),
...
class TestPredictionRecording:
    ...
    @pytest.mark.asyncio
    async def test_basic_prediction_recording(self, prediction_validator, sample_prediction_result):
        """Test basic prediction recording."""
        room_id = "basic_room"
        
        with patch.object(prediction_validator, '_store_prediction_to_db') as mock_store:
    ...
    async def test_prediction_recording_with_metadata(self, prediction_validator):
        ...
        metadata = {
            ...
            "feature_importance": {"temporal": 0.4, "sequential": 0.35, "contextual": 0.25}
        }
    ...
    async def test_prediction_expiration_handling(self, prediction_validator):
        ...
        with patch.object(prediction_validator, '_store_prediction_to_db'):
            ...
            if room_id in prediction_validator._pending_predictions:
                remaining_predictions = prediction_validator._pending_predictions[room_id]
                for pred in remaining_predictions:
                    assert pred.status in [ValidationStatus.EXPIRED, ValidationStatus.PENDING]
...
class TestPredictionValidation:
    ...
    @pytest.mark.asyncio
    async def test_prediction_validation_multiple_candidates(self, prediction_validator):
        ...
        with patch.object(prediction_validator, '_store_prediction_to_db'):
            ...
            actual_time = base_time + timedelta(minutes=35)  # Closest to second prediction
...
class TestAccuracyMetricsCalculation:
    ...
    def test_basic_accuracy_metrics_calculation(self, sample_validation_records):
        ...
        validated_records = [r for r in sample_validation_records if r.status == ValidationStatus.VALIDATED]
        ...
        metrics.accuracy_rate = (metrics.accurate_predictions / metrics.validated_predictions) * 100
    ...
    def test_bias_analysis(self, sample_validation_records):
        ...
        for record in validated_records:
            if record.actual_time and record.predicted_time:
                time_diff = (record.actual_time - record.predicted_time).total_seconds() / 60
    ...
    def test_confidence_analysis(self, sample_validation_records):
        ...
        if len(confidences) > 1:
            ...
            metrics.confidence_vs_accuracy_correlation = correlation if not np.isnan(correlation) else 0.0
        
        # Calculate overconfidence/underconfidence rates
        high_conf_wrong = sum(1 for c, a in zip(confidences, accuracies) if c > 0.8 and a == 0)
        low_conf_right = sum(1 for c, a in zip(confidences, accuracies) if c < 0.6 and a == 1)
        
        metrics.overconfidence_rate = high_conf_wrong / len(validated_records) if validated_records else 0
        metrics.underconfidence_rate = low_conf_right / len(validated_records) if validated_records else 0
    ...
    def test_accuracy_metrics_serialization(self):
        ...
        metrics = AccuracyMetrics(
            ...
            accuracy_by_level={"excellent": 15, "good": 25, "acceptable": 28, "poor": 12, "unacceptable": 5}
        )
...
class TestAccuracyMetricsRetrieval:
    ...
    @pytest.mark.asyncio
    async def test_room_accuracy_metrics(self, prediction_validator, prediction_history):
        """Test room-specific accuracy metrics retrieval."""
        room_id = "metrics_room"
        
        with patch.object(prediction_validator, '_get_predictions_from_db', return_value=prediction_history):
            metrics = await prediction_validator.get_room_accuracy(room_id, hours_back=24)
    ...
    @pytest.mark.asyncio
    async def test_overall_accuracy_metrics(self, prediction_validator, prediction_history):
    ...
    @pytest.mark.asyncio
    async def test_model_specific_accuracy_metrics(self, prediction_validator, prediction_history):
        """Test model-specific accuracy metrics."""
        model_type = ModelType.LSTM
        
        # Filter predictions by model type
        model_predictions = [p for p in prediction_history if p.model_type == model_type.value]
        
        with patch.object(prediction_validator, '_get_predictions_from_db', return_value=model_predictions):
            metrics = await prediction_validator.get_model_accuracy(model_type, hours_back=24)
    ...
    async def test_time_filtered_accuracy_metrics(self, prediction_validator):
        ...
        with patch.object(prediction_validator, '_get_predictions_from_db', return_value=[]):
    ...
    @pytest.mark.asyncio
    async def test_accuracy_trend_analysis(self, prediction_validator, prediction_history):
        """Test accuracy trend analysis over time."""
        room_id = "trend_room"
        
        with patch.object(prediction_validator, '_get_predictions_from_db', return_value=prediction_history):
            trend_data = await prediction_validator.get_accuracy_trend(room_id, hours_back=48, interval_hours=6)
...
class TestValidationStatistics:
    ...
    @pytest.mark.asyncio
    async def test_room_prediction_counts(self, prediction_validator, prediction_history):
...
class TestDatabaseIntegration:
    ...
    @pytest.mark.asyncio
    async def test_prediction_storage_to_database(self, prediction_validator, mock_db_session):
        """Test prediction storage to database."""
        room_id = "db_room"
        predicted_time = datetime.now() + timedelta(minutes=30)
        
        with patch('src.adaptation.validator.get_db_session', return_value=mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_validation_update_in_database(self, prediction_validator, mock_db_session):
    ...
    @pytest.mark.asyncio
    async def test_predictions_retrieval_from_database(self, prediction_validator, mock_db_session):
...
class TestCleanupAndMaintenance:
    ...
    async def test_expired_predictions_cleanup(self, prediction_validator):
        ...
        initial_count = len(prediction_validator._pending_predictions.get("cleanup_room", []))
        
        # Run cleanup
        await prediction_validator._cleanup_expired_predictions()
        
        # Should clean up old predictions
        final_count = len(prediction_validator._pending_predictions.get("cleanup_room", []))
    ...
    async def test_validation_history_cleanup(self, prediction_validator):
        ...
        remaining_records = [r for r in prediction_validator._validation_history 
    ...
    async def test_pending_predictions_size_limit(self, prediction_validator):
        ...
        total_pending = sum(len(preds) for preds in prediction_validator._pending_predictions.values())
    ...
    async def test_concurrent_validation_operations(self, prediction_validator):
        ...
        with patch.object(prediction_validator, '_store_prediction_to_db'), \
            ...
            for result in results:
                assert not isinstance(result, Exception) or isinstance(result, (OccupancyPredictionError, Exception))
