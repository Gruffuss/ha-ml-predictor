...
class TestConceptDriftDetector:
    ...
    @pytest.mark.asyncio
    async def test_drift_detection_with_no_drift(
    ...
    @pytest.mark.asyncio
    async def test_drift_detection_with_accuracy_degradation(
        self, drift_detector, mock_prediction_validator
    ):
    ...
    async def test_page_hinkley_drift_detection(self, drift_detector):
        ...
        with patch.object(
            drift_detector, "_get_recent_prediction_errors"
        ) as mock_errors:
    ...
    async def test_statistical_confidence_calculation(self, drift_detector):
        ...
        )  # Should be higher due to significant tests
...
class TestFeatureDriftDetector:
    ...
    @pytest.mark.asyncio
    async def test_numerical_feature_drift_detection(
    ...
    async def test_categorical_feature_drift_detection(
        ...
        feature_3_result = next(
            (r for r in drift_results if r.feature_name == "feature_3"), None
        )
    ...
    async def test_feature_drift_with_insufficient_data(self, feature_drift_detector):
        ...
        drift_results = await feature_drift_detector.detect_feature_drift(
            room_id="test_room", feature_data=small_data
        )
...
class TestDriftDetectionIntegration:
    ...
    @pytest.mark.asyncio
    async def test_combined_drift_detection_workflow(
        self, drift_detector, mock_prediction_validator
    ):
...
class TestStatisticalTests:
    ...
    async def test_population_stability_index(self, drift_detector):
        ...
        psi_score = await drift_detector._calculate_psi(
            baseline_df, current_df, features
        )
        ...
        psi_identical = await drift_detector._calculate_psi(
            baseline_df, identical_df, features
        )
...
class TestDriftDetectionEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_drift_detection(
        self, drift_detector, mock_prediction_validator
    ):
    ...
    async def test_feature_drift_with_mixed_data_types(self, feature_drift_detector):
        ...
        drift_results = await feature_drift_detector.detect_feature_drift(
            room_id="mixed_room", feature_data=mixed_data
        )
