...
class TestBasePredictor:
    ...
    def test_save_model_success(self):
        ...
        try:
            ...
            # Note: logger would normally log this, but we just pass in tests
    ...
    def test_get_model_info(self):
        ...
        training_result = TrainingResult(
            True, 20.0, "v2.3", 1000, validation_score=0.92
        )
        predictor.training_history.append(training_result)
...
class TestLSTMPredictor:
    ...
    def test_lstm_initialization(self):
        ...
        assert (
            "hidden_units" in predictor.model_params
            or "lstm_units" in predictor.model_params
        )
        assert (
            "dropout" in predictor.model_params
            or "dropout_rate" in predictor.model_params
        )

    def test_lstm_initialization_with_custom_params(self):
        """Test LSTMPredictor initialization with custom parameters."""
        custom_params = {"hidden_units": 128, "dropout": 0.3, "learning_rate": 0.002}

        predictor = LSTMPredictor(room_id="bedroom", **custom_params)

        assert (
            predictor.model_params["hidden_units"] == 128
            or predictor.model_params.get("lstm_units") == 128
        )
        assert (
            predictor.model_params["dropout"] == 0.3
            or predictor.model_params.get("dropout_rate") == 0.3
        )
    ...
    async def test_lstm_train_success(self, mock_mlp):
        ...
        with patch.object(predictor, "_create_sequences") as mock_create_seq:
            ...
            with patch.object(
                predictor.feature_scaler,
                "fit_transform",
                return_value=np.random.randn(50, 150),
            ):
                with patch.object(
                    predictor.target_scaler,
                    "fit_transform",
                    return_value=np.random.randn(50, 1),
                ):
                    with patch.object(
                        predictor.target_scaler,
                        "inverse_transform",
    ...
    async def test_lstm_predict_success(self, mock_mlp):
        ...
        predictor.feature_scaler.transform.return_value = np.random.randn(
            1, 60
        )  # Scaled sequence
    ...
    def test_create_sequences_basic(self):
        ...
        # Create test data
        features = pd.DataFrame(
            {"feature1": np.arange(50), "feature2": np.arange(50) * 2}
        )
    ...
    def test_create_sequences_insufficient_data(self):
        ...
        features = pd.DataFrame({"feature1": np.arange(10)})  # Only 10 samples
    ...
    def test_lstm_get_feature_importance_trained(self):
        ...
        mock_model = Mock()
        mock_model.coefs_ = [
            np.array([[0.5, 0.3, 0.8], [0.2, 0.7, 0.1]])
        ]  # Input layer weights
...
class TestXGBoostPredictor:
    ...
    async def test_xgboost_train_success(self, mock_scaler, mock_xgb):
        ...
        with patch.object(
            predictor, "_prepare_targets", return_value=np.random.randint(60, 7200, 100)
        ):
            result = await predictor.train(features, targets)
    ...
    async def test_xgboost_train_failure(self):
        ...
        features = pd.DataFrame({"feature1": np.random.randn(50)})
        ...
        with patch.object(
            predictor, "_prepare_targets", side_effect=Exception("Training failed")
        ):
            result = await predictor.train(features, targets)
    ...
    async def test_xgboost_predict_success(self, mock_xgb):
        ...
        with patch.object(predictor, "validate_features", return_value=True):
            with patch.object(predictor, "_calculate_confidence", return_value=0.85):
                results = await predictor.predict(
                    features, prediction_time, current_state="occupied"
                )
...
class TestHMMPredictor:
    ...
    def test_hmm_initialization(self):
        ...
        # Test default parameters from constants
        assert (
            "n_components" in predictor.model_params
            or "n_states" in predictor.model_params
        )
        assert "covariance_type" in predictor.model_params
        assert (
            predictor.model_params.get(
                "n_iter", predictor.model_params.get("max_iter", 100)
            )
            == 100
        )
    ...
    def test_hmm_initialization_with_custom_params(self):
        ...
        custom_params = {"n_components": 6, "covariance_type": "diag", "n_iter": 150}

        predictor = HMMPredictor(room_id="basement", **custom_params)

        assert (
            predictor.model_params.get(
                "n_components", predictor.model_params.get("n_states")
            )
            == 6
        )
        assert predictor.model_params["covariance_type"] == "diag"
        assert (
            predictor.model_params.get("n_iter", predictor.model_params.get("max_iter"))
            == 150
        )
    ...
    async def test_hmm_train_success(self, mock_scaler, mock_gmm):
        ...
        with patch.object(
            predictor, "_prepare_targets", return_value=np.random.randint(60, 7200, 80)
        ):
    ...
    async def test_hmm_train_insufficient_data(self):
        ...
        features = pd.DataFrame({"feature1": np.random.randn(15)})
        targets = pd.DataFrame(
    ...
    async def test_hmm_predict_success(self):
        ...
        with patch.object(predictor, "validate_features", return_value=True):
            with patch.object(predictor, "_predict_durations", return_value=1500):
                with patch.object(
                    predictor, "_calculate_confidence", return_value=0.75
                ):
                    results = await predictor.predict(
                        features, prediction_time, current_state="vacant"
                    )
    ...
    def test_build_transition_matrix(self):
        ...
        if hasattr(predictor, "_build_transition_matrix"):
            ...
            np.testing.assert_array_almost_equal(
                transition_matrix.sum(axis=1), [1, 1, 1]
            )
    ...
    def test_hmm_get_feature_importance_trained(self):
        ...
        mock_model = Mock()
        mock_model.covariances_ = np.array(
            [[[1.0, 0.2], [0.2, 0.8]], [[0.5, 0.1], [0.1, 0.6]]]
        )
    ...
    def test_get_state_info(self):
        ...
        predictor.state_characteristics = {
            0: {"mean_duration": 600},
            1: {"mean_duration": 1800},
            2: {"mean_duration": 3600},
        }
        predictor.transition_matrix = np.array(
            [[0.6, 0.3, 0.1], [0.2, 0.7, 0.1], [0.4, 0.2, 0.4]]
        )
...
class TestGaussianProcessPredictor:
    ...
    def test_gp_initialization(self):
        ...
        assert (
            predictor.model_type == ModelType.GAUSSIAN_PROCESS
            or predictor.model_type == ModelType.GP
        )
    ...
    def test_gp_initialization_with_custom_params(self):
        ...
        custom_params = {"kernel": "matern", "alpha": 1e-5, "normalize_y": False}
    ...
    async def test_gp_train_success(self, mock_scaler, mock_gpr):
        ...
        with patch.object(
            predictor, "_prepare_targets", return_value=np.random.randint(60, 7200, 50)
        ):
            result = await predictor.train(features, targets)
    ...
    async def test_gp_train_insufficient_data(self):
        ...
        features = pd.DataFrame({"feature1": np.random.randn(8)})
        targets = pd.DataFrame(
    ...
    async def test_gp_predict_with_uncertainty(self):
        ...
        features = pd.DataFrame({"temp": [22.5], "humidity": [65.0], "light": [450.0]})

        prediction_time = datetime.now(timezone.utc)

        with patch.object(predictor, "validate_features", return_value=True):
            with patch.object(
                predictor, "_calculate_confidence_intervals", return_value=(1500, 2100)
            ):
                with patch.object(
                    predictor, "_calculate_confidence_score", return_value=0.82
                ):
                    results = await predictor.predict(
                        features, prediction_time, current_state="vacant"
                    )
...
class TestEnsembleModel:
    ...
    def test_ensemble_initialization_with_base_models(self):
        ...
        try:
            base_models = [LSTMPredictor(), XGBoostPredictor(), HMMPredictor()]

            # Try different initialization patterns
            ensemble = OccupancyEnsemble(
                room_id="conference_room", base_models=base_models
            )
    ...
    async def test_ensemble_train_success(self):
        ...
        for i in range(3):
            ...
            mock_model.train = AsyncMock(
                return_value=TrainingResult(True, 10.0, f"v1.{i}", 100)
            )
        ...
        targets = pd.DataFrame(
            {
                "time_until_transition_seconds": np.random.randint(60, 7200, 100),
                "transition_type": ["vacant_to_occupied"] * 50
                + ["occupied_to_vacant"] * 50,
        ...
        targets = pd.DataFrame(
            {
                ...
                "transition_type": ["vacant_to_occupied"] * 15
                + ["occupied_to_vacant"] * 15,
    ...
    def test_ensemble_get_feature_importance(self):
        ...
        for model_name, importance in feature_importances.items():
            ...
            ensemble.base_models[model_name].get_feature_importance = Mock(
    ...
    def test_ensemble_initialization_with_tracking_manager(self):
        ...
        ensemble = OccupancyEnsemble(room_id="library", tracking_manager=mock_tracker)
    ...
    async def test_ensemble_train_validation_errors(self):
        ...
        invalid_targets = pd.DataFrame({"invalid_column": np.random.randn(60)})
    ...
    async def test_ensemble_incremental_update(self):
        ...
        targets = pd.DataFrame(
            {
                ...
                "transition_type": ["vacant_to_occupied"] * 10
                + ["occupied_to_vacant"] * 10,
        ...
        with patch.object(ensemble, "_predict_ensemble") as mock_predict:
            ...
            result = await ensemble.incremental_update(
                features, targets, learning_rate=0.05
            )
    ...
    async def test_ensemble_incremental_update_insufficient_data(self):
        ...
        assert (
            "training failed" in str(exc_info.value).lower()
    ...
    def test_ensemble_create_meta_features_error_cases(self):
        ...
        with pytest.raises(
            ValueError, match="Cannot create meta-features with zero-length"
        ):
    ...
    def test_ensemble_prepare_targets(self):
        ...
        targets2 = pd.DataFrame(
            {"target_time": target_times, "next_transition_time": next_times}
        )
