...
class TestDatasetGenerator:
    ...
    @staticmethod
    def create_occupancy_dataset(n_samples: int = 100, n_features: int = 10, 
                                noise: float = 0.1) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        ...
        targets = pd.DataFrame({
            ...
            'transition_type': np.random.choice(
                ['vacant_to_occupied', 'occupied_to_vacant'], 
                n_samples
            ),
            'target_time': [
...
class TestLSTMPredictorReal:
    ...
    async def test_lstm_training_validation(self, lstm_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=40, n_features=4)
    ...
    def test_lstm_feature_importance(self, lstm_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=30, n_features=3)
    ...
    def test_lstm_real_model_serialization(self, lstm_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=25, n_features=3)
        ...
        try:
            ...
            predictions = asyncio.run(new_predictor.predict(
                features.iloc[:2], datetime.now(timezone.utc)
            ))
...
class TestXGBoostPredictorReal:
    ...
    def test_xgboost_real_feature_importance(self, xgb_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=60, n_features=4)
    ...
    async def test_xgboost_early_stopping(self, xgb_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=70, n_features=5)
    ...
    def test_xgboost_model_complexity_info(self, xgb_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=50, n_features=3)
...
class TestHMMPredictorReal:
    ...
    def test_hmm_state_analysis(self, hmm_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=80, n_features=5)
    ...
    def test_hmm_feature_importance_real(self, hmm_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=70, n_features=4)
...
class TestGaussianProcessPredictorReal:
    ...
    @pytest.mark.asyncio
    async def test_gp_real_uncertainty_quantification(self, gp_predictor, small_dataset):
        ...
        prediction_time = datetime.now(timezone.utc)
        predictions = await gp_predictor.predict(
            features.iloc[:3], prediction_time
        )
...
class TestOccupancyEnsembleReal:
    ...
    def medium_dataset(self):
        ...
        return TestDatasetGenerator.create_occupancy_dataset(n_samples=150, n_features=8)
    ...
    async def test_ensemble_real_prediction(self, ensemble_predictor, medium_dataset):
        ...
        for pred in predictions:
            ...
            assert expected_models.issubset(actual_models) or len(base_preds) >= 3  # At least 3 succeeded
    ...
    def test_ensemble_base_model_performance_tracking(self, ensemble_predictor):
        ...
        features, targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=100, n_features=6)
    ...
    async def test_ensemble_incremental_update(self, ensemble_predictor):
        ...
        new_features, new_targets = TestDatasetGenerator.create_occupancy_dataset(n_samples=30, n_features=6)
    ...
    def test_ensemble_model_serialization_real(self, ensemble_predictor):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            predictions = asyncio.run(new_ensemble.predict(
                features.iloc[:2], datetime.now(timezone.utc)
            ))
            assert len(predictions) == 2
