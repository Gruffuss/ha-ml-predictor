...
from src.data.ingestion.ha_client import (
    HomeAssistantClient, HAEvent, RateLimiter
)
from src.core.config import SystemConfig, HomeAssistantConfig
...
class TestHomeAssistantClient:
    ...
    async def test_connect_success(self, test_system_config):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class, \
            ...
            assert f'Bearer {test_system_config.home_assistant.token}' in kwargs['headers']['Authorization']
    ...
    async def test_connect_failure_cleanup(self, test_system_config):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class, \
             patch.object(client, '_test_authentication', side_effect=Exception("Auth failed")), \
    ...
    async def test_test_authentication_success(self, test_system_config):
        ...
        mock_session.get.assert_called_once_with(f"{test_system_config.home_assistant.url}/api/")
    ...
    async def test_connect_websocket_success(self, test_system_config):
        ...
        with patch('websockets.connect') as mock_connect, \
            ...
            expected_url = test_system_config.home_assistant.url.replace('http://', 'ws://') + '/api/websocket'
    ...
    @pytest.mark.asyncio
    async def test_authenticate_websocket_wrong_initial_message(self, test_system_config):
    ...
    async def test_get_entity_history_success(self, test_system_config):
        ...
        result = await client.get_entity_history("binary_sensor.test", start_time, end_time)
    ...
    async def test_validate_entities_success(self, test_system_config):
        ...
        entities = ["binary_sensor.exists1", "binary_sensor.exists2", "binary_sensor.missing"]
...
class TestHomeAssistantClientWebSocketHandling:
    ...
    async def test_should_process_event_deduplication(self, test_system_config):
        ...
        event2 = HAEvent(
            ...
            state="off",
            previous_state="on",
            timestamp=base_time + timedelta(seconds=2),  # Less than MIN_EVENT_SEPARATION
            attributes={}
        )
        ...
        event3 = HAEvent(
            ...
            state="on",
            previous_state="off",
            timestamp=base_time + timedelta(seconds=10),  # More than MIN_EVENT_SEPARATION
            attributes={}
        )
...
class TestHomeAssistantClientIntegration:
    ...
    async def test_async_context_manager(self, test_system_config):
        ...
        with patch.object(HomeAssistantClient, 'connect') as mock_connect, \
    ...
    async def test_bulk_history_processing(self, test_system_config):
        ...
        results = []
        async for batch in client.get_bulk_history(entity_ids, start_time, end_time, batch_size=2):
