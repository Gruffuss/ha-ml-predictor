...
async def test_sprint3_model_training_basic():
    ...
    base_time = 1800 + features.iloc[:, 0] * 600  # 30 min +/- 10 min based on first feature
    targets_data = np.clip(base_time + np.random.normal(0, 300, n_samples), 60, 86400)
    
    targets = pd.DataFrame({
        'time_until_transition_seconds': targets_data
    })
...
async def test_sprint3_model_prediction_basic():
    ...
    train_targets = pd.DataFrame({
        'time_until_transition_seconds': np.clip(
            1800 + train_features.iloc[:, 0] * 600 + np.random.normal(0, 300, n_samples),
            60, 86400
        )
    })
...
async def test_sprint3_lstm_sequence_handling():
    ...
    features = pd.DataFrame(features_data, columns=[f'seq_feature_{i}' for i in range(n_features)])
    ...
    for i in range(n_samples):
        if i < 10:
            ...
            recent_avg = np.mean([features_data[j][0] for j in range(max(0, i-10), i)])
    ...
    # Test prediction
    pred_features = features.tail(predictor.sequence_length + 5)  # Ensure enough for sequence
...
async def test_sprint3_hmm_state_identification():
    ...
    features = pd.DataFrame(features_data, columns=[f'state_feature_{i}' for i in range(n_features)])
    targets = pd.DataFrame({'time_until_transition_seconds': targets_data})
...
async def test_sprint3_ensemble_training():
    ...
    for i in range(1, n_samples):
        features_data[i, :5] = 0.8 * features_data[i-1, :5] + 0.2 * features_data[i, :5]
    
    # Add some categorical-like features for XGBoost
    features_data[:, 15:20] = np.round(features_data[:, 15:20])
    
    features = pd.DataFrame(features_data, columns=[f'ensemble_feature_{i}' for i in range(n_features)])
    ...
    # Train ensemble
    result = await ensemble.train(train_features, train_targets, val_features, val_targets)
    ...
    predictions = await ensemble.predict(val_features.head(5), datetime.utcnow(), 'vacant')
