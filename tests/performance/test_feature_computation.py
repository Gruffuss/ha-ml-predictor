...
class TestFeatureComputationLatency:
    ...
    def sample_events(self):
        ...
        for i in range(1000):
            event = SensorEvent(
                ...
                timestamp=base_time + timedelta(minutes=i * 1.44),  # ~1.44 min intervals
                attributes={"friendly_name": f"Living Room Motion {i % 3}"},
                is_human_triggered=True if i % 10 != 0 else False  # 90% human, 10% cat
            )
            events.append(event)
    ...
    async def test_temporal_feature_extraction_performance(self, temporal_extractor, sample_events):
        ...
        for _ in range(50):
            ...
            features = await temporal_extractor.extract_features(sample_events, target_time)
        ...
        assert p95_latency < 100, f"P95 temporal extraction {p95_latency:.2f}ms too slow"

    async def test_sequential_feature_extraction_performance(self, sequential_extractor, sample_events):
        ...
        for _ in range(30):
            start_time = time.perf_counter()
            features = await sequential_extractor.extract_features(sample_events, room_configs)
            end_time = time.perf_counter()
        ...
        assert mean_latency < 150, f"Sequential extraction {mean_latency:.2f}ms too slow"
        assert p95_latency < 250, f"P95 sequential extraction {p95_latency:.2f}ms too slow"

    async def test_contextual_feature_extraction_performance(self, contextual_extractor, sample_events):
        ...
        assert mean_latency < 200, f"Contextual extraction {mean_latency:.2f}ms too slow"
        assert p95_latency < 350, f"P95 contextual extraction {p95_latency:.2f}ms too slow"

    async def test_complete_feature_pipeline_performance(self, feature_store, sample_events):
        ...
        with patch.object(feature_store, '_get_recent_events', return_value=sample_events), \
        ...
        assert mean_latency < 500, f"Complete pipeline {mean_latency:.2f}ms exceeds requirement"
    ...
    async def test_large_dataset_feature_computation(self, feature_store, large_event_dataset):
        """Test feature computation performance with large datasets."""
        room_id = "living_room"
        target_time = datetime.now()
        
        with patch.object(feature_store, '_get_recent_events', return_value=large_event_dataset), \
             patch.object(feature_store, '_get_room_context', return_value={}):
            ...
            print(f"Events per ms: {len(large_event_dataset) / large_dataset_latency:.2f}")
            
            # Large datasets should still complete in reasonable time
            assert large_dataset_latency < 2000, f"Large dataset computation {large_dataset_latency:.2f}ms too slow"
            assert features_df is not None
            assert len(features_df) > 0
    ...
    async def test_concurrent_feature_computation(self, feature_store, sample_events):
        ...
        with patch.object(feature_store, '_get_recent_events', return_value=sample_events), \
            ...
            assert mean_concurrent_latency < 600, f"Mean concurrent latency {mean_concurrent_latency:.2f}ms too high"
            assert max_concurrent_latency < 800, f"Max concurrent latency {max_concurrent_latency:.2f}ms too high"
    ...
    async def test_feature_caching_performance(self, feature_store, sample_events):
        ...
        with patch.object(feature_store, '_get_recent_events', return_value=sample_events), \
            ...
            cache_improvement = ((cold_latency - mean_warm_latency) / cold_latency) * 100
            ...
            assert mean_warm_latency <= cold_latency, "Caching should not slow down computation"
            assert mean_warm_latency < 400, f"Cached computation {mean_warm_latency:.2f}ms still too slow"
    ...
    async def test_feature_computation_scalability(self, feature_store):
        ...
        for size in dataset_sizes:
            ...
            with patch.object(feature_store, '_get_recent_events', return_value=events), \
                 patch.object(feature_store, '_get_room_context', return_value={}):
                
                # Measure computation time
                start_time = time.perf_counter()
                features = await feature_store.compute_features(room_id, target_time, 24)
                latency = (time.perf_counter() - start_time) * 1000
        ...
        for size, latency in scalability_results.items():
            assert latency < 500 + (size * 0.1), f"Computation for {size} events ({latency:.2f}ms) doesn't scale properly"
