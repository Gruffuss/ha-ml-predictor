...
class TestMemoryProfiling:
    ...
    async def test_predictor_memory_usage(self, memory_profiler):
        ...
        with patch('src.models.predictor.FeatureStore'), \
    ...
    async def test_feature_store_memory_efficiency(self, memory_profiler, sample_events):
        ...
        with patch('src.features.store.get_database_manager'):
            ...
            assert memory_increase < 30, f"Feature store memory increase {memory_increase:.2f} MB too high"
    ...
    async def test_event_processing_memory_scaling(self, memory_profiler):
        ...
        with patch.object(processor, '_store_event', new_callable=AsyncMock) as mock_store:
            ...
            for batch_size in batch_sizes:
                ...
                for i in range(batch_size):
                    event_data = {
                        ...
                        'attributes': {'friendly_name': f'Motion Sensor {i}', 'data': f'test_data_{i}'}
                    }
            ...
            for batch_size, metrics in memory_by_batch.items():
                memory_per_event = metrics['increase'] / batch_size * 1024  # KB per event
                print(f"Batch {batch_size}: {metrics['increase']:.2f} MB increase ({memory_per_event:.2f} KB/event)")
            ...
            assert scaling_ratio <= batch_size_ratio * 2, f"Memory scaling {scaling_ratio:.2f} too high"
    ...
    async def test_long_running_memory_stability(self, memory_profiler):
        ...
        with patch('src.models.predictor.FeatureStore'), \
            ...
            if len(memory_samples) >= 3:
                ...
                assert memory_growth < 20, f"Memory growth {memory_growth:.2f} MB indicates potential leak"
    ...
    async def test_garbage_collection_effectiveness(self, memory_profiler):
        ...
        recovery_rate = (memory_recovered / memory_created) * 100 if memory_created > 0 else 0
    ...
    def test_object_lifecycle_memory_tracking(self):
        ...
        class TrackableObject:
            def __init__(self, data_size: int = 1000):
                ...
                weakref.finalize(self, lambda obj_id=self.id: destroyed_objects.append(obj_id))
        ...
        assert destruction_rate > 90, f"Object destruction rate {destruction_rate:.1f}% too low"
