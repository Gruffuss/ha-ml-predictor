...
class TestPredictionPerformance:
    ...
    async def test_prediction_latency_under_100ms(self):
        ...
        targets = pd.DataFrame({
            ...
            'transition_type': ['occupied_to_vacant', 'vacant_to_occupied', 'occupied_to_vacant'],
        ...
        print(f"Prediction latency - Avg: {avg_latency:.2f}ms, Max: {max_latency:.2f}ms")
    ...
    async def test_batch_prediction_performance(self):
        ...
        targets = pd.DataFrame({
            'time_until_transition_seconds': np.random.normal(3600, 1800, n_samples),
        ...
        for batch_size in batch_sizes:
            ...
            assert time_per_prediction < 150, f"Batch prediction {time_per_prediction:.2f}ms per item exceeds threshold"
            
            print(f"Batch size {batch_size}: {total_time:.2f}ms total, {time_per_prediction:.2f}ms per prediction")
    ...
    async def test_model_inference_speed(self):
        ...
        targets = pd.DataFrame({
            ...
            'transition_type': np.random.choice(['occupied_to_vacant', 'vacant_to_occupied'], 100),
        ...
        for model_name, model in models.items():
            try:
                ...
                assert avg_inference < 50, f"{model_name} average inference {avg_inference:.2f}ms too slow"
                assert memory_increase < 50, f"{model_name} memory increase {memory_increase:.2f}MB too high"
                
                print(f"{model_name} - Avg: {avg_inference:.2f}ms, Max: {max_inference:.2f}ms, Memory: +{memory_increase:.2f}MB")
...
class TestFeatureExtractionPerformance:
    ...
    async def test_feature_computation_speed(self):
        ...
        assert avg_time < 500, f"Average feature computation {avg_time:.2f}ms exceeds 500ms requirement"
        assert max_time < 1000, f"Max feature computation {max_time:.2f}ms exceeds 1000ms threshold"
        ...
        assert avg_cache_time < 100, f"Feature store access {avg_cache_time:.2f}ms too slow"
    ...
    async def test_parallel_processing_performance(self):
        ...
        start_time = time.perf_counter()
        try:
            parallel_results = await asyncio.gather(*[compute_features_async(f"room_{i}", i) for i in range(10)], return_exceptions=True)
            # Handle any exceptions in results
            parallel_results = [r if not isinstance(r, Exception) else 0 for r in parallel_results]
        except Exception as e:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            start_time = time.perf_counter()
            thread_futures = [executor.submit(cpu_intensive_task, 1000) for _ in range(10)]
    ...
    async def test_memory_usage_optimization(self):
        ...
        for i in range(5):
            ...
            targets = pd.DataFrame({
                ...
                'transition_type': np.random.choice(['occupied_to_vacant', 'vacant_to_occupied'], 200),
        ...
        print(f"Memory usage - Initial: {initial_memory:.2f}MB, After training: {after_training_memory:.2f}MB, Increase: {training_memory_increase:.2f}MB")
        ...
        # Performance assertions
        assert training_memory_increase < 500, f"Model training memory increase {training_memory_increase:.2f}MB too high"
        assert cache_memory_increase < 100, f"Cache memory increase {cache_memory_increase:.2f}MB too high"
    ...
    async def test_database_query_performance(self):
        ...
        try:
            ...
            for query in basic_queries:
                ...
                assert avg_time < 50, f"Query '{query}' avg time {avg_time:.2f}ms too slow"
                assert max_time < 200, f"Query '{query}' max time {max_time:.2f}ms too slow"
            ...
            print(f"Connection pool - Status: {pool_status}, Utilization: {utilization:.1f}%")
        ...
        try:
            ...
            for batch_size in batch_sizes:
                ...
                if batch_times:
                    ...
                    events_per_second = batch_size / (avg_batch_time / 1000) if avg_batch_time > 0 else 0
                    
                    assert avg_batch_time < 1000, f"Batch size {batch_size} too slow: {avg_batch_time:.2f}ms"
                    assert events_per_second > 100, f"Throughput too low: {events_per_second:.0f} events/sec"
                    
                    print(f"Batch size {batch_size} - Avg time: {avg_batch_time:.2f}ms, Throughput: {events_per_second:.0f} events/sec")
            ...
            assert avg_connection_time < 100, f"Connection overhead {avg_connection_time:.2f}ms too high"
        ...
        try:
            ...
            for query in time_queries:
                ...
                if query_times:
                    ...
                    assert avg_time < 1000, f"Time series query avg {avg_time:.2f}ms too slow"
                    assert max_time < 5000, f"Time series query max {max_time:.2f}ms too slow"
                    
                    print(f"Time series query - Avg: {avg_time:.2f}ms, Max: {max_time:.2f}ms")
            ...
            for query in aggregation_queries:
                ...
                try:
                    ...
                    assert query_time < 2000, f"Aggregation query {query_time:.2f}ms too slow"
            ...
            try:
                ...
                for query in timescale_queries:
                    ...
                    query_time = (end_time - start_time) * 1000
                    assert query_time < 500, f"TimescaleDB query {query_time:.2f}ms too slow"
...
class TestSystemLoadTesting:
    ...
    async def test_concurrent_request_handling(self):
        ...
        try:
            ...
            for concurrency in concurrency_levels:
                ...
                # Process results
                successful_requests = [r for r in results if not isinstance(r, Exception) and len(r) == 3 and r[2]]
                failed_requests = [r for r in results if isinstance(r, Exception) or (len(r) == 3 and not r[2])]
                ...
                if successful_requests:
                    ...
                    assert avg_response_time < 200, f"Avg response time {avg_response_time:.2f}ms too high at concurrency {concurrency}"
                    
                    print(f"  Concurrency {concurrency}: {avg_response_time:.2f}ms avg, {max_response_time:.2f}ms max, {requests_per_second:.1f} req/sec, {success_rate:.2%} success")
                else:
                    print(f"  Concurrency {concurrency}: All requests failed")
                    assert False, f"All requests failed at concurrency level {concurrency}"
            ...
            for _ in range(10):  # 10 rounds of requests
                ...
                successful_round = [r for r in round_results if not isinstance(r, Exception) and len(r) == 3 and r[2]]
                if successful_round:
                    avg_round_time = sum(r[0] for r in successful_round) / len(successful_round)
            ...
            if sustained_results:
                ...
                sustained_degradation = max(sustained_results) / min(sustained_results) if min(sustained_results) > 0 else 1
                
                assert sustained_degradation < 2.0, f"Performance degraded {sustained_degradation:.2f}x under sustained load"
                print(f"  Sustained load: {sustained_avg:.2f}ms avg, {sustained_degradation:.2f}x max degradation")
    ...
    async def test_high_volume_data_processing(self):
        ...
        for volume in volume_levels:
            ...
            sequential_time = time.perf_counter() - start_time
            sequential_rate = sequential_processed / sequential_time if sequential_time > 0 else 0
            ...
            for i in range(0, len(events), batch_size):
                ...
                try:
                    ...
                    batch_processed += sum(1 for r in batch_results if not isinstance(r, Exception) and r is not None)
            ...
            # Performance assertions
            assert processing_efficiency > 0.8, f"Processing efficiency {processing_efficiency:.2%} too low for volume {volume}"
            assert batch_rate > 50, f"Batch processing rate {batch_rate:.1f} events/sec too low for volume {volume}"
            assert speedup >= 1.0, f"Batch processing should not be slower than sequential"
            
            print(f"  Volume {volume}: Sequential {sequential_rate:.1f} evt/sec, Batch {batch_rate:.1f} evt/sec, Efficiency {processing_efficiency:.2%}, Speedup {speedup:.2f}x")
        ...
        memory_per_event = total_memory_increase / processed_count if processed_count > 0 else 0
        
        assert total_memory_increase < 200, f"Memory usage increased by {total_memory_increase:.2f}MB - too high"
        assert memory_per_event < 0.1, f"Memory per event {memory_per_event:.4f}MB too high"
        
        print(f"  Memory usage: +{total_memory_increase:.2f}MB total, {memory_per_event:.4f}MB per event")
    ...
    async def test_resource_utilization_monitoring(self):
        ...
        try:
            ...
            print(f"Initial resource state:")
            print(f"  CPU: {initial_stats['cpu_percent']:.1f}%")
            print(f"  Memory: {initial_stats['memory_info'].percent:.1f}% ({initial_stats['memory_info'].used / 1024**3:.2f}GB used)")
            ...
            workload_task = asyncio.create_task(asyncio.gather(*tasks, return_exceptions=True))
            ...
            if isinstance(resource_samples, list) and resource_samples:
                ...
                assert cpu_efficiency > 0.3, f"CPU utilization inefficient: {cpu_efficiency:.2f}"
            ...
            try:
                if hasattr(health_monitor, 'get_system_health'):
                    ...
                    print(f"System health status: {getattr(system_health, 'overall_status', 'unknown')}")
            except Exception as e:
            ...
            memory_recovery = initial_stats['memory_info'].percent - recovery_stats['memory_info'].percent
            
            print(f"\nResource recovery:")
            print(f"  CPU: {recovery_stats['cpu_percent']:.1f}% (change: {cpu_recovery:+.1f}%)")
            print(f"  Memory: {recovery_stats['memory_info'].percent:.1f}% (change: {memory_recovery:+.1f}%)")
...
class TestScalabilityTesting:
    ...
    async def test_horizontal_scaling_simulation(self):
        ...
        async def create_worker_instance(worker_id: int, room_count: int):
            ...
            for i in range(room_count):
                ...
                targets = pd.DataFrame({
                    'time_until_transition_seconds': np.random.normal(3600, 1800, 50),
                    'transition_type': np.random.choice(['occupied_to_vacant', 'vacant_to_occupied'], 50),
                ...
                try:
                    ...
                    print(f"Worker {worker_id} model training failed for {room_id}: {e}")
        ...
        for worker_count, rooms_per_worker in scaling_scenarios:
            ...
            try:
                ...
                successful_workers = [r for r in worker_results if not isinstance(r, Exception) and len(r) == 2]
                total_rooms = sum(len(models) for models, stores in successful_workers)
                
                print(f"  Setup: {setup_time:.2f}s, {len(successful_workers)}/{worker_count} workers successful, {total_rooms} total rooms")
                ...
                if successful_workers:
                    ...
                    for worker_models, worker_stores in successful_workers:
                        for room_id, model in worker_models.items():
                            ...
                            task = model.predict(pred_features, datetime.now(timezone.utc), "occupied")
                    ...
                    success_rate = len(successful_predictions) / len(prediction_results) if prediction_results else 0
                    predictions_per_second = len(successful_predictions) / prediction_time if prediction_time > 0 else 0
                    
                    # Scaling assertions
                    assert success_rate > 0.8, f"Prediction success rate {success_rate:.2%} too low with {worker_count} workers"
                    assert predictions_per_second > worker_count * 2, f"Throughput {predictions_per_second:.1f} pred/sec too low for {worker_count} workers"
                    
                    print(f"  Predictions: {len(successful_predictions)}/{len(prediction_results)} successful ({success_rate:.1%}), {predictions_per_second:.1f} pred/sec")
                    
                    # Test horizontal scaling efficiency
                    if worker_count > 1:
                        expected_efficiency = min(worker_count * 0.7, worker_count)  # Account for overhead
                        actual_efficiency = predictions_per_second / (total_rooms / prediction_time) if prediction_time > 0 and total_rooms > 0 else 0
                        
                        scaling_efficiency = actual_efficiency / expected_efficiency if expected_efficiency > 0 else 0
                        
                        print(f"  Scaling efficiency: {scaling_efficiency:.2f} (expected ~{expected_efficiency:.1f}, actual ~{actual_efficiency:.1f})")
                        
                        assert scaling_efficiency > 0.5, f"Horizontal scaling efficiency {scaling_efficiency:.2f} too low"
        ...
        async def simulate_load_balanced_request(worker_pool, request_id):
            ...
            if worker_models:
                ...
                pred_features = pd.DataFrame({'feature_1': [0.5], 'feature_2': [0.5], 'feature_3': [0.5]})
                
                try:
                    result = await model.predict(pred_features, datetime.now(timezone.utc), "occupied")
        ...
        try:
            ...
            worker_pool = await asyncio.gather(*worker_pool_tasks, return_exceptions=True)
            worker_pool = [w for w in worker_pool if not isinstance(w, Exception) and len(w) == 2]
            
            if worker_pool:
                # Simulate load balanced requests
                lb_requests = [simulate_load_balanced_request(worker_pool, i) for i in range(50)]
                
                start_time = time.perf_counter()
                lb_results = await asyncio.gather(*lb_requests, return_exceptions=True)
                lb_time = time.perf_counter() - start_time
                
                successful_lb = [r for r in lb_results if not isinstance(r, Exception) and len(r) == 2 and r[1]]
                lb_success_rate = len(successful_lb) / len(lb_requests)
                lb_throughput = len(successful_lb) / lb_time if lb_time > 0 else 0
                
                assert lb_success_rate > 0.9, f"Load balanced success rate {lb_success_rate:.2%} too low"
                assert lb_throughput > 20, f"Load balanced throughput {lb_throughput:.1f} req/sec too low"
                
                print(f"Load balancing: {len(successful_lb)}/{len(lb_requests)} successful ({lb_success_rate:.1%}), {lb_throughput:.1f} req/sec")
    ...
    async def test_vertical_scaling_simulation(self):
        ...
        scenarios = [
            ...
            {"name": "Maximum Resources", "threads": 8, "batch_size": 100, "models": 20},
        ]
        ...
        for scenario in scenarios:
            ...
            for i in range(scenario['models']):
                ...
                targets = pd.DataFrame({
                    'time_until_transition_seconds': np.random.normal(3600, 1800, data_size),
                    'transition_type': np.random.choice(['occupied_to_vacant', 'vacant_to_occupied'], data_size),
                    'target_time': [datetime.now(timezone.utc) for _ in range(data_size)]
                })
            ...
            engine = FeatureEngineeringEngine(enable_parallel=True, max_workers=scenario['threads'])
            ...
            if models:
                ...
                def make_prediction_sync(model, features):
                    ...
                    try:
                        return loop.run_until_complete(
                            model.predict(features, datetime.now(timezone.utc), "occupied")
                        )
                    finally:
                        loop.close()
                ...
                with ThreadPoolExecutor(max_workers=scenario['threads']) as executor:
                    ...
                    for model in models:
                        ...
                        future = executor.submit(make_prediction_sync, model, pred_features)
            ...
            avg_feature_time = sum(feature_extraction_times) / len(feature_extraction_times) if feature_extraction_times else 0
            
            # Calculate performance metrics
            models_per_second = len(models) / training_time_total if training_time_total > 0 else 0
            predictions_per_second = successful_predictions / concurrent_prediction_time if concurrent_prediction_time > 0 else 0
            resource_efficiency = predictions_per_second / (memory_usage + 1) if memory_usage >= 0 else 0
            ...
            print(f"  Models: {len(models)}, Training: {training_time_total:.2f}s, Memory: +{memory_usage:.1f}MB")
            print(f"  Feature extraction: {avg_feature_time*1000:.2f}ms, Predictions: {predictions_per_second:.1f}/sec")
        ...
        low_performance = next(r for r in performance_results if r['scenario'] == 'Low Resources')
        high_performance = next(r for r in performance_results if r['scenario'] == 'High Resources')
        max_performance = next(r for r in performance_results if r['scenario'] == 'Maximum Resources')
        
        # Performance should improve with more resources
        training_speedup = high_performance['models_per_second'] / low_performance['models_per_second'] if low_performance['models_per_second'] > 0 else 1
        prediction_speedup = high_performance['predictions_per_second'] / low_performance['predictions_per_second'] if low_performance['predictions_per_second'] > 0 else 1
        feature_speedup = low_performance['feature_extraction_time'] / high_performance['feature_extraction_time'] if high_performance['feature_extraction_time'] > 0 else 1
        ...
        assert training_speedup > 1.2, f"Training performance should improve with more resources (got {training_speedup:.2f}x)"
        assert prediction_speedup > 1.1, f"Prediction performance should improve with more resources (got {prediction_speedup:.2f}x)"
        
        # Check resource efficiency doesn't degrade too much at maximum scale
        efficiency_ratio = max_performance['resource_efficiency'] / low_performance['resource_efficiency'] if low_performance['resource_efficiency'] > 0 else 1
        
        print(f"Resource efficiency ratio (Max/Low): {efficiency_ratio:.2f}")
        assert efficiency_ratio > 0.5, f"Resource efficiency degrades too much at scale ({efficiency_ratio:.2f})"
        
        # Memory usage should scale reasonably
        memory_scaling = high_performance['memory_usage_mb'] / max(low_performance['memory_usage_mb'], 1)
        assert memory_scaling < 10, f"Memory usage scales too aggressively ({memory_scaling:.2f}x)"
    ...
    async def test_performance_degradation_detection(self):
        ...
        for _ in range(20):
            ...
            start_time = time.perf_counter()
            prediction = await model.predict(pred_features, datetime.now(timezone.utc), "occupied")
            end_time = time.perf_counter()
        ...
        print(f"Baseline performance - Avg: {baseline_avg:.2f}ms, Max: {baseline_max:.2f}ms")
        ...
        for scenario in degradation_scenarios:
            ...
            if degraded_times:
                ...
                print(f"  Under load - Avg: {degraded_avg:.2f}ms, Max: {degraded_max:.2f}ms, P95: {degraded_p95:.2f}ms")
                print(f"  Degradation - Avg: {avg_degradation:.2f}x, Max: {max_degradation:.2f}x")
                
                # Performance degradation assertions
                assert avg_degradation < 5.0, f"Average performance degraded {avg_degradation:.2f}x - too high"
                assert max_degradation < 10.0, f"Maximum performance degraded {max_degradation:.2f}x - too high"
                assert degraded_p95 < 500, f"P95 latency {degraded_p95:.2f}ms under load - too high"
                ...
                recovery_avg = sum(recovery_times) / len(recovery_times) if recovery_times else baseline_avg
                recovery_ratio = recovery_avg / baseline_avg if baseline_avg > 0 else 1
                
                print(f"  Recovery - Avg: {recovery_avg:.2f}ms, Ratio to baseline: {recovery_ratio:.2f}x")
                
                # Recovery should be reasonable
                assert recovery_ratio < 2.0, f"Performance recovery {recovery_ratio:.2f}x baseline - poor recovery"
        ...
        print(f"Performance trend - Early: {early_avg:.2f}ms, Late: {late_avg:.2f}ms, Degradation: {trend_degradation:.2f}x")
        
        # Trend detection should identify significant degradation
        assert trend_degradation > 1.5, f"Trend degradation {trend_degradation:.2f}x should be detectable"
        ...
        print(f"Performance violations: {performance_violations}/{len(trend_times)} ({violation_rate:.1%})")
        
        assert violation_rate > 0.2, f"Should detect performance violations in degradation test (got {violation_rate:.1%})"
