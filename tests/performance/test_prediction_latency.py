...
class TestPredictionLatency:
    ...
    async def mock_ensemble_model(self):
        ...
        prediction_result = {
            ...
            "alternatives": [
                {
                    "time": pd.Timestamp.now() + pd.Timedelta(minutes=45),
                    "confidence": 0.65,
                },
                {
                    "time": pd.Timestamp.now() + pd.Timedelta(minutes=60),
                    "confidence": 0.45,
                },
            ],
        }
    ...
    async def test_single_prediction_latency(self, predictor):
        ...
        assert (
            mean_latency < 100
        ), f"Mean latency {mean_latency:.2f}ms exceeds 100ms requirement"
    ...
    async def test_batch_prediction_latency(self, predictor):
        ...
        assert (
            per_room_latency < 80
        ), f"Per-room batch latency {per_room_latency:.2f}ms too high"
        assert (
            mean_batch_latency < 400
        ), f"Total batch latency {mean_batch_latency:.2f}ms too high"
    ...
    async def test_cold_start_vs_warm_cache_latency(self, predictor):
        ...
        print(
            f"Cache improvement: {((cold_latency - mean_warm_latency) / cold_latency * 100):.1f}%"
        )
        ...
        assert (
            mean_warm_latency < 100
        ), f"Warm cache latency {mean_warm_latency:.2f}ms exceeds requirement"
    ...
    async def test_prediction_latency_under_load(self, predictor):
        ...
        assert (
            mean_concurrent_latency < 150
        ), f"Mean latency under load {mean_concurrent_latency:.2f}ms too high"
        assert (
            max_concurrent_latency < 250
        ), f"Max latency under load {max_concurrent_latency:.2f}ms too high"
    ...
    async def test_feature_complexity_impact_on_latency(self, predictor):
        ...
        for feature_count, mean_latency in latency_results.items():
            assert (
                mean_latency < 100
            ), f"Latency with {feature_count} features ({mean_latency:.2f}ms) exceeds requirement"
