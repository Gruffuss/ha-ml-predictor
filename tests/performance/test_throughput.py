...
class TestSystemThroughput:
    ...
    async def test_api_endpoint_throughput(self, mock_predictor):
        ...
        with patch('src.integration.api_server.get_predictor', return_value=mock_predictor):
            ...
            assert throughput >= 100, f"API throughput {throughput:.2f} req/s below requirement"
            assert (successful_requests/request_count) >= 0.99, f"Success rate too low"
            assert mean_response_time < 200, f"Mean response time {mean_response_time:.2f}ms too high"
    ...
    async def test_concurrent_prediction_throughput(self, mock_predictor):
        ...
        for concurrent_count, metrics in throughput_results.items():
            expected_min_throughput = min(100, concurrent_count * 0.8)  # Allow some overhead
            ...
            assert metrics['success_rate'] >= 0.98, f"Success rate too low for {concurrent_count} concurrent"
    ...
    async def test_mqtt_publishing_throughput(self, mock_mqtt_publisher):
        ...
        for batch_start in range(0, message_count, batch_size):
            ...
            for i in range(batch_start, min(batch_start + batch_size, message_count)):
                ...
                async def publish_message(room, prediction):
                    ...
                    result = await mock_mqtt_publisher.publish_prediction(room, prediction)
        ...
        assert (successful_publishes/message_count) >= 0.99, "MQTT publish success rate too low"
        assert mean_publish_time < 50, f"Mean publish time {mean_publish_time:.2f}ms too high"
    ...
    async def test_event_processing_throughput(self):
        ...
        for batch_start in range(0, event_count, batch_size):
            ...
            # Process batch
            with patch.object(processor, '_store_event', new_callable=AsyncMock) as mock_store:
                ...
                successful_in_batch = len([r for r in batch_results if not isinstance(r, Exception)])
        ...
        assert throughput >= 500, f"Event processing throughput {throughput:.2f} events/s too low"
        assert (processed_events/event_count) >= 0.95, "Event processing success rate too low"
    ...
    async def test_system_resource_utilization(self, mock_predictor):
        ...
        mean_memory = statistics.mean(memory_samples) if memory_samples else initial_memory
    ...
    async def test_database_operation_throughput(self):
        ...
        with patch('src.data.storage.database.get_database_manager') as mock_db:
            ...
            assert throughput >= 50, f"Database throughput {throughput:.2f} ops/s too low"
            assert mean_op_time < 100, f"Mean DB operation time {mean_op_time:.2f}ms too high"
