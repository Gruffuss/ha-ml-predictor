...
class PredictionValidationFramework:
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            await self.validator.register_calibration_alert_handler(self._handle_calibration_alert)
    ...
    async def validate_prediction(
        ...
        try:
            ...
            record = ValidationRecord(
                ...
                prediction_interval=getattr(prediction_result, 'prediction_interval', None),
                ...
                feature_importance=getattr(prediction_result, 'feature_importance', None)
            )
    ...
    async def validate_actual_outcome(
        ...
        try:
            ...
            self.logger.debug(f"Validated {len(validated_records)} predictions for room {room_id}")
            return validated_records
            
        except Exception as e:
            self.logger.error(f"Error validating actual outcome for room {room_id}: {e}")
    ...
    async def generate_validation_report(
        ...
        try:
            ...
            # Gather core metrics
            accuracy_metrics = self._get_accuracy_metrics_dict(room_id, time_window_hours)
            calibration_metrics = self._get_calibration_metrics_dict(room_id, time_window_hours)
            performance_metrics = self._get_performance_metrics_dict(room_id, time_window_hours)
            
            # Perform trend analysis
            trend_analysis = await self._perform_trend_analysis(room_id, time_window_hours)
            anomaly_detection = await self._perform_anomaly_detection(room_id, time_window_hours)
            ...
            data_quality_score = self._calculate_data_quality_score(room_id, time_window_hours)
            ...
            self.logger.error(f"Error generating validation report for room {room_id}: {e}")
    ...
    async def get_system_health_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            uptime = (current_time - self.framework_metrics["last_restart"]).total_seconds()
            ...
            return {
                ...
                "failed_tasks_24h": len([t for t in self.failed_tasks 
    ...
    async def _schedule_follow_up_tasks(
        ...
        validation_check_task = ValidationTask(
            ...
            parameters={"validation_record_id": validation_record.timestamp.isoformat()}
        )
    ...
    def _get_accuracy_metrics_dict(self, room_id: str, time_window_hours: int) -> Dict[str, Any]:
        """Get accuracy metrics as dictionary."""
        try:
            metrics = self.validator.get_accuracy_metrics(room_id, window_hours=time_window_hours)
            if metrics:
                return {
                    ...
                    "accuracy_level": metrics.accuracy_level.value if metrics.accuracy_level else None
                }
            return {}
        except Exception as e:
    ...
    def _get_calibration_metrics_dict(self, room_id: str, time_window_hours: int) -> Dict[str, Any]:
        """Get calibration metrics as dictionary."""
        try:
            metrics = self.validator.get_calibration_metrics(room_id, window_hours=time_window_hours)
            return metrics if metrics else {}
        except Exception as e:
            self.logger.error(f"Error getting calibration metrics for room {room_id}: {e}")
            return {}
    
    def _get_performance_metrics_dict(self, room_id: str, time_window_hours: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error getting performance metrics for room {room_id}: {e}")
            return {}
    
    async def _perform_trend_analysis(self, room_id: str, time_window_hours: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error performing trend analysis for room {room_id}: {e}")
            return {}
    
    async def _perform_anomaly_detection(self, room_id: str, time_window_hours: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Error performing anomaly detection for room {room_id}: {e}")
            return {}
    ...
    def _calculate_data_quality_score(self, room_id: str, time_window_hours: int) -> float:
        """Calculate data quality score for the room."""
        try:
            records = self.validator.get_validation_history(room_id, window_hours=time_window_hours)
            if not records:
                return 0.0
            
            # Simple quality score based on data completeness and consistency
            complete_records = len([r for r in records if r.status == ValidationStatus.VALIDATED])
    ...
    async def _handle_calibration_alert(self, room_id: str, alert_data: Dict[str, Any]) -> None:
