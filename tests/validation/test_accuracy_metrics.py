...
class TestAccuracyMetricsCalculation:
    ...
    def test_basic_accuracy_metrics_calculation(self, validator, sample_validation_records):
        ...
        expected_accurate = sum(1 for r in sample_validation_records if r.error_minutes <= 15)
        ...
        assert metrics.mean_error_minutes == pytest.approx(statistics.mean(expected_errors), rel=0.01)
        assert metrics.median_error_minutes == pytest.approx(statistics.median(expected_errors), rel=0.01)
        assert metrics.mae_minutes == pytest.approx(statistics.mean(expected_errors), rel=0.01)
    ...
    def test_error_distribution_metrics(self, validator, sample_validation_records):
        ...
        metrics = validator._calculate_metrics_from_records(sample_validation_records, 24)
    ...
    def test_bias_analysis_calculation(self, validator, sample_validation_records):
        ...
        for record in sample_validation_records:
            if record.actual_time and record.predicted_time:
                bias_seconds = (record.actual_time - record.predicted_time).total_seconds()
    ...
    def test_confidence_analysis_calculation(self, validator, sample_validation_records):
        ...
        assert metrics.mean_confidence == pytest.approx(expected_mean_confidence, rel=0.01)
        ...
        expected_correlation = correlation_matrix[0, 1] if not np.isnan(correlation_matrix[0, 1]) else 0.0
        
        assert metrics.confidence_accuracy_correlation == pytest.approx(expected_correlation, abs=0.01)
        
        # Verify calibration score
        expected_calibration = max(0.0, expected_correlation)
        assert metrics.confidence_calibration_score == pytest.approx(expected_calibration, abs=0.01)
    
    def test_overconfidence_underconfidence_rates(self, validator, sample_validation_records):
        ...
        assert metrics.overconfidence_rate == pytest.approx(expected_overconfidence, rel=0.01)
        assert metrics.underconfidence_rate == pytest.approx(expected_underconfidence, rel=0.01)
    ...
    def test_time_based_analysis_metrics(self, validator, sample_validation_records):
        ...
        if period_hours > 0:
            ...
            assert metrics.predictions_per_hour == pytest.approx(expected_rate, rel=0.01)
        else:
...
class TestAccuracyMetricsAggregation:
        ...
        for room in rooms:
            for model in models:
                for hour_offset in range(4):  # 4 records per room-model combination
                    ...
                    error_minutes = 5.0 + (record_id % 8) * 3  # Errors from 5 to 26 minutes
                    actual_time = predicted_time + timedelta(minutes=error_minutes)
    ...
    @pytest.mark.asyncio
    async def test_room_specific_metrics_calculation(self, validator, multi_room_records):
        ...
        living_room_metrics = await validator.get_room_accuracy("living_room", hours_back=24)
        
        # Verify only living room records are included
        expected_living_room_count = sum(1 for r in multi_room_records if r.room_id == "living_room")
        ...
        living_room_records = [r for r in multi_room_records if r.room_id == "living_room"]
    ...
    @pytest.mark.asyncio
    async def test_model_specific_metrics_calculation(self, validator, multi_room_records):
        ...
        expected_lstm_count = sum(1 for r in multi_room_records if r.model_type == "LSTM")
        ...
        assert lstm_metrics.accuracy_rate == pytest.approx(expected_accuracy_rate, rel=0.01)
    ...
    async def test_time_window_filtering(self, validator, multi_room_records):
        ...
        expected_recent_count = sum(
            1 for r in multi_room_records 
            if r.prediction_time >= cutoff_time
        )
