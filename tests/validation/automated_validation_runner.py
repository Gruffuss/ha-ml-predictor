...
class AutomatedValidationRunner:
    ...
    def __init__(
        ...
        self.artifacts_directory = artifacts_directory or Path("validation_runner_artifacts")
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            if self.active_runs:
                self.logger.info(f"Waiting for {len(self.active_runs)} active runs to complete...")
        ...
        try:
            ...
            async with self.run_semaphore:
                ...
                if wait_for_completion:
                    ...
                    asyncio.create_task(self._execute_validation_run(config, run_result))
    ...
    async def schedule_validation_run(
        ...
        if schedule_expression == "daily":
            ...
            if len(parts) == 3 and parts[2] == "minutes":
                ...
                schedule.every(interval).minutes.do(run_scheduled_validation).tag(job_id)
        
        self.scheduled_jobs.append(job_id)
        self.logger.info(f"Scheduled validation run {job_id} with schedule: {schedule_expression}")
    ...
    async def get_runner_statistics(self) -> Dict[str, Any]:
        ...
        successful_runs = len([r for r in self.completed_runs if r.status == ValidationRunStatus.COMPLETED])
        failed_runs = len([r for r in self.completed_runs if r.status == ValidationRunStatus.FAILED])
        
        average_duration = 0.0
        if self.completed_runs:
            durations = [r.duration_seconds for r in self.completed_runs if r.duration_seconds]
            if durations:
                average_duration = sum(durations) / len(durations)
        
        return {
            "runner_status": "active" if not self.shutdown_requested else "shutting_down",
            ...
            "framework_status": (await self.framework.get_system_health_status())["framework_status"]
        }
    ...
    async def _process_single_room(
        ...
        try:
            ...
            # Extract metrics
            room_metrics = {
                "accuracy_percentage": report.accuracy_metrics.get("accuracy_percentage", 0),
                "calibration_score": report.calibration_metrics.get("calibration_score", 0),
                "total_predictions": report.accuracy_metrics.get("total_predictions", 0),
    ...
    async def _generate_run_summary(
        ...
        if run_result.room_results:
            ...
            accuracies = [r.get("accuracy_percentage", 0) for r in run_result.room_results.values()]
            calibrations = [r.get("calibration_score", 0) for r in run_result.room_results.values()]
            confidences = [r.get("confidence_score", 0) for r in run_result.room_results.values()]
            
            run_result.overall_accuracy_percentage = np.mean(accuracies) if accuracies else 0.0
            run_result.overall_calibration_score = np.mean(calibrations) if calibrations else 0.0
            run_result.average_confidence_score = np.mean(confidences) if confidences else 0.0
