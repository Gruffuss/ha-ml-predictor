...
class CalibrationTestCase:
    ...
    name: str
    predictions: List[Tuple[datetime, float, datetime]]  # (predicted_time, confidence, actual_time)
...
class TestConfidenceCalibrationMetrics:
    ...
    def poorly_calibrated_predictions(self):
        ...
        for i in range(10):
            predicted = base_time + timedelta(minutes=i*30)
            actual = predicted + timedelta(minutes=25)  # Poor accuracy despite high confidence
            predictions.append((predicted, 0.9, actual))
        
        # Low confidence (0.4) predictions that are actually very accurate
        for i in range(10):
            predicted = base_time + timedelta(minutes=(i+10)*30)
            actual = predicted + timedelta(minutes=1)  # Excellent accuracy despite low confidence
            predictions.append((predicted, 0.4, actual))
        
        return predictions
    
    def test_calibration_score_well_calibrated(self, validator, well_calibrated_predictions):
    ...
    def test_calibration_score_poorly_calibrated(self, validator, poorly_calibrated_predictions):
    ...
    def test_reliability_diagram_binning(self, validator):
        ...
        for conf_level in confidence_levels:
            for i in range(5):  # 5 predictions per confidence level
                ...
                error_minutes = (1.0 - conf_level) * 20  # Lower confidence = higher error
    ...
    def test_confidence_threshold_filtering(self, validator):
        ...
        high_conf_records = validator.get_high_confidence_predictions("office", 
                                                                    confidence_threshold=0.8, 
                                                                    window_hours=24)
        very_high_conf_records = validator.get_high_confidence_predictions("office", 
...
class TestConfidenceIntervalValidation:
    ...
    def test_adaptive_confidence_calibration(self, validator_with_intervals):
        ...
        for i in range(50):
            ...
            confidence = 0.9
            actual = predicted + timedelta(minutes=12)  # ~12 min error (moderate accuracy)
        ...
        calibration_recommendation = validator_with_intervals.get_confidence_calibration_recommendation(
            "bedroom",
            window_hours=24
        )
...
class TestConfidenceBasedDecisionMaking:
    ...
    def test_confidence_based_prediction_acceptance(self, multi_confidence_validator):
        ...
        for i, (confidence, should_accept, description) in enumerate(test_cases):
            ...
            should_accept_prediction = multi_confidence_validator.should_accept_prediction(
                confidence=confidence,
                room_id="test_room"
            )
    ...
    def test_confidence_weighted_accuracy_metrics(self, multi_confidence_validator):
        ...
        for i, (confidence, error_minutes) in enumerate(prediction_data):
            ...
            multi_confidence_validator._pending_validations["weighted_test"].append(record)
        ...
        standard_metrics = multi_confidence_validator.get_accuracy_metrics("weighted_test", window_hours=24)
        assert weighted_metrics["weighted_mean_error"] <= standard_metrics.mean_error_minutes
...
class TestConfidenceCalibrationIntegration:
    ...
    def test_end_to_end_calibration_workflow(self, integrated_validator):
        ...
        for i in range(20):
            ...
            actual = predicted + timedelta(minutes=20)  # Poor accuracy despite high confidence
        ...
        initial_calibration = integrated_validator.get_calibration_metrics("integration_test", window_hours=24)
        assert initial_calibration["calibration_score"] < 0.6  # Poor initial calibration
        
        # Phase 2: Apply calibration correction and add better-calibrated predictions
        calibration_adjustment = integrated_validator.get_confidence_calibration_recommendation(
            "integration_test", 
            window_hours=24
        )
        ...
        for i in range(20):
            ...
            adjusted_confidence = max(0.1, min(0.99, original_confidence + adjustment_factor))
        ...
        final_calibration = integrated_validator.get_calibration_metrics("integration_test", window_hours=24)
        assert final_calibration["calibration_score"] > initial_calibration["calibration_score"]
    ...
    async def test_real_time_calibration_monitoring(self, integrated_validator):
        ...
        integrated_validator.register_calibration_alert_handler(calibration_alert_handler)
        ...
        for i in range(30):
            ...
            await integrated_validator.validate_prediction_async("real_time_test", actual)
            
            # Check for calibration alerts after sufficient data
            if i > 10 and i % 5 == 0:
                await integrated_validator.check_calibration_status_async("real_time_test")
    ...
    def test_multi_room_calibration_comparison(self, integrated_validator):
        ...
        for i in range(20):
            predicted = base_time + timedelta(minutes=i*15)
            confidence = 0.8
            actual = predicted + timedelta(minutes=8)  # Good accuracy matching confidence
        ...
        assert room_1_data["calibration_ranking"] < room_2_data["calibration_ranking"]  # Lower rank = better