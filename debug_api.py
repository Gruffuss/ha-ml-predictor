...
def test_api_creation():
    ...
    with patch('src.core.config.get_config', return_value=mock_config):
        with patch('src.integration.api_server.get_tracking_manager') as mock_get_tracking:
            with patch('src.integration.api_server.get_database_manager') as mock_get_db:
                with patch('src.integration.api_server.get_mqtt_manager') as mock_get_mqtt:
                    
                    # Mock the tracking manager
                    mock_tracking = Mock()
                    mock_tracking.get_room_prediction = Mock(return_value={
                        "room_id": "living_room",
                        "confidence": 0.85
                    })
