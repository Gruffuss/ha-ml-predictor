...
class TrainingScheduler:
    ...
    async def setup(self):
        ...
        try:
            ...
            training_config = config_manager.get_training_config(TrainingProfile.PRODUCTION)
            ...
            while not self._shutdown_event.is_set():
                try:
                    ...
                    await asyncio.wait_for(
                        self._shutdown_event.wait(), timeout=600
                    )
    ...
    async def _check_scheduled_operations(self):
        ...
        try:
            ...
            # Daily check
            if (self.config["daily_check_enabled"] and 
                current_date > self._last_daily_check and
                current_hour >= self.config["daily_check_hour"]):
                
                await self._run_daily_check()
                self._last_daily_check = current_date
            
            # Weekly training
            if (self.config["weekly_training_enabled"] and
                current_weekday == self.config["weekly_training_day"] and
                current_hour >= self.config["weekly_training_hour"] and
                self._get_current_week() > self._last_weekly_check):
                
                await self._run_weekly_training()
                self._last_weekly_check = self._get_current_week()
            
            # Monthly comprehensive training
            if (self.config["monthly_comprehensive_enabled"] and
                current_date.day == self.config["monthly_training_day"] and
                current_hour >= self.config["monthly_training_hour"] and
                current_date > self._last_monthly_check):
        ...
        try:
            ...
            if self.tracking_manager:
                ...
                if status.get("accuracy_tracker"):
                    active_alerts = len(status["accuracy_tracker"].get("active_alerts", []))
                    if active_alerts > 0:
                ...
                if rooms_needing_attention:
                    logger.info(f"Daily check identified {len(rooms_needing_attention)} rooms needing training")
                    
                    # Run quick training for problematic rooms
                    profile = TrainingProfile(self.config["daily_training_profile"])
                    await self._train_specific_rooms(rooms_needing_attention, profile, "daily_check")
                else:
                    logger.info("Daily check: all rooms performing within acceptable ranges")
    ...
    async def _run_weekly_training(self):
        ...
        try:
            ...
            if rooms_to_train:
                ...
                logger.info(f"Weekly training completed: {success_count} successful, {failure_count} failed")
            else:
    ...
    async def _run_monthly_training(self):
        ...
        try:
            ...
            logger.info(f"Monthly comprehensive training completed: {success_count} successful, {failure_count} failed")
        ...
        try:
            ...
            for room_id in room_ids:
                try:
                    ...
                    accuracy_metrics = await self.tracking_manager.get_accuracy_metrics(room_id)
                    
                    if accuracy_metrics:
                        accuracy_rate = accuracy_metrics.get("accuracy_rate", 1.0)
                        error_minutes = accuracy_metrics.get("average_error_minutes", 0.0)
                        
                        needs_training = (
                            accuracy_rate < self.config["accuracy_threshold"] or
                            error_minutes > self.config["error_threshold_minutes"]
                        )
    ...
    async def _train_rooms_with_profile(
        ...
        try:
            ...
            successful_rooms = [
                room_id for room_id, progress in training_results.items()
                if hasattr(progress, 'stage') and progress.stage.value == "completed"
            ]
            ...
            while not self._shutdown_event.is_set():
                try:
                    ...
                    if not health_ok and self.config["restart_on_health_failure"]:
                        logger.error("System health check failed, initiating restart...")
    ...
    async def _check_accuracy_and_trigger_training(self):
        ...
        try:
            ...
            if rooms_needing_training:
                logger.info(f"Accuracy monitor triggered training for {len(rooms_needing_training)} rooms")
                ...
                self._scheduler_stats["accuracy_triggered_trainings"] += len(rooms_needing_training)
                self._scheduler_stats["successful_trainings"] += success_count
                self._scheduler_stats["failed_trainings"] += failure_count
            ...
            for room_id in room_ids:
                try:
                    ...
                    if drift_metrics and drift_metrics.retraining_recommended:
                        ...
                        logger.info(f"Drift detected in room {room_id}, retraining recommended")
                
                except Exception as e:
                    logger.warning(f"Failed to check drift for room {room_id}: {e}")
            
            if rooms_with_drift:
                logger.info(f"Drift monitor triggered training for {len(rooms_with_drift)} rooms")
                ...
                self._scheduler_stats["drift_triggered_trainings"] += len(rooms_with_drift)
                self._scheduler_stats["successful_trainings"] += success_count
                self._scheduler_stats["failed_trainings"] += failure_count
    ...
    async def shutdown(self):
        ...
        try:
            ...
            uptime = (datetime.utcnow() - self._scheduler_stats["start_time"]).total_seconds() / 3600 if self._scheduler_stats["start_time"] else 0
            logger.info(f"Training scheduler shutdown completed after {uptime:.1f} hours")
    ...
    def get_scheduler_status(self) -> Dict:
        ...
        return {
            ...
            "start_time": self._scheduler_stats["start_time"].isoformat() if self._scheduler_stats["start_time"] else None,
            "uptime_hours": ((datetime.utcnow() - self._scheduler_stats["start_time"]).total_seconds() / 3600) if self._scheduler_stats["start_time"] else 0,
