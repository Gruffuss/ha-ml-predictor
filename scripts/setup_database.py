...
from data.storage.models import Base, create_timescale_hypertables, optimize_database_performance
...
class DatabaseSetup:
    ...
    async def _create_database_and_user(self, force_recreate: bool = False) -> None:
        ...
        try:
            ...
            if force_recreate and db_exists:
                logger.warning(f"Dropping existing database: {self.setup_config['database_name']}")
                ...
                await conn.execute(f"DROP DATABASE IF EXISTS {self.setup_config['database_name']}")
                db_exists = False
            
            if not db_exists:
                logger.info(f"Creating database: {self.setup_config['database_name']}")
                await conn.execute(f"CREATE DATABASE {self.setup_config['database_name']}")
            else:
                logger.info(f"Database already exists: {self.setup_config['database_name']}")
    ...
    async def _verify_setup(self) -> None:
        ...
        try:
            ...
            tables_to_check = [
                'sensor_events', 'room_states', 'predictions',
                'model_accuracy', 'feature_store'
            ]
            ...
            async with db_manager.get_session() as session:
                ...
                try:
                    ...
                    logger.info("TimescaleDB verification skipped (extension not available)")
            ...
            if health['status'] != 'healthy':
                logger.warning(f"Database health check warnings: {health.get('errors', [])}")
            else:
            ...
            async with db_manager.get_session() as session:
                ...
                for room in sample_rooms:
                    for i in range(100):  # 100 events per room
                        timestamp = base_time + timedelta(minutes=random.randint(0, 10080))  # Week span
