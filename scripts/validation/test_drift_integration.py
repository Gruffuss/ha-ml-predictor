...
async def test_drift_integration():
    ...
    assert hasattr(config, 'drift_detection_enabled'), "TrackingConfig missing drift_detection_enabled"
    assert hasattr(config, 'drift_check_interval_hours'), "TrackingConfig missing drift_check_interval_hours"
    assert hasattr(config, 'drift_baseline_days'), "TrackingConfig missing drift_baseline_days"
    ...
    assert tracking_manager.drift_detector is not None, "TrackingManager should have drift detector"
    assert isinstance(tracking_manager.drift_detector, ConceptDriftDetector), "Should be ConceptDriftDetector instance"
    ...
    assert 'drift_detection_enabled' in drift_status, "Status should include drift_detection_enabled"
    assert 'drift_detector_available' in drift_status, "Status should include drift_detector_available"
    assert 'drift_config' in drift_status, "Status should include drift_config"
    assert drift_status['drift_detection_enabled'] == True, "Should show drift detection enabled"
    assert drift_status['drift_detector_available'] == True, "Should show drift detector available"
    ...
    assert 'drift_detector' in overall_status, "Overall status should include drift_detector section"
    assert 'total_drift_checks_performed' in overall_status['performance'], "Performance should include drift check count"
    ...
    task_names = [task.get_name() if hasattr(task, 'get_name') else str(task) for task in tracking_manager._background_tasks]
    logger.info(f"Background tasks: {len(tracking_manager._background_tasks)} tasks running")
...
async def test_drift_configuration_integration():
    ...
    config_disabled = TrackingConfig(
        enabled=True,
    ...
    assert manager_disabled.drift_detector is None, "Should not initialize drift detector when disabled"
    
    drift_status = await manager_disabled.get_drift_status()
    assert drift_status['drift_detection_enabled'] == False, "Status should show disabled"
    assert drift_status['drift_detector_available'] == False, "Should show detector not available"
