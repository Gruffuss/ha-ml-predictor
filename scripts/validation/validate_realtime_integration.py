...
async def validate_realtime_integration():
    ...
    try:
        ...
        assert (
            tracking_manager.realtime_publisher is not None
        ), "Real-time publisher should be initialized automatically"
        ...
        test_prediction = PredictionResult(
            ...
            prediction_metadata={"room_id": "living_room", "test_prediction": True},
        )
        ...
        if total_published > 0:
            print(
                f"   ✅ Real-time broadcasting confirmed: {total_published} predictions published"
            )
        else:
            print(
                "   ⚠️  No real-time clients connected, but system is ready for broadcasting"
            )
        ...
        assert (
            "realtime_publishing" in tracking_status
        ), "Tracking status should include real-time publishing"
        print("   ✅ Real-time publishing status integrated into system status")

        system_stats = await tracking_manager.get_system_stats()
        assert (
            "realtime_publishing_stats" in system_stats
        ), "System stats should include real-time publishing"
        ...
        print(
            "   ✅ No manual setup required - real-time publishing works automatically"
        )
        ...
        print(
            "   ✅ Proper error handling - real-time failures don't break main workflow"
        )
        ...
        print(
            "   • Broadcasts predictions automatically when record_prediction() is called"
        )
