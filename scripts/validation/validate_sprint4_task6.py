...
from src.adaptation.retrainer import AdaptiveRetrainer, RetrainingRequest, RetrainingStrategy, RetrainingTrigger
...
async def test_model_optimizer_standalone():
    ...
    # Test optimization
    performance_context = {
        'accuracy_metrics': type('AccuracyMetrics', (), {
    ...
    print(f"  → Optimization stats: {stats['total_optimizations']} total, {stats['success_rate_percent']:.1f}% success")
...
async def test_optimizer_integration_with_retrainer():
    ...
    try:
        ...
        if model.parameters:
            print(f"  → ✅ Model parameters were optimized: {len(model.parameters)} parameters set")
        else:
...
async def main():
    ...
    print("\n" + "=" * 60)
    if all_passed:
        print("🎉 ALL TESTS PASSED - Model Optimization Engine Successfully Integrated!")
