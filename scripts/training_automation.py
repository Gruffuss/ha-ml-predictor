...
async def run_system_retraining(
    ...
    try:
        ...
        for room_id in room_ids:
            ...
            needs_retraining = (
                mock_accuracy < accuracy_threshold or
                mock_error > error_threshold_minutes
            )
            
            if needs_retraining:
                rooms_needing_retraining.append(room_id)
                logger.info(f"Room {room_id} needs retraining: accuracy={mock_accuracy:.3f}, error={mock_error:.1f}min")
        
        if not rooms_needing_retraining:
            logger.info("No rooms need retraining based on current thresholds")
            return 0, 0
        
        logger.info(f"Retraining {len(rooms_needing_retraining)} rooms that need improvement")
...
async def check_training_status():
    ...
    try:
        ...
        print(f"Average training time: {stats['average_training_time_minutes']:.1f} minutes")
        ...
        if active_pipelines:
            ...
            for pipeline_id, progress in active_pipelines.items():
                ...
                print(f"  {pipeline_id}: Room {room_id}, Stage {stage.value if hasattr(stage, 'value') else stage}, {percent:.1f}%")
        ...
        for profile_name, profile_info in profile_comparison['profiles'].items():
            ...
            print(f"    Training time: {profile_info['max_training_time_minutes']} minutes")
            print(f"    Min accuracy: {profile_info['min_accuracy_threshold']:.2f}")
            print(f"    Max error: {profile_info['max_error_threshold_minutes']:.1f} minutes")
            print(f"    Optimization: {'enabled' if profile_info['optimization_enabled'] else 'disabled'}")
    ...
    try:
        ...
        for profile in TrainingProfile:
            try:
                ...
                if config_issues:
                    validation_warnings.extend([f"{profile.value}: {issue}" for issue in config_issues])
            except Exception as e:
                validation_errors.append(f"Configuration validation failed for {profile.value}: {e}")
        ...
        try:
            training_config = config_manager.get_training_config(TrainingProfile.TESTING)
            training_pipeline = ModelTrainingPipeline(
                config=training_config,
        ...
        try:
            ...
            if hasattr(database_manager, 'health_check'):
                ...
                print("⚠️  Database connectivity: Cannot verify (no health check method)")
        except Exception as e:
...
def main():
    ...
    init_parser.add_argument(
        "--profile", choices=[p.value for p in TrainingProfile],
    ...
    retrain_parser.add_argument(
        "--accuracy-threshold", type=float, default=0.7,
        help="Minimum accuracy threshold for retraining"
    )
    retrain_parser.add_argument(
        "--error-threshold", type=float, default=25.0,
