    ...
    for env in Environment:
        ...
        if env_config.exists():
            ...
            if env == Environment.DEVELOPMENT and base_config.exists():
                environments.append((env.value, "📄 Uses base config", str(base_config)))
            else:
...
def manage_secrets(env_name: str, action: str, key: Optional[str] = None, value: Optional[str] = None) -> None:
...
Examples:
  ...
  %(prog)s export production production_template.yaml
    ...
    setup_parser.add_argument("environment", choices=["development", "testing", "staging", "production"])
    
    # Validate command
    validate_parser = subparsers.add_parser("validate", help="Validate environment configuration")
    validate_parser.add_argument("environment", choices=["development", "testing", "staging", "production"])
    validate_parser.add_argument("--test-connections", action="store_true", help="Test external connections")
    
    # Secrets command
    secrets_parser = subparsers.add_parser("secrets", help="Manage environment secrets")
    secrets_parser.add_argument("environment", choices=["development", "testing", "staging", "production"])
    ...
    backup_parser.add_argument("environment", choices=["development", "testing", "staging", "production"])
    backup_parser.add_argument("--type", choices=["database", "models", "config", "all", "disaster-recovery"], default="all")
    
    # Deploy command
    deploy_parser = subparsers.add_parser("deploy", help="Deploy environment")
    deploy_parser.add_argument("environment", choices=["development", "testing", "staging", "production"])
    deploy_parser.add_argument("--build-only", action="store_true", help="Build images only, don't start services")
    
    # Export command
    export_parser = subparsers.add_parser("export", help="Export environment template")
    export_parser.add_argument("environment", choices=["development", "testing", "staging", "production"])
