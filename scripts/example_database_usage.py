...
async def example_sensor_data_operations():
    ...
    try:
        ...
        async with get_db_session() as session:
            ...
            logger.info(f"Found {len(recent_events)} recent motion events in living room")
...
async def example_room_state_operations():
    ...
    try:
        async with get_db_session() as session:
            ...
            if current_state:
                logger.info(f"Current room state: occupied={current_state.is_occupied}, "
...
async def example_database_health_check():
    ...
    try:
        ...
        logger.info(f"Response time: {health_status['performance_metrics'].get('response_time_ms', 'N/A')}ms")
    ...
    try:
        async with get_db_session() as session:
            ...
            for i, sequence in enumerate(sequences[:3]):  # Show first 3
                logger.info(f"Sequence {i+1}: {len(sequence)} events, "
