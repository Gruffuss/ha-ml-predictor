...
class TrainingConfig:
    ...
    base_models_enabled: List[str] = field(default_factory=lambda: ["lstm", "xgboost", "hmm"])
...
class TrainingProgress:
    ...
    def update_stage(self, new_stage: TrainingStage, details: Optional[Dict[str, Any]] = None):
...
class ModelTrainingPipeline:
    ...
    def __init__(
        ...
        logger.info(f"Initialized ModelTrainingPipeline with artifacts path: {self.artifacts_path}")
    ...
    async def run_initial_training(
        ...
        """
        logger.info(f"Starting initial training pipeline for {len(room_ids) if room_ids else 'all'} rooms")
        ...
        try:
            ...
            for result in results:
                ...
                if progress.stage == TrainingStage.COMPLETED:
                    ...
                    logger.error(f"Initial training failed for room {room_id}: {progress.stage}")
            ...
            logger.info(f"Initial training pipeline completed: {len(training_results)} rooms processed")
    ...
    async def run_retraining_pipeline(
        ...
        Returns:
            Training progress object
        """
        logger.info(f"Starting retraining pipeline for room {room_id}: {trigger_reason}")
        
        try:
            training_type = TrainingType.FULL_RETRAIN if force_full_retrain else TrainingType.ADAPTATION
    ...
    async def train_room_models(
        ...
        try:
            ...
            progress.update_stage(TrainingStage.INITIALIZATION, {
                "room_id": room_id,
            ...
            if not quality_report.passed:
                ...
                if not self._can_proceed_with_quality_issues(quality_report):
                    raise ModelTrainingError(f"Data quality validation failed: {quality_report.recommendations}")
            
            # Stage 4: Feature extraction
            progress.update_stage(TrainingStage.FEATURE_EXTRACTION)
            features_df, targets_df = await self._extract_features_and_targets(raw_data, room_id)
            
            if features_df.empty or targets_df.empty:
                raise ModelTrainingError(f"Feature extraction failed for room {room_id}")
            ...
            best_model_key, evaluation_metrics = await self._evaluate_and_select_best_model(
                trained_models, validation_results, progress
            )
            
            progress.best_model = best_model_key
            progress.best_score = evaluation_metrics.get(self.config.model_selection_metric)
            ...
            training_duration = (datetime.utcnow() - progress.start_time).total_seconds() / 60
            logger.info(
            ...
            raise ModelTrainingError(f"Training pipeline failed for room {room_id}: {str(e)}")
    ...
    async def _prepare_training_data(
        ...
        try:
            logger.debug(f"Preparing training data for room {room_id}, lookback_days={lookback_days}")
            ...
            if not self.database_manager:
                logger.warning("No database manager available - using mock data preparation")
            ...
            logger.debug(f"Prepared {len(raw_data) if raw_data is not None else 0} raw data samples for room {room_id}")
            return raw_data
    ...
    async def _query_room_events(
        ...
        try:
            ...
            logger.debug(f"Querying events for room {room_id} from {start_date} to {end_date}")
            ...
            mock_data = pd.DataFrame({
                ...
                'room_id': room_id,
                'sensor_type': 'motion',
                'state': np.random.choice(['on', 'off'], size=len(pd.date_range(start_date, end_date, freq='5min'))),
                'occupancy_state': np.random.choice(['occupied', 'vacant'], size=len(pd.date_range(start_date, end_date, freq='5min'))),
            })
    ...
    async def _validate_data_quality(
        ...
        try:
            ...
            if 'timestamp' in raw_data.columns:
                ...
                data_freshness_ok = (datetime.utcnow() - latest_timestamp) <= timedelta(hours=24)
            else:
            ...
            missing_values_percent = (raw_data.isnull().sum().sum() / (len(raw_data) * len(raw_data.columns))) * 100
            ...
            passed = (
                ...
                feature_completeness_ok and
                temporal_consistency_ok and
            ...
            report = DataQualityReport(
                ...
                valid_samples=total_samples - int(missing_values_percent * total_samples / 100),
            ...
            if not sufficient_samples:
                report.add_recommendation(f"Insufficient samples: {total_samples} < {self.config.min_samples_per_room}")
            
            if not data_freshness_ok:
                report.add_recommendation("Data is not fresh - consider updating data sources")
            
            if not feature_completeness_ok:
                report.add_recommendation(f"Missing required columns: {missing_columns}")
            ...
            if missing_values_percent > 10:
                report.add_recommendation(f"High missing values: {missing_values_percent:.1f}%")
            
            if duplicates_count > 0:
                report.add_recommendation(f"Found {duplicates_count} duplicate records")
            
            if len(data_gaps) > 0:
                report.add_recommendation(f"Found {len(data_gaps)} significant data gaps")
            
            logger.debug(f"Data quality validation completed for room {room_id}: passed={passed}")
            return report
    ...
    def _can_proceed_with_quality_issues(self, quality_report: DataQualityReport) -> bool:
        """Determine if training can proceed despite quality issues."""
        # Must have sufficient samples and basic completeness
        return (
            quality_report.sufficient_samples and
            quality_report.feature_completeness_ok and
            quality_report.missing_values_percent < 50.0
        )
    ...
    async def _extract_features_and_targets(
        ...
        try:
            ...
            features_df = pd.DataFrame({
                'temporal_hour': raw_data.index.hour if isinstance(raw_data.index, pd.DatetimeIndex) else range(len(raw_data)),
                'temporal_day_of_week': raw_data.index.dayofweek if isinstance(raw_data.index, pd.DatetimeIndex) else range(len(raw_data)) % 7,
            ...
            targets_df = pd.DataFrame({
                'time_until_transition_seconds': np.random.exponential(3600, len(raw_data)),  # 1 hour average
                'transition_type': np.random.choice(['occupied_to_vacant', 'vacant_to_occupied'], len(raw_data)),
            })
            
            logger.debug(f"Extracted {len(features_df)} feature samples and {len(targets_df)} targets for room {room_id}")
    ...
    async def _split_training_data(
        self, features_df: pd.DataFrame, targets_df: pd.DataFrame, progress: TrainingProgress
        ...
        try:
            ...
            logger.debug(f"Data split: train={train_size}, val={val_size}, test={test_size}")
            ...
            for model_type in models_to_train:
                try:
                    ...
                    if model_type == "ensemble":
                        ...
                        logger.warning(f"Direct training of {model_type} not implemented - using ensemble")
                        continue
                    ...
                    if training_result.success:
                        ...
                        logger.info(f"Successfully trained {model_type} for room {room_id}")
                    else:
                        logger.error(f"Training failed for {model_type}: {training_result.error_message}")
                        progress.errors.append(f"{model_type} training failed: {training_result.error_message}")
                    
                except Exception as e:
                    logger.error(f"Failed to train {model_type} for room {room_id}: {e}")
                    progress.errors.append(f"{model_type} training error: {str(e)}")
            
            if not trained_models:
                raise ModelTrainingError("No models were successfully trained")
            
            logger.info(f"Successfully trained {len(trained_models)} models for room {room_id}")
            ...
            for model_name, model in trained_models.items():
                try:
                    ...
                    if 'time_until_transition_seconds' in val_targets.columns:
                        true_values = val_targets['time_until_transition_seconds'].values
                    else:
                    ...
                    validation_results[model_name] = float('inf')  # Worst possible score
    ...
    async def _evaluate_and_select_best_model(
        ...
        try:
            ...
            if not validation_results:
                raise ModelTrainingError("No validation results available for model selection")
            
            # Select best model based on metric
            if self.config.model_selection_metric in ["mae", "rmse"]:
                # Lower is better
                best_model = min(validation_results.keys(), key=lambda k: validation_results[k])
            else:  # r2 or others where higher is better
                best_model = max(validation_results.keys(), key=lambda k: validation_results[k])
            ...
            for model_name, score in validation_results.items():
                evaluation_metrics[f"{model_name}_{self.config.model_selection_metric}"] = score
            
            # Add additional quality metrics (mock implementation)
            evaluation_metrics.update({
                "mae": abs(best_score) if self.config.model_selection_metric != "mae" else best_score,
                "rmse": abs(best_score) * 1.2 if self.config.model_selection_metric != "rmse" else best_score,
                "r2": max(0.0, 1.0 - abs(best_score) / 1000) if self.config.model_selection_metric != "r2" else best_score,
            })
            
            logger.info(f"Best model selected: {best_model} with {self.config.model_selection_metric}={best_score:.3f}")
    ...
    def _meets_quality_thresholds(self, evaluation_metrics: Dict[str, float]) -> bool:
        ...
        meets_error = mae_score <= (self.config.max_error_threshold_minutes * 60)  # Convert to seconds
        ...
        try:
            ...
            for model_name, model in trained_models.items():
                try:
                    ...
                    logger.debug(f"Deployed {model_name} v{model_version} for room {room_id}")
                    
                except Exception as e:
                    logger.error(f"Failed to deploy {model_name}: {e}")
                    progress.errors.append(f"Deployment error for {model_name}: {str(e)}")
            
            logger.info(f"Deployed {len(deployment_info['deployed_models'])} models for room {room_id}")
    ...
    async def _cleanup_training_artifacts(self, pipeline_id: str, keep_best: bool = True):
    ...
    async def _register_trained_models(self, training_results: Dict[str, TrainingProgress]):
            ...
            for room_id, progress in training_results.items():
                if progress.stage == TrainingStage.COMPLETED and progress.best_model:
                    ...
                    if model_key in self._model_registry:
                        ...
                        logger.info(f"Registered model {model_key} with tracking manager")
    ...
    async def _notify_tracking_manager_of_completion(self, progress: TrainingProgress):
        ...
        try:
            ...
            logger.debug(f"Notified tracking manager of training completion for {progress.room_id}")
    ...
    def _update_training_stats(self, training_results: Dict[str, TrainingProgress]):
        ...
        try:
            ...
            successful = sum(1 for p in training_results.values() if p.stage == TrainingStage.COMPLETED)
            ...
            if len(training_results) > 0:
                ...
                new_average = (current_average * (total_pipelines - len(training_results)) + total_time) / total_pipelines
    ...
    async def get_model_performance(self, room_id: str, model_type: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"Failed to get model performance for {room_id}_{model_type}: {e}")
            return None
    
    async def load_model_from_artifacts(self, room_id: str, model_name: str, model_version: str) -> Optional[BasePredictor]:
        ...
        try:
            ...
            logger.info(f"Loaded model {room_id}_{model_name} v{model_version} from artifacts")
            return model
    ...
    async def compare_models(
        ...
        try:
            logger.info(f"Comparing model versions for room {room_id}: {model_versions}")
