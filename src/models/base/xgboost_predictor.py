...
class XGBoostPredictor(BasePredictor):
    ...
    async def train(
        ...
        try:
            ...
            X_train_df = pd.DataFrame(
                X_train_scaled, columns=features.columns, index=features.index
            )
    ...
    def _determine_transition_type(
        self, current_state: str, features: pd.Series, prediction_time: datetime
    ) -> str:
        """
        Determine transition type based on current state and features.
        ...
        if current_state == "occupied":
            ...
            for feature_name in features.index:
                if (
                    "work_hours" in feature_name.lower()
                    and features[feature_name] > 0.5
                ):
                    is_work_hours = True
                elif (
                    "sleep_hours" in feature_name.lower()
                    and features[feature_name] > 0.5
                ):
                    is_sleep_hours = True
    ...
    def get_model_complexity(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "important_features": len(
                [f for f in self.feature_importance_.values() if f > 0.01]
            ),
            "regularization": {
                "reg_alpha": self.model_params["reg_alpha"],
                "reg_lambda": self.model_params["reg_lambda"],
