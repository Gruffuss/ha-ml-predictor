...
class HMMPredictor(BasePredictor):
    ...
    def __init__(self, room_id: Optional[str] = None, **kwargs):
        ...
        # Model components
        self.state_model: Optional[GaussianMixture] = None  # Hidden state identification
    ...
    async def train(
        ...
        try:
            ...
            logger.info(f"Training GMM with {self.model_params['n_components']} components")
            ...
            self._analyze_states(X_train_scaled, state_labels, y_train, features.columns)
            ...
            training_metrics = {
                ...
                'state_distribution': [int(np.sum(state_labels == i)) for i in range(self.model_params['n_components'])]
            }
    ...
    async def predict(
        ...
        try:
            ...
            for idx in range(len(X_scaled)):
                ...
                current_state_probs = self.state_model.predict_proba(X_scaled[idx:idx+1])[0]
                ...
                next_state_probs = self.transition_matrix[most_likely_state] if self.transition_matrix is not None else None
                ...
                result = PredictionResult(
                    ...
                    prediction_metadata={
                        ...
                        'state_label': self.state_labels.get(most_likely_state, f'State_{most_likely_state}'),
                        'all_state_probabilities': current_state_probs.tolist(),
                        'next_state_probabilities': next_state_probs.tolist() if next_state_probs is not None else None
                    }
                )
    ...
    def get_feature_importance(self) -> Dict[str, float]:
        ...
        try:
            ...
            if not hasattr(self.state_model, 'means_') or not hasattr(self.state_model, 'covariances_'):
                return {}
            ...
            for i, feature_name in enumerate(self.feature_names):
                ...
                if self.model_params['covariance_type'] == 'full':
                    ...
                    within_state_variances = [np.mean(np.diag(covariances)) for _ in range(len(means))]
    ...
    def _analyze_states(
        ...
        for state_id in range(self.model_params['n_components']):
            ...
            label = self._assign_state_label(state_id, avg_duration, feature_means, feature_names)
        ...
        for state_id, label in self.state_labels.items():
            ...
            logger.info(f"  {label}: avg_duration={characteristics['avg_duration']:.0f}s, "
    ...
    def _build_transition_matrix(self, state_labels: np.ndarray):
        ...
        for i in range(n_states):
            logger.info(f"  {self.state_labels.get(i, f'State_{i}')} -> "
        ...
        for state_id in range(self.model_params['n_components']):
            ...
            if np.sum(state_mask) < 5:  # Need at least 5 samples
                ...
                if state_id in self.state_durations and self.state_durations[state_id]:
                    ...
                    self.transition_models[state_id] = {'type': 'average', 'value': avg_duration}
                continue
            ...
            self.transition_models[state_id] = {'type': 'regression', 'model': model}
