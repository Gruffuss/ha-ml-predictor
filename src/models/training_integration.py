...
class TrainingIntegrationManager:
    ...
    def __init__(
        ...
        self._last_training_times: Dict[str, datetime] = {}  # room_id -> last_train_time
    ...
    async def _register_tracking_callbacks(self):
        ...
        try:
            ...
            if hasattr(self.tracking_manager, 'add_accuracy_callback'):
                self.tracking_manager.add_accuracy_callback(self._on_accuracy_degradation)
            ...
            if hasattr(self.tracking_manager, 'add_performance_callback'):
                self.tracking_manager.add_performance_callback(self._on_performance_change)
    ...
    async def _on_accuracy_degradation(self, room_id: str, accuracy_metrics: Dict[str, Any]):
        """Handle accuracy degradation notification from tracking manager."""
        try:
            logger.info(f"Accuracy degradation detected for room {room_id}: {accuracy_metrics}")
            ...
            accuracy_threshold = env_config.quality_thresholds.min_accuracy_threshold * 100  # Convert to percentage
            ...
            if should_retrain:
                await self._queue_retraining_request(
                    ...
                    priority=self._calculate_priority(current_accuracy, accuracy_threshold),
                    metadata={
                ...
                logger.debug(f"Accuracy degradation for room {room_id} within acceptable thresholds")
            ...
            if retraining_recommended:
                ...
                await self._queue_retraining_request(
                    ...
                    metadata={
                        ...
                        "immediate_attention_required": drift_metrics.get('immediate_attention_required', False),
                    }
                )
            else:
                logger.debug(f"Drift detected for room {room_id} but retraining not recommended")
            
        except Exception as e:
            logger.error(f"Error handling drift detection for room {room_id}: {e}")
    
    async def _on_performance_change(self, room_id: str, performance_metrics: Dict[str, Any]):
        """Handle model performance change notification."""
        try:
            logger.debug(f"Performance change detected for room {room_id}: {performance_metrics}")
    ...
    async def _queue_retraining_request(
        ...
        try:
            ...
            if room_id in self._active_training_requests:
                logger.info(f"Training already in progress for room {room_id}, skipping request")
                return
            ...
            logger.info(f"Queued retraining request for room {room_id}: {trigger_reason}")
    ...
    async def _process_training_queue(self):
        ...
        try:
            ...
            if active_training_count >= self._max_concurrent_training:
                logger.debug(f"Max concurrent training reached ({active_training_count}), waiting")
                return
            ...
            for request in self._training_queue:
                ...
                try:
                    ...
                    logger.error(f"Failed to execute training request for room {room_id}: {e}")
                    processed_requests.append(request)  # Remove failed request from queue
    ...
    async def _execute_training_request(self, request: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Executing training request for room {room_id}: {trigger_reason}")
    ...
    async def _handle_training_completion(
        ...
        try:
            ...
            if hasattr(progress, 'stage') and progress.stage.value == "completed":
                ...
                logger.error(f"Training failed for room {room_id}: {progress.errors if hasattr(progress, 'errors') else 'Unknown error'}")
    ...
    async def _update_model_registration(self, room_id: str, progress):
        ...
        try:
            ...
            if model_key in model_registry:
                ...
                logger.info(f"Updated model registration for {room_id}: {progress.best_model}")
            
        except Exception as e:
            logger.error(f"Failed to update model registration for room {room_id}: {e}")
    
    async def _handle_training_failure(self, room_id: str, progress, request: Dict[str, Any]):
        ...
        try:
            ...
            if failure_count < max_retries:
                ...
                logger.info(f"Retrying training for room {room_id} (attempt {failure_count + 1})")
                ...
                retry_request["priority"] = min(1, request["priority"])  # Increase priority
                
                # Add back to queue
                self._training_queue.append(retry_request)
                
            else:
                logger.error(f"Training failed permanently for room {room_id} after {failure_count} attempts")
    ...
    async def _cleanup_old_data(self):
        ...
        try:
            ...
            self._training_queue = [
                req for req in self._training_queue
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "rooms_with_active_training": list(self._active_training_requests.keys()),
                "rooms_in_cooldown": [
                    room_id for room_id in self._last_training_times.keys()
                    if not self._can_retrain_room(room_id)
                ],
                "next_queued_rooms": [
    ...
    def set_training_capacity(self, max_concurrent: int):
        ...
        if max_concurrent > 0:
            ...
            logger.info(f"Set training capacity to {max_concurrent} concurrent operations")
        else:
