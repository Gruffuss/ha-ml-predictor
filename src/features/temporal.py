...
class TemporalFeatureExtractor:
    ...
    def extract_features(
        ...
        try:
            ...
            # Time-based features
            features.update(self._extract_time_since_features(sorted_events, target_time))
            ...
            features.update(self._extract_historical_patterns(sorted_events, target_time))
            
            # State transition timing features
            features.update(self._extract_transition_timing_features(sorted_events, target_time))
            
            # Room state duration features if available
            if room_states:
                features.update(self._extract_room_state_features(room_states, target_time))
    ...
    def _extract_time_since_features(
        ...
        features['time_since_last_event'] = min(time_since_last, 86400.0)  # Cap at 24 hours
    ...
    def _extract_duration_features(
        ...
        features['avg_on_duration'] = statistics.mean(on_durations) if on_durations else 1800.0
        features['avg_off_duration'] = statistics.mean(off_durations) if off_durations else 1800.0
    ...
    def _extract_historical_patterns(
        ...
        features['hour_activity_rate'] = (
            ...
            if current_hour_events else 0.5
        )
        ...
        features['day_activity_rate'] = (
            ...
            if current_day_events else 0.5
        )
        
        # Overall activity rate
        all_states = [event.state == 'on' for event in events]
        features['overall_activity_rate'] = sum(all_states) / len(all_states) if all_states else 0.5
        ...
        features['similar_time_activity_rate'] = (
            ...
            if similar_hour_events else 0.5
        )
    ...
    def _extract_transition_timing_features(
        ...
        features['avg_transition_interval'] = statistics.mean(intervals) if intervals else 1800.0
        ...
        if len(recent_events) > 1:
            ...
            features['recent_transition_rate'] = (len(recent_events) - 1) / (recent_duration / 3600)
        else:
        ...
        if len(intervals) > 1:
            ...
            features['time_variability'] = std_interval / mean_interval if mean_interval > 0 else 0.0
        else:
    ...
    def _extract_room_state_features(
        ...
        confidences = [state.occupancy_confidence for state in sorted_states 
        ...
        if len(sorted_states) > 1:
            ...
            for i in range(1, len(sorted_states)):
                duration = (sorted_states[i].timestamp - sorted_states[i-1].timestamp).total_seconds()
                durations.append(duration)
            features['state_stability'] = statistics.mean(durations) / 3600.0  # Convert to hours
        else:
