...
from ..core.exceptions import FeatureExtractionError, DatabaseConnectionError
...
class FeatureCache:
    ...
    def _make_key(self, room_id: str, target_time: datetime, 
                  lookback_hours: int, feature_types: List[str]) -> str:
        """Create cache key from parameters."""
        key_data = f"{room_id}:{target_time.isoformat()}:{lookback_hours}:{':'.join(sorted(feature_types))}"
        return hashlib.md5(key_data.encode()).hexdigest()
    
    def get(self, room_id: str, target_time: datetime, 
            lookback_hours: int, feature_types: List[str], 
            max_age_hours: int = 1) -> Optional[Dict[str, float]]:
        """
    ...
    def put(self, room_id: str, target_time: datetime, 
            lookback_hours: int, feature_types: List[str],
            features: Dict[str, float], data_hash: str):
        """
...
class FeatureStore:
    ...
    async def get_features(
        ...
        if not force_recompute:
            cached_features = self.cache.get(
                ...
                feature_types, cache_max_age_hours
            )
        ...
        if self.enable_persistence and not force_recompute:
            ...
            if db_features is not None:
                ...
                self.cache.put(room_id, target_time, lookback_hours, 
                             feature_types, db_features, data_hash)
                return db_features
        ...
        data_hash = self._compute_data_hash(room_id, target_time, lookback_hours)
        self.cache.put(room_id, target_time, lookback_hours, 
                      feature_types, features, data_hash)
    ...
    async def get_batch_features(
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                logger.error(f"Failed to get features for {room_id} at {target_time}: {result}")
    ...
    async def compute_training_data(
        ...
        """
        logger.info(f"Computing training data for {room_id} from {start_date} to {end_date}")
        ...
        targets_df = pd.DataFrame({
            ...
            'next_transition_time': [t + timedelta(minutes=30) for t in time_points],
            'transition_type': ['occupied_to_vacant'] * len(time_points),
            'room_id': [room_id] * len(time_points)
        })
        
        logger.info(f"Created training data: {len(features_df)} samples, {len(features_df.columns)} features")
    ...
    async def _get_data_for_features(
        ...
        try:
            ...
            async with self.db_manager.get_session() as session:
                ...
                events_query = select(SensorEvent).where(
                    and_(
                ...
                # Query room states
                states_query = select(RoomState).where(
                    and_(
