...
from ..core.constants import (
    SensorType, MAX_SEQUENCE_GAP, MIN_EVENT_SEPARATION,
...
from ..data.ingestion.event_processor import MovementSequence, MovementPatternClassifier
...
class SequentialFeatureExtractor:
    ...
    def extract_features(
        ...
        try:
            ...
            if self.classifier and room_configs:
                features.update(self._extract_movement_classification_features(
    ...
    def _extract_room_transition_features(self, events: List[SensorEvent]) -> Dict[str, float]:
        ...
        # Calculate features
        features['room_transition_count'] = len(room_sequence) - 1 if len(room_sequence) > 1 else 0.0
        features['unique_rooms_visited'] = len(set(room_sequence)) if room_sequence else 1.0
        ...
        features['room_revisit_ratio'] = revisits / total_visits if total_visits > 0 else 0.0
    ...
    def _extract_sensor_sequence_features(self, events: List[SensorEvent]) -> Dict[str, float]:
        ...
        features['sensor_diversity_score'] = entropy / max_entropy if max_entropy > 0 else 0.0
        ...
        features['presence_sensor_ratio'] = presence_count / total_types if total_types > 0 else 0.0
        
        # Door sensor ratio
        door_count = type_counts.get('door', 0)
        features['door_sensor_ratio'] = door_count / total_types if total_types > 0 else 0.0
        
        return features
    
    def _extract_cross_room_features(self, events: List[SensorEvent]) -> Dict[str, float]:
        ...
        features['room_correlation_score'] = (
            ...
            if correlation_windows else 0.0
        )
    ...
    def _extract_movement_classification_features(
        ...
        human_probability = len(human_scores) / total_sequences if total_sequences > 0 else 0.5
        cat_probability = len(cat_scores) / total_sequences if total_sequences > 0 else 0.5
    ...
    def _extract_ngram_features(self, events: List[SensorEvent]) -> Dict[str, float]:
        ...
        for i in range(len(sensor_sequence) - 2):
            trigrams.append((sensor_sequence[i], sensor_sequence[i+1], sensor_sequence[i+2]))
        ...
        pattern_score = (features['common_bigram_ratio'] + features['common_trigram_ratio']) / 2
        ...
        for room_id in set(event.room_id for event in events):
            ...
            for event in room_events:
                if (not current_sequence or 
                    (event.timestamp - current_sequence[-1].timestamp).total_seconds() <= MAX_SEQUENCE_GAP):
