...
class ContextualFeatureExtractor:
    ...
    def extract_features(
        ...
        try:
            ...
            recent_room_states = [r for r in (room_states or []) if r.timestamp >= cutoff_time]
            
            features = {}
            
            # Environmental condition features
            features.update(self._extract_environmental_features(recent_events, target_time))
            
            # Door state and transition features
            features.update(self._extract_door_state_features(recent_events, target_time))
            
            # Multi-room occupancy correlation features
            features.update(self._extract_multi_room_features(
            ...
            if room_configs:
                features.update(self._extract_room_context_features(
    ...
    def _extract_environmental_features(
        ...
        temp_values = self._extract_numeric_values(env_events['temperature'], 'temperature')
        if temp_values:
            ...
            features['temperature_variance'] = statistics.variance(temp_values) if len(temp_values) > 1 else 0.0
            
            # Temperature comfort zones
            current_temp = temp_values[-1]
            features['is_cold'] = 1.0 if current_temp < self.temp_thresholds['cold'] else 0.0
            features['is_comfortable_temp'] = 1.0 if (
                self.temp_thresholds['cold'] <= current_temp <= self.temp_thresholds['warm']
            ) else 0.0
            features['is_warm'] = 1.0 if current_temp > self.temp_thresholds['warm'] else 0.0
        else:
            features.update({
        ...
        # Humidity features
        humidity_values = self._extract_numeric_values(env_events['humidity'], 'humidity')
        if humidity_values:
        ...
        if light_values:
            ...
            features['is_dark'] = 1.0 if current_light < self.light_thresholds['dark'] else 0.0
            features['is_dim'] = 1.0 if (
                self.light_thresholds['dark'] <= current_light < self.light_thresholds['bright']
            ) else 0.0
            features['is_bright'] = 1.0 if current_light >= self.light_thresholds['bright'] else 0.0
        else:
            features.update({
                'current_light': 500.0,
                'avg_light': 500.0,
    ...
    def _extract_door_state_features(
        ...
        # Filter door events
        door_events = [e for e in events if e.sensor_type.lower() == 'door' or 'door' in e.sensor_id.lower()]
        ...
        for door_id in set(e.sensor_id for e in door_events):
            ...
            if door_specific_events:
                ...
                current_door_states[door_id] = current_state == 'on' or current_state == 'open'
        ...
        features['door_open_ratio'] = (
            ...
            if current_door_states else 0.0
        )
    ...
    def _extract_multi_room_features(
        ...
        features['simultaneous_occupancy_ratio'] = occupied_rooms / len(current_occupancy) if current_occupancy else 0.0
        
        # Room activity correlation (events happening in multiple rooms within time windows)
        correlation_score = self._calculate_room_activity_correlation(room_events, target_time)
        features['room_activity_correlation'] = correlation_score
        
        # Dominant room activity ratio
        room_activity_counts = {room_id: len(events) for room_id, events in room_events.items()}
        if room_activity_counts:
            max_activity = max(room_activity_counts.values())
            total_activity = sum(room_activity_counts.values())
            features['dominant_room_activity_ratio'] = max_activity / total_activity if total_activity > 0 else 0.0
        else:
        ...
        if room_activity_counts:
            ...
            features['room_activity_balance'] = entropy / max_entropy if max_entropy > 0 else 0.0
        else:
    ...
    def _extract_seasonal_features(self, target_time: datetime) -> Dict[str, float]:
        ...
        day = target_time.day
        features['is_holiday_season'] = 1.0 if (month == 12 and day >= 20) or (month == 1 and day <= 7) else 0.0
    ...
    def _extract_sensor_correlation_features(self, events: List[SensorEvent]) -> Dict[str, float]:
        ...
        features['multi_sensor_event_ratio'] = (
            multi_sensor_events / len(correlation_windows) if correlation_windows else 0.0
        )
    ...
    def _extract_room_context_features(
        ...
        if room_sensor_counts:
            ...
            avg_sensor_types = statistics.mean([len(types) for types in room_sensor_types.values()])
            ...
            features['room_activity_variance'] = (
                ...
                if len(room_sensor_counts) > 1 else 0.0
            )
        else:
            features.update({
    ...
    def _calculate_room_activity_correlation(
        self, 
        room_events: Dict[str, List[SensorEvent]], 
        target_time: datetime
    ) -> float:
