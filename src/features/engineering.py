...
from ..core.exceptions import FeatureExtractionError, ConfigurationError
...
class FeatureEngineeringEngine:
        ...
        try:
            ...
            if self.enable_parallel and self.executor:
                features = await self._extract_features_parallel(
                    room_events, room_room_states, target_time, 
                    room_config, feature_types, lookback_hours
                )
            else:
            ...
            features.update(self._add_metadata_features(
            ...
            self.stats['avg_extraction_time'] = (
                (self.stats['avg_extraction_time'] * (self.stats['successful_extractions'] - 1) + extraction_time) /
                self.stats['successful_extractions']
            )
            
            logger.debug(f"Extracted {len(features)} features for room {room_id} in {extraction_time:.3f}s")
            return features
            
        except Exception as e:
            self.stats['failed_extractions'] += 1
            logger.error(f"Feature extraction failed for room {room_id}: {e}")
            raise FeatureExtractionError(f"Failed to extract features for room {room_id}: {e}")
    
    async def extract_batch_features(
        self,
        extraction_requests: List[Tuple[str, datetime, List[SensorEvent], List[RoomState]]],
        lookback_hours: int = 24,
        feature_types: Optional[List[str]] = None
    ) -> List[Dict[str, float]]:
        """
        ...
        if self.enable_parallel and len(extraction_requests) > 1:
            ...
            for room_id, target_time, events, room_states in extraction_requests:
                task = self.extract_features(
                    room_id, target_time, events, room_states, 
                    lookback_hours, feature_types
                )
                tasks.append(task)
    ...
    async def _extract_features_sequential(
        ...
        try:
            if 'temporal' in feature_types:
                ...
                prefixed_temporal = {f"temporal_{k}": v for k, v in temporal_features.items()}
        ...
        try:
            if 'sequential' in feature_types:
                ...
                prefixed_sequential = {f"sequential_{k}": v for k, v in sequential_features.items()}
        ...
        try:
            if 'contextual' in feature_types:
                ...
                prefixed_contextual = {f"contextual_{k}": v for k, v in contextual_features.items()}
    ...
    def _add_metadata_features(
        ...
        return {
            ...
            'meta_data_quality_score': min(1.0, event_count / 100.0),  # Normalize event count
        }
    ...
    async def validate_configuration(self) -> Dict[str, Any]:
        ...
        for extractor_name, extractor in extractors.items():
            if extractor is None:
                validation_results['errors'].append(f"{extractor_name} extractor not initialized")
                validation_results['valid'] = False
        
        # Check parallel processing setup
        if self.enable_parallel and self.executor is None:
            validation_results['warnings'].append("Parallel processing enabled but executor not available")
