...
class Incident:
    ...
    def acknowledge(self, acknowledged_by: str):
        ...
        self.updated_at = datetime.now()
        self.add_timeline_entry("Incident acknowledged", {"acknowledged_by": acknowledged_by})
    ...
    def needs_escalation(self) -> bool:
        ...
        if self.created_at < datetime.now() - timedelta(minutes=self.escalation_threshold_minutes):
            return True
...
class IncidentResponseManager:
    ...
    async def _handle_health_incident(self, health: ComponentHealth):
        ...
        for incident in self.active_incidents.values():
            if (incident.component == health.component_name and 
    ...
    async def _attempt_automated_recovery(self, incident: Incident):
        ...
        if component not in self.recovery_actions:
            self.logger.info(f"No recovery actions registered for component {component}")
            return
    ...
    def _check_recovery_conditions(self, action: RecoveryAction, incident: Incident) -> bool:
    ...
    async def _should_auto_resolve_incident(self, incident: Incident) -> bool:
        ...
        return (component_health.is_healthy() and 
                incident.recovery_success and 
        ...
        for incident in self.active_incidents.values():
            if incident.needs_escalation():
                ...
                try:
                    ...
                    self.logger.error(f"Failed to create escalation alert for {incident.incident_id}: {e}")
    ...
    async def _cleanup_incidents(self):
        ...
        for incident_id, incident in list(self.active_incidents.items()):
            if incident.status == IncidentStatus.RESOLVED:
                ...
                if incident.resolved_at and incident.resolved_at < datetime.now() - timedelta(hours=1):
                    incident.status = IncidentStatus.CLOSED
                    incident.add_timeline_entry("Incident closed", {"auto_closed": True})
    ...
    def _update_incident_metrics(self):
        ...
        try:
            ...
            for status in IncidentStatus:
                ...
                self.metrics_collector.set_gauge(f"incidents_active_{status.value}", count)
            
            # Active incidents by severity
            severity_counts = {}
            for severity in IncidentSeverity:
                count = sum(1 for i in self.active_incidents.values() if i.severity == severity)
                severity_counts[severity.value] = count
                self.metrics_collector.set_gauge(f"incidents_active_{severity.value}", count)
            
            # Overall statistics
            self.metrics_collector.set_gauge("incidents_total_created", self.stats["incidents_created"])
            self.metrics_collector.set_gauge("incidents_total_resolved", self.stats["incidents_resolved"])
            self.metrics_collector.set_gauge("auto_recoveries_attempted", self.stats["auto_recoveries_attempted"])
            self.metrics_collector.set_gauge("auto_recoveries_successful", self.stats["auto_recoveries_successful"])
            self.metrics_collector.set_gauge("incident_escalations_total", self.stats["escalations"])
    ...
    async def _recover_memory_usage(self, incident: Incident) -> bool:
        ...
        try:
            ...
            if health:
                ...
                memory_percent = current_health.metrics.get("process_memory_percent", 100)
    ...
    async def resolve_incident(self, incident_id: str, resolution_notes: str) -> bool:
        ...
        if incident_id in self.active_incidents:
            ...
            self.logger.info(
                ...
                extra={"incident_id": incident_id, "resolution_notes": resolution_notes}
            )
