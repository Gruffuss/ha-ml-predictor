...
class HealthMonitor:
    ...
    async def _monitoring_loop(self):
        ...
        while self._monitoring_active:
            try:
                ...
                self.metrics_collector.record_duration('health_check_cycle_duration_seconds', cycle_time)
                
                consecutive_errors = 0
                
                # Log periodic summary
                if len(self.performance_metrics['monitoring_cycle_time']) % 20 == 0:  # Every 20 cycles
                ...
                sleep_time = min(60, self.check_interval * (2 ** min(consecutive_errors, 5)))
    ...
    async def _run_single_health_check(self, name: str, check_func: Callable) -> ComponentHealth:
        ...
        try:
            ...
            # Update metrics
            self.metrics_collector.record_duration(f'health_check_{name}_duration_seconds', response_time)
            self.metrics_collector.set_gauge(f'health_check_{name}_status', 
            ...
            health_result = ComponentHealth(
                ...
                message=error_msg,
                consecutive_failures=self.component_health.get(name, ComponentHealth(
                    name, ComponentType.APPLICATION, HealthStatus.UNKNOWN, datetime.now(), 0.0, ""
    ...
    async def _check_api_endpoints(self) -> ComponentHealth:
        ...
        try:
                    ...
                    try:
                        ...
                        async with session.get(f"{base_url}{endpoint}") as response:
                            ...
                            if response.status in [200, 404]:  # 404 is OK for some endpoints
                                ...
                                issues.append(f"{description} returned {response.status}")
            ...
            avg_response_time = total_response_time / len(endpoints_to_test) if endpoints_to_test else 0.0
            success_rate = successful_checks / len(endpoints_to_test) if endpoints_to_test else 0.0
            ...
            if success_rate >= 1.0:
                if avg_response_time > self.thresholds.response_time_critical:
                    ...
                    status = HealthStatus.HEALTHY
                    message = f"All API endpoints healthy (avg {avg_response_time:.2f}s)"
            elif success_rate >= 0.5:
                status = HealthStatus.DEGRADED
                message = f"Some API endpoints failing ({success_rate*100:.0f}% success)"
            else:
                status = HealthStatus.CRITICAL
                message = f"Most API endpoints failing ({success_rate*100:.0f}% success)"
    ...
    async def _check_network_connectivity(self) -> ComponentHealth:
        ...
        try:
            ...
            avg_response_time = total_response_time / len(test_urls) if test_urls else 0.0
            ...
            if success_rate >= 1.0:
                ...
                status = HealthStatus.WARNING
                message = f"Partial network connectivity ({success_rate*100:.0f}% success)"
            else:
                status = HealthStatus.CRITICAL
                message = f"Network connectivity issues ({success_rate*100:.0f}% success)"
        ...
        try:
            ...
            if hasattr(self.metrics_collector, 'get_recent_metrics'):
                ...
                if 'prediction_latency' in recent_metrics:
                    avg_latency = statistics.mean(recent_metrics['prediction_latency'][-10:])
                    if avg_latency > 2.0:
                        performance_issues.append(f"High prediction latency: {avg_latency:.2f}s")
                
                # Check error rates
                if 'error_count' in recent_metrics:
                    recent_errors = sum(recent_metrics['error_count'][-5:])
                    if recent_errors > 5:
                        performance_issues.append(f"High error rate: {recent_errors} errors in 5 minutes")
            ...
            if self.performance_metrics['monitoring_cycle_time']:
                ...
                if avg_cycle_time > 10.0:  # 10 seconds is too slow
                    performance_issues.append(f"Slow monitoring cycles: {avg_cycle_time:.2f}s")
            ...
            if not performance_issues:
                ...
                status = HealthStatus.DEGRADED
                message = f"Multiple performance issues: {'; '.join(performance_issues[:3])}"
    ...
    def _calculate_system_health(self) -> SystemHealth:
        ...
        healthy_ratio = (healthy_count + warning_count * 0.5) / total_components if total_components > 0 else 0.0
        ...
        return SystemHealth(
            ...
            healthy_components=healthy_count + warning_count,  # Warning is still functional
    ...
    async def _process_incidents(self):
        ...
        for name, health in self.component_health.items():
            ...
            if health.needs_attention() and health.consecutive_failures >= self.alert_threshold:
    ...
    async def _trigger_incident_response(self, health: ComponentHealth):
        ...
        try:
            ...
            self.logger.error(f"Failed to trigger alert for {health.component_name}: {e}")
        ...
        for callback in self.incident_callbacks:
            try:
                ...
                self.logger.error(f"Incident callback failed for {health.component_name}: {e}")
    ...
    def _update_monitoring_metrics(self, system_health: SystemHealth):
        ...
        try:
            ...
            self.metrics_collector.set_gauge('system_health_score', system_health.health_score())
            self.metrics_collector.set_gauge('system_healthy_components', system_health.healthy_components)
            self.metrics_collector.set_gauge('system_degraded_components', system_health.degraded_components)
            self.metrics_collector.set_gauge('system_critical_components', system_health.critical_components)
            self.metrics_collector.set_gauge('system_uptime_seconds', system_health.uptime_seconds)
            ...
            for name, health in self.component_health.items():
                ...
                self.metrics_collector.set_gauge_with_labels('component_health_status', status_value, labels)
                self.metrics_collector.set_gauge_with_labels('component_response_time', health.response_time, labels)
                self.metrics_collector.set_gauge_with_labels('component_consecutive_failures', health.consecutive_failures, labels)
    ...
    def _log_monitoring_summary(self, system_health: SystemHealth):
        ...
        self.logger.info(
            ...
            extra={
                ...
                'memory_usage_gb': system_health.total_memory_gb - system_health.available_memory_gb,
                'active_alerts': system_health.active_alerts,
                'performance_score': system_health.performance_score,
                'avg_monitoring_cycle_time': statistics.mean(self.performance_metrics['monitoring_cycle_time'][-10:]) 
                    if self.performance_metrics['monitoring_cycle_time'] else 0.0
            }
        )
    ...
    def get_component_health(self, component_name: Optional[str] = None) -> Dict[str, ComponentHealth]:
        ...
        if component_name:
            return {component_name: self.component_health.get(component_name)} if component_name in self.component_health else {}
        return self.component_health.copy()
    
    def get_health_history(self, component_name: str, hours: int = 24) -> List[Tuple[datetime, HealthStatus]]:
    ...
    def get_monitoring_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'check_interval': self.check_interval,
            'alert_threshold': self.alert_threshold,
            'avg_cycle_time': statistics.mean(self.performance_metrics['monitoring_cycle_time'][-20:]) 
                if self.performance_metrics['monitoring_cycle_time'] else 0.0,
