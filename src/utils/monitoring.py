...
class PerformanceMonitor:
    ...
    def _check_threshold(self, metric_name: str, value: float,
        ...
        if alert_type:
            alert = AlertEvent(
                ...
                current_value=value,
                threshold=threshold.critical_threshold if alert_type == 'critical' 
    ...
    def get_trend_analysis(self, metric_name: str, room_id: Optional[str] = None,
        ...
        cutoff_time = datetime.now() - timedelta(hours=hours)
        recent_data = [
            (ts, value) for ts, value in self.performance_history[key]
            if ts >= cutoff_time
        ]
        ...
        return {
            ...
            'current_mean': statistics.mean(values[-10:]) if len(values) >= 10 else statistics.mean(values),
            'baseline_mean': statistics.mean(values[:10]) if len(values) >= 20 else statistics.mean(values)
        }
...
class SystemHealthMonitor:
    ...
    def _check_memory_usage(self) -> HealthCheckResult:
        ...
        try:
            ...
            if process_percent > 50:
                ...
                status = 'warning'
                message = f"Moderate memory usage: {process_percent:.1f}% of system memory"
            else:
                status = 'healthy'
                message = f"Memory usage normal: {process_percent:.1f}% of system memory"
    ...
    def get_overall_health_status(self) -> Tuple[str, Dict[str, Any]]:
        ...
        outdated_checks = [
            name for name, last_check in self.last_health_check.items()
...
class MonitoringManager:
    ...
    async def start_monitoring(self, health_check_interval: int = 300,  # 5 minutes
                             performance_summary_interval: int = 900):  # 15 minutes
    ...
    async def _monitoring_loop(self, health_check_interval: int,
                             performance_summary_interval: int):
        ...
        while self._running:
            try:
                ...
                if current_time - last_performance_summary >= performance_summary_interval:
                    summary = self.performance_monitor.get_performance_summary()
                    self.logger.info("Performance summary generated", extra={
                        'summary': summary,
