...
class PerformanceLogger:
    ...
    def log_model_metrics(
        self, room_id: str, model_type: str, metrics: Dict[str, float]
    ):
    ...
    def log_resource_usage(
        self, cpu_percent: float, memory_mb: float, disk_usage_percent: float
    ):
...
class ErrorTracker:
    ...
    def track_error(
        ...
        if severity == "critical":
            self.logger.critical(
                f"Critical error: {error}", extra=error_info, exc_info=True
            )
        else:
            self.logger.error(
                f"Error tracked: {error}", extra=error_info, exc_info=True
            )
    ...
    def track_data_error(
        self, error: Exception, data_source: str, entity_id: Optional[str] = None
    ):
    ...
    def track_integration_error(
        self, error: Exception, integration_type: str, endpoint: Optional[str] = None
    ):
...
class MLOperationsLogger:
    ...
    def log_feature_importance(
        self, room_id: str, model_type: str, feature_importance: Dict[str, float]
    ):
