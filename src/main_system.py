...
class OccupancyPredictionSystem:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if api_status["enabled"] and api_status["running"]:
                logger.info(
                    f"✅ API server automatically started at http://{api_status['host']}:{api_status['port']}"
                )
                logger.info(
            ...
            self.running = True
            logger.info(
                "🎉 System initialization complete - all components integrated automatically!"
            )
    ...
    async def run(self) -> None:
        ...
        if api_status["running"]:
            ...
            logger.info(
                f"   - Available at: http://{api_status['host']}:{api_status['port']}"
            )
