...
class EnhancedIntegrationManager:
    ...
    def __init__(
        ...
        tracking_manager: Optional[
            Any
        ] = None,  # TrackingManager - avoid circular import
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if self.mqtt_integration_manager:
                if (
                    not getattr(self.mqtt_integration_manager, "stats", None)
                    or not self.mqtt_integration_manager.stats.initialized
                ):
                    await self.mqtt_integration_manager.initialize()

            # Initialize HA entity definitions
            if (
                self.mqtt_integration_manager
                and self.mqtt_integration_manager.discovery_publisher
            ):
                ...
                logger.warning(
                    "MQTT integration not available - enhanced features disabled"
                )
        ...
        try:
            ...
            if self.ha_entity_definitions:
                ...
                if entity_config and entity_config.state_topic:
                    ...
                    if (
                        self.mqtt_integration_manager
                        and self.mqtt_integration_manager.mqtt_publisher
                    ):
    ...
    async def process_command(
        ...
        try:
            ...
            logger.info(
                f"Processing HA command: {command} with parameters: {parameters}"
            )
    ...
    async def handle_prediction_update(
        self, room_id: str, prediction_result: PredictionResult
    ) -> None:
        """
        ...
        try:
            ...
            await self.update_entity_state(
                f"{room_id}_time_until", prediction_result.time_until_human
            )
    ...
    async def _define_and_publish_entities(self) -> None:
        ...
        try:
            ...
            self.stats.entities_published = sum(
                1 for r in results.values() if r.success
            )

            logger.info(
                f"Published {self.stats.entities_published}/{self.stats.entities_defined} entities"
            )
    ...
    async def _define_and_publish_services(self) -> None:
        ...
        try:
            ...
            self.stats.services_published = sum(
                1 for r in results.values() if r.success
            )

            logger.info(
                f"Published {self.stats.services_published}/{self.stats.services_defined} services"
            )
    ...
    async def _cleanup_old_responses(self) -> None:
        ...
        try:
            ...
            expired_responses = [
                correlation_id
                for correlation_id, response in self.command_responses.items()
            ...
            if expired_responses:
                logger.debug(
                    f"Cleaned up {len(expired_responses)} expired command responses"
                )
            ...
            if self.tracking_manager:
                if hasattr(self.tracking_manager, "trigger_retraining"):
                    result = await self.tracking_manager.trigger_retraining(
                        room_id=room_id, force=force
                    )
                    return {"status": "success", "result": result}

            return {
                "status": "error",
                "message": "Tracking manager not available",
            }
    ...
    async def _handle_restart_system(
        self, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_refresh_discovery(
        self, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            if (
                ...
                results = (
                    await self.mqtt_integration_manager.discovery_publisher.refresh_discovery()
                )
            ...
            return {
                "status": "error",
                "message": "Discovery publisher not available",
            }

        except Exception as e:
            return {"status": "error", "message": str(e)}

    async def _handle_reset_statistics(
        self, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.mqtt_integration_manager:
                self.mqtt_integration_manager.stats = type(
                    self.mqtt_integration_manager.stats
                )()
    ...
    async def _handle_generate_diagnostic(
        self, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_force_prediction(
        self, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _handle_prediction_enable(
        self, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
