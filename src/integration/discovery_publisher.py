...
class DiscoveryPublisher:
    ...
    def __init__(
        ...
        logger.info(f"Initialized Enhanced DiscoveryPublisher for device: {self.config.device_name}")
        logger.info(f"Device capabilities: {list(self.device_info.capabilities.keys())}")
    ...
    async def publish_all_discovery(self) -> Dict[str, MQTTPublishResult]:
        ...
        try:
            ...
            if successful == total:
                logger.info(f"Successfully published discovery for all {total} entities")
            else:
                failed = total - successful
                self.discovery_stats['discovery_errors'] += failed
                logger.warning(f"Discovery publishing partially failed: {successful}/{total} successful, {failed} failed")
    ...
    def get_discovery_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_availability_publish': self.last_availability_publish.isoformat() if self.last_availability_publish else None,
    ...
    async def publish_device_availability(self, online: bool = True) -> MQTTPublishResult:
        """
        ...
        try:
            ...
            if result.success:
                ...
                logger.info(f"Published device availability: {'online' if online else 'offline'}")
            else:
                logger.error(f"Failed to publish device availability: {result.error_message}")
    ...
    async def cleanup_entities(self, entity_ids: Optional[List[str]] = None) -> Dict[str, MQTTPublishResult]:
        """
    ...
    async def _validate_published_entities(self, results: Dict[str, MQTTPublishResult]) -> None:
        """
    ...
    async def _publish_service_button(self, service: Dict[str, Any]) -> MQTTPublishResult:
        """
        ...
        try:
            ...
            discovery_payload = {
                ...
                "payload_press": json.dumps({"action": service['service_name'], "timestamp": "{{ now().isoformat() }}"})
            }
            ...
            if result.success:
                ...
                logger.error(f"Failed to publish service button {service['friendly_name']}: {result.error_message}")
    ...
    async def _publish_sensor_discovery(self, sensor_config: SensorConfig) -> MQTTPublishResult:
        ...
        try:
            ...
            if hasattr(sensor_config.device, 'suggested_area') and sensor_config.device.suggested_area:
                discovery_payload["device"]["suggested_area"] = sensor_config.device.suggested_area
            if hasattr(sensor_config.device, 'configuration_url') and sensor_config.device.configuration_url:
                discovery_payload["device"]["configuration_url"] = sensor_config.device.configuration_url
            if hasattr(sensor_config.device, 'hw_version') and sensor_config.device.hw_version:
                discovery_payload["device"]["hw_version"] = sensor_config.device.hw_version
            ...
            enhanced_attrs = [
                ...
                'availability_topic', 'availability_template', 'availability_mode',
                'force_update', 'last_reset_topic', 'last_reset_value_template',
                'suggested_display_precision', 'entity_picture', 'command_topic',
                'command_template', 'retain', 'qos'
            ]
            ...
            for attr in enhanced_attrs:
                ...
                if value is not None:
                    ...
                    if attr in ['enabled_by_default', 'force_update', 'retain'] and value is False:
            ...
            if result.success:
                ...
                if sensor_config.unique_id not in self.entity_metadata:
                    metadata = EntityMetadata(
                        ...
                        created_at=datetime.utcnow(),
                        entity_category=EntityCategory.DIAGNOSTIC if sensor_config.entity_category == "diagnostic" else EntityCategory.SYSTEM,
                        state=EntityState.ONLINE,
                        last_seen=datetime.utcnow()
                    )
                ...
                logger.debug(f"Published enhanced discovery for sensor: {sensor_config.name}")
            else:
                logger.error(f"Failed to publish sensor discovery: {result.error_message}")
