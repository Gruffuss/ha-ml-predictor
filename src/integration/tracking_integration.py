...
class TrackingIntegrationManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Tracking integration with real-time publishing initialized successfully"
            )
    ...
    def get_integration_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.enhanced_mqtt_manager:
                stats["enhanced_mqtt_stats"] = (
                    self.enhanced_mqtt_manager.get_integration_stats()
                )
                stats["connection_info"] = (
                    self.enhanced_mqtt_manager.get_connection_info()
                )
                ...
                try:
                    ...
                    if self.enhanced_mqtt_manager:
                        await self.enhanced_mqtt_manager.publish_system_status(
                            ...
                            active_alerts=len(
                                await self.tracking_manager.get_active_alerts()
                            ),
                        )
            ...
            while not self._shutdown_event.is_set():
                try:
                    if self.enhanced_mqtt_manager:
                        ...
                        total_connections = connection_info.get(
                            "total_active_connections", 0
                        )
                        if total_connections > 0:
                            logger.debug(
                                f"Real-time connections: {total_connections} active"
                            )

                        # Check for connection limits
                        ws_connections = connection_info.get(
                            "websocket_connections", {}
                        ).get("total_active_connections", 0)
                        sse_connections = connection_info.get(
                            "sse_connections", {}
                        ).get("total_active_connections", 0)

                        if (
                            ws_connections
                            > self.integration_config.max_websocket_connections
                        ):
                            logger.warning(
                        ...
                        if (
                            sse_connections
                            > self.integration_config.max_sse_connections
                        ):
                            logger.warning(
    ...
    def _handle_alert_broadcast(self, alert_message: Any) -> None:
        ...
        try:
            if (
                not self.enhanced_mqtt_manager
                or not self.integration_config.broadcast_alerts
            ):
                return
...
async def integrate_tracking_with_realtime_publishing(
    ...
    try:
        ...
        logger.error(
            f"Failed to integrate TrackingManager with real-time publishing: {e}"
        )
        raise
