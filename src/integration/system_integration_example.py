...
async def demonstrate_mqtt_integration():
    ...
    try:
        ...
        logger.info("✅ Prediction recorded and automatically published to Home Assistant!")
        
        # Show updated stats
        updated_stats = mqtt_manager.get_integration_stats()
        logger.info(f"Predictions published: {updated_stats['predictions_published']}")
        logger.info(f"Last prediction published: {updated_stats['last_prediction_published']}")
        ...
        logger.info(f"  - Predictions Published: {final_stats['predictions_published']}")
        logger.info(f"  - Status Updates Published: {final_stats['status_updates_published']}")
        ...
        logger.info("\nKey Benefits:")
        logger.info("✅ Automatic prediction publishing - NO manual intervention required")
...
def demonstrate_topic_structure():
    ...
    for room_id, room_config in config.rooms.items():
        ...
        logger.info(f"  Room {room_config.name}:")
        logger.info(f"    Prediction: {config.mqtt.discovery_prefix}/sensor/{device_id}/{device_id}_{room_id}_prediction/config")
        logger.info(f"    Confidence: {config.mqtt.discovery_prefix}/sensor/{device_id}/{device_id}_{room_id}_confidence/config")
        logger.info(f"    Time Until: {config.mqtt.discovery_prefix}/sensor/{device_id}/{device_id}_{room_id}_time_until/config")
