...
class PredictionPublisher:
    ...
    async def publish_prediction(
        ...
        try:
            # Get room name
            room_name = self.room_name_cache.get(room_id, room_id.replace('_', ' ').title())
            ...
            reliability = self._calculate_reliability(prediction_result.confidence_score)
            
            # Extract base model predictions from metadata
            base_predictions = prediction_result.prediction_metadata.get('base_model_predictions', {})
            model_weights = prediction_result.prediction_metadata.get('model_weights', {})
            ...
            for alt_time, alt_confidence in (prediction_result.alternatives or [])[:3]:
                alternatives.append({
                    ...
                    'time_until_seconds': max(0, int((alt_time - datetime.utcnow()).total_seconds()))
                })
    ...
    async def publish_system_status(
        ...
        try:
            ...
            total_predictions = tracking_stats.get('performance', {}).get('total_predictions_recorded', 0) if tracking_stats else 0
            ...
            payload = SystemStatusPayload(
                ...
                database_connected=database_connected,
                mqtt_connected=self.mqtt_publisher.connection_status.connected,
                tracking_active=tracking_stats.get('tracking_active', False) if tracking_stats else False,
                active_alerts=active_alerts,
    ...
    async def _publish_legacy_topics(self, room_id: str, payload: PredictionPayload) -> None:
