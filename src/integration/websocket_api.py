...
class ClientSubscription(BaseModel):
    ...
    endpoint: str = Field(..., pattern=r"^/ws/(predictions|system-status|alerts|room/[a-zA-Z0-9_]+)$")
    room_id: Optional[str] = None
...
class WebSocketConnectionManager:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.message_acknowledgment_timeout = self.config.get("acknowledgment_timeout_seconds", 30)
    ...
    async def authenticate_client(
        ...
        logger.info(f"Client authenticated: {connection_id} ({auth_request.client_name})")
        return True
    ...
    async def subscribe_client(
        ...
        if not connection.authenticated:
            raise WebSocketAuthenticationError("Authentication required for subscriptions")
        
        # Validate room access if room-specific subscription
        if subscription.room_id and not connection.can_access_room(subscription.room_id):
            raise WebSocketValidationError(f"Access denied for room: {subscription.room_id}")
    ...
    async def broadcast_to_endpoint(
        ...
        async with self._lock:
            target_connections = [
                ...
                if (endpoint in conn.subscriptions and
                    conn.authenticated and
                    (not room_filter or room_filter in conn.room_subscriptions))
            ]
        ...
        for connection in target_connections:
            try:
                ...
                logger.warning(f"Failed to send to connection {connection.connection_id}: {e}")
                failed_connections.append(connection.connection_id)
    ...
    async def send_heartbeat(self, connection_id: str) -> bool:
        ...
        heartbeat_message = WebSocketMessage(
            ...
            data={
                ...
                "connection_uptime": (datetime.utcnow() - connection.connected_at).total_seconds()
            }
        )
...
class WebSocketAPIServer:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            acknowledgment_task = asyncio.create_task(self._acknowledgment_timeout_loop())
            
            self._background_tasks.extend([heartbeat_task, cleanup_task, acknowledgment_task])
            
            logger.info("WebSocket API server initialized successfully")
            
        except Exception as e:
            logger.error(f"Failed to initialize WebSocket API server: {e}")
            raise WebSocketConnectionError("Failed to initialize WebSocket server", cause=e)
    ...
    async def publish_prediction_update(
        ...
        try:
            ...
            message = WebSocketMessage(
                ...
                room_id=room_id,
                data=self._format_prediction_data(prediction_result, room_id, current_state)
            )
            ...
            logger.debug(f"Published prediction update for {room_id} to {predictions_sent + room_sent} clients")
        ...
        try:
            ...
            async for raw_message in websocket:
                try:
                    ...
                    await self._send_error_message(
                        connection_id, f"Message processing error: {str(e)}", "MESSAGE_ERROR"
                    )
    ...
    async def _process_client_message(self, connection_id: str, raw_message: str):
        ...
        try:
            ...
            if message_type == MessageType.AUTHENTICATION:
                ...
                await self._send_error_message(
                    connection_id, f"Unsupported message type: {message_type.value}", "UNSUPPORTED_MESSAGE_TYPE"
                )
    ...
    async def _handle_authentication(self, connection_id: str, auth_data: Dict[str, Any]):
        """Handle client authentication request."""
        try:
            auth_request = ClientAuthRequest(**auth_data)
            success = await self.connection_manager.authenticate_client(connection_id, auth_request)
            ...
            await self._send_error_message(connection_id, str(e), "AUTHENTICATION_ERROR")
        except Exception as e:
            logger.error(f"Authentication error for {connection_id}: {e}")
            await self._send_error_message(connection_id, "Authentication processing error", "AUTH_PROCESSING_ERROR")
    
    async def _handle_subscription(self, connection_id: str, subscription_data: Dict[str, Any]):
        """Handle client subscription request."""
        try:
            subscription = ClientSubscription(**subscription_data)
            success = await self.connection_manager.subscribe_client(connection_id, subscription)
            ...
            response_message = WebSocketMessage(
                ...
                data={
                    ...
                    "endpoint": subscription.endpoint,
                    "room_id": subscription.room_id,
                    "message": f"Subscribed to {subscription.endpoint}" if success else "Subscription failed"
                }
            )
            ...
            await self._send_error_message(connection_id, "Subscription processing error", "SUBSCRIPTION_PROCESSING_ERROR")
    
    async def _handle_unsubscription(self, connection_id: str, unsubscription_data: Dict[str, Any]):
        """Handle client unsubscription request."""
        try:
            endpoint = unsubscription_data["endpoint"]
            room_id = unsubscription_data.get("room_id")
            
            success = await self.connection_manager.unsubscribe_client(connection_id, endpoint, room_id)
            ...
            response_message = WebSocketMessage(
                ...
                data={
                    ...
                    "endpoint": endpoint,
                    "room_id": room_id,
                    "message": f"Unsubscribed from {endpoint}" if success else "Unsubscription failed"
                }
            )
            ...
            await self._send_error_message(connection_id, "Unsubscription processing error", "UNSUBSCRIPTION_PROCESSING_ERROR")
    
    async def _handle_heartbeat_response(self, connection_id: str, heartbeat_data: Dict[str, Any]):
    ...
    async def _handle_acknowledgment(self, connection_id: str, ack_data: Dict[str, Any]):
        """Handle message acknowledgment from client."""
        message_id = ack_data.get("message_id")
        if message_id:
            await self.connection_manager.process_acknowledgment(connection_id, message_id)
    
    async def _send_error_message(self, connection_id: str, error_message: str, error_code: str):
        """Send error message to client."""
        error_msg = WebSocketMessage(
            message_id=str(uuid.uuid4()),
            message_type=MessageType.ERROR,
            timestamp=datetime.utcnow(),
            endpoint=self.connection_manager.connections.get(connection_id, {}).endpoint or "unknown",
            data={
                "error": error_message,
    ...
    def _format_prediction_data(
        ...
        config = get_config()
        room_name = config.rooms.get(room_id, {}).get("name", room_id.replace("_", " ").title())
    ...
    async def _heartbeat_loop(self):
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                for connection_id, connection in list(self.connection_manager.connections.items()):
                    if connection.authenticated:
                        try:
                ...
                await asyncio.wait_for(
                    self._shutdown_event.wait(), timeout=self.connection_manager.heartbeat_interval
                )
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                for connection_id, connection in self.connection_manager.connections.items():
                    # Check for timeout
                    if (current_time - connection.last_activity).total_seconds() > self.connection_manager.connection_timeout:
                        stale_connections.append(connection_id)
                    # Check for missed heartbeats
                    elif connection.authenticated and (current_time - connection.last_heartbeat).total_seconds() > (self.connection_manager.heartbeat_interval * 3):
                        stale_connections.append(connection_id)
                ...
                await asyncio.wait_for(self._shutdown_event.wait(), timeout=60)  # Run every minute
            ...
            try:
                ...
                for connection in self.connection_manager.connections.values():
                    ...
                    for msg_id, message in connection.unacknowledged_messages.items():
                        ...
                        if (current_time - message.timestamp).total_seconds() > self.connection_manager.message_acknowledgment_timeout:
                            expired_messages.append(msg_id)
                    
                    # Remove expired messages
                    for msg_id in expired_messages:
                        del connection.unacknowledged_messages[msg_id]
                        logger.warning(f"Message {msg_id} acknowledgment timeout for connection {connection.connection_id}")
                
                await asyncio.wait_for(self._shutdown_event.wait(), timeout=30)  # Check every 30 seconds
    ...
    async def _register_with_tracking_manager(self):
        ...
        try:
            if self.tracking_manager:
                ...
                self.tracking_manager.add_notification_callback(self._handle_tracking_manager_callback)
