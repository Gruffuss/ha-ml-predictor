...
class JWTManager:
    ...
    def refresh_access_token(self, refresh_token: str) -> Tuple[str, str]:
        ...
        permissions = ["read", "write"]  # Default permissions - should be fetched from user store
    ...
    def _create_jwt_token(self, payload: Dict[str, Any]) -> str:
        ...
        header_encoded = self._base64url_encode(json.dumps(header, separators=(',', ':')))
        payload_encoded = self._base64url_encode(json.dumps(payload, separators=(',', ':')))
    ...
    def _decode_jwt_token(self, token: str, verify_expiration: bool = True) -> Dict[str, Any]:
        """
    ...
    def _check_rate_limit(self, user_id: str) -> None:
        ...
        if user_id in self._token_operations:
            self._token_operations[user_id] = [
                op_time for op_time in self._token_operations[user_id]
        ...
        if len(self._token_operations[user_id]) >= self._max_operations_per_minute:
            raise APISecurityError(
                "rate_limit_exceeded",
                f"Too many token operations for user {user_id}"
            )
