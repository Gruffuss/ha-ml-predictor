...
from .ha_entity_definitions import HAEntityDefinitions, HAEntityType, HADeviceClass, HAStateClass
...
class ComprehensiveHAIntegrationDemo:
    ...
    async def initialize_complete_system(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Defined {len(entities)} entities and {len(services)} services")
    ...
    async def demonstrate_entity_types(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Demonstrated {len(demo_results['entity_types_demonstrated'])} entity types")
            return demo_results
    ...
    async def demonstrate_service_execution(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Demonstrated {len(service_demo['services_executed'])} services")
            return service_demo
    ...
    async def get_comprehensive_system_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.ha_entity_definitions:
                stats['entity_definitions'] = self.ha_entity_definitions.get_entity_stats()
            
            # Get discovery publisher stats
            if self.discovery_publisher:
                stats['discovery_publisher'] = self.discovery_publisher.get_discovery_stats()
            ...
            if self.mqtt_integration_manager:
                stats['integration_manager'] = self.mqtt_integration_manager.get_integration_stats()
        ...
        try:
            ...
            for room_id, room_config in self.rooms.items():
                room_data = {
                    ...
                    'model_used': 'ensemble',
                    'alternatives': [
                        {'transition_type': 'vacant', 'predicted_time': (datetime.utcnow() + timedelta(minutes=30)).isoformat(), 'confidence': 0.65}
                    ]
                }
                
                # Publish prediction data
                prediction_topic = f"{self.mqtt_config.topic_prefix}/{room_id}/prediction"
    ...
    async def _demonstrate_binary_sensor_entities(self, results: Dict[str, Any]) -> None:
    ...
    async def _demonstrate_system_services(self, results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            refresh_topic = f"{self.mqtt_config.topic_prefix}/commands/refresh_discovery"
            await self.mqtt_publisher.publish_json(
                topic=refresh_topic,
    ...
    async def _handle_entity_state_change(self, entity_id: str, state: Any, attributes: Optional[Dict[str, Any]] = None) -> None:
...
async def create_comprehensive_ha_integration_demo() -> ComprehensiveHAIntegrationDemo:
    ...
    if init_results.get('errors'):
        raise OccupancyPredictionError(f"Demo initialization failed: {init_results['errors']}")
...
async def run_comprehensive_demo():
    ...
    try:
        ...
        logger.info(f"System statistics: {json.dumps(system_stats, indent=2, default=str)}")
