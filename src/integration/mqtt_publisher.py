...
class MQTTPublisher:
        ...
        try:
            ...
            if not self.client or not self.connection_status.connected:
                ...
                with self._lock:
                    if len(self.message_queue) < self.max_queue_size:
                        ...
                        logger.debug(
                            f"Queued message for topic {topic} (queue size: {len(self.message_queue)})"
                        )
                    else:
                        ...
                        self.message_queue.append(queued_message)
                        logger.warning(
                            f"Message queue full, removed oldest message for topic {topic}"
                        )
            ...
            try:
                ...
                if info.rc == mqtt_client.MQTT_ERR_SUCCESS:
                    ...
                    logger.debug(
                        f"Published message to {topic} (size: {payload_size} bytes)"
                    )
    ...
    async def publish_json(
        self, topic: str, data: Dict[str, Any], qos: int = 1, retain: bool = False
    ) -> MQTTPublishResult:
        """
    ...
    async def _connect_to_broker(self) -> None:
        ...
        while (
            ...
            try:
                logger.info(
                    f"Connecting to MQTT broker {self.config.broker}:{self.config.port} (attempt {attempt + 1})"
                )
                ...
                while (
                    not self.connection_status.connected
                    and not self._shutdown_event.is_set()
                ):
                    if (
                        datetime.utcnow() - wait_start
                ...
                logger.error(
                    f"Failed to connect to MQTT broker (attempt {attempt}): {e}"
                )
    ...
    async def _process_message_queue(self) -> None:
        ...
        try:
            ...
            for message in messages_to_process:
                try:
                    ...
                    if result.success:
                        ...
                        failed += 1
                        logger.warning(
                            f"Failed to publish queued message to {message['topic']}: {result.error_message}"
                        )
            ...
            logger.info(
                f"Processed queued messages: {processed} successful, {failed} failed"
            )
    ...
    def _on_disconnect(self, client, userdata, flags, reason_code, properties):
        ...
        try:
            ...
            if reason_code == 0:
                ...
                logger.warning(
                    f"MQTT client disconnected unexpectedly with reason code: {reason_code}"
                )
