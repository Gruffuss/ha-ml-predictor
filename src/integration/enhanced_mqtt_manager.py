...
from .realtime_publisher import (
    RealtimePublishingSystem, PublishingChannel, RealtimePredictionEvent,
    PublishingMetrics
)
...
class EnhancedMQTTIntegrationManager:
    ...
    def __init__(
        ...
        self.realtime_publisher = RealtimePublishingSystem(
            mqtt_config=mqtt_config,
            rooms=rooms,
            enabled_channels=enabled_realtime_channels or [
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            if hasattr(self.base_mqtt_manager, 'prediction_publisher'):
                self.realtime_publisher.prediction_publisher = self.base_mqtt_manager.prediction_publisher
            ...
            raise EnhancedMQTTIntegrationError("Failed to initialize integration", cause=e)
    ...
    async def publish_prediction(
        ...
        try:
            ...
            publish_latency = (datetime.utcnow() - publish_start_time).total_seconds() * 1000
            ...
            if hasattr(self.base_mqtt_manager, 'stats'):
                ...
                self.base_mqtt_manager.stats.last_prediction_published = datetime.utcnow()
    ...
    async def publish_system_status(
        ...
        try:
            ...
            combined_results = {
                'mqtt': {
                    'success': mqtt_result.success if hasattr(mqtt_result, 'success') else True,
                    'error': getattr(mqtt_result, 'error_message', None)
                },
                **realtime_results
            }
            
            logger.debug(f"Published system status across {len(combined_results)} channels")
    ...
    async def handle_websocket_connection(self, websocket, path: str):
        ...
        return await self.realtime_publisher.handle_websocket_connection(websocket, path)
        ...
        try:
            ...
            return {
                ...
                'channels': {
                    ...
                    'enabled_channels': [
                        channel.value for channel in self.realtime_publisher.enabled_channels
                    ]
                },
                'connections': {
                    'total_clients': self.enhanced_stats.total_clients_connected,
                    'websocket_clients': realtime_stats.get('websocket_stats', {}).get('total_active_connections', 0),
                    'sse_clients': realtime_stats.get('sse_stats', {}).get('total_active_connections', 0)
                }
            }
    ...
    def get_connection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'mqtt_connection': {
                    'connected': getattr(self.base_mqtt_manager, 'mqtt_connected', False)
                },
                'total_active_connections': (
                    realtime_stats.get('websocket_stats', {}).get('total_active_connections', 0) +
                    realtime_stats.get('sse_stats', {}).get('total_active_connections', 0)
                )
            }
    ...
    def _record_publish_performance(self, latency_ms: float, results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            self._publish_latencies = [
                lat for lat, t in zip(self._publish_latencies, self._publish_times)
                if t > cutoff_time
            ]
            
            # Update success rate
            successful_channels = sum(1 for result in results.values() 
    ...
    def _update_enhanced_stats(self) -> None:
        ...
        try:
            ...
            if self._publish_latencies:
                self.enhanced_stats.average_publish_latency_ms = (
            ...
            ws_connections = realtime_stats.get('websocket_stats', {}).get('total_active_connections', 0)
            sse_connections = realtime_stats.get('sse_stats', {}).get('total_active_connections', 0)
            self.enhanced_stats.total_clients_connected = ws_connections + sse_connections
