...
from .discovery_publisher import EntityState, EntityCategory
...
class EnhancedHAIntegrationDemo:
    ...
    async def run_demo(self) -> None:
        ...
        try:
            ...
            logger.info("✅ Enhanced Home Assistant Integration Demo completed successfully!")
    ...
    async def _initialize_integration(self) -> None:
        ...
        logger.info(f"📊 Integration Status: {stats['system_health']['overall_status']}")
    ...
    async def _demo_device_availability(self) -> None:
        ...
        if self.integration_manager.discovery_publisher:
            discovery_stats = self.integration_manager.discovery_publisher.get_discovery_stats()
            logger.info(f"📈 Availability Updates: {discovery_stats['statistics']['availability_updates']}")
    ...
    async def _demo_service_integration(self) -> None:
        ...
        retrain_result = await self.integration_manager.handle_service_command(
            "manual_retrain",
            {"room_id": "living_room", "strategy": "incremental"}
        )
    ...
    async def _demo_entity_state_management(self) -> None:
        ...
        if not self.integration_manager.discovery_publisher:
            logger.warning("Discovery publisher not available for state management demo")
            return
        ...
        for entity_id, metadata in list(entity_metadata.items())[:2]:
            logger.info(f"📋 Entity {entity_id}: state={metadata.state.value}, last_updated={metadata.last_updated}")
    ...
    async def _demo_enhanced_statistics(self) -> None:
        ...
        logger.info(f"🏥 Overall System Status: {health.get('overall_status', 'unknown')}")
        ...
        logger.info(f"🔧 Services Available: {insights.get('services_available', False)}")
        ...
        if 'discovery_publisher' in stats:
            ...
            logger.info(f"📋 Published Entities: {discovery.get('published_entities_count', 0)}")
            logger.info(f"📊 Entity Metadata: {discovery.get('entity_metadata_count', 0)}")
            logger.info(f"🔧 Available Services: {discovery.get('available_services_count', 0)}")
    ...
    async def _demo_cleanup_functionality(self) -> None:
        ...
        if entities_to_cleanup:
            ...
            result = await self.integration_manager.cleanup_discovery(entities_to_cleanup)
