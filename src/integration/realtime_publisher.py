...
from .prediction_publisher import PredictionPublisher, PredictionPayload
...
class WebSocketConnectionManager:
    ...
    async def connect(self, websocket: WebSocketServerProtocol, client_id: str = None) -> str:
    ...
    async def broadcast_to_room(self, room_id: str, event: RealtimePredictionEvent) -> int:
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'connections_by_room': {
                room_id: sum(1 for client in self.client_metadata.values() 
                           if room_id in client.room_subscriptions)
                for room_id in set().union(*(client.room_subscriptions 
            ...
            for client_id, queue in self.connections.items():
                ...
                if client_meta and room_id in client_meta.room_subscriptions:
                    try:
                        ...
                        logger.warning(f"Failed to queue message for SSE client {client_id}: {e}")
                        failed_connections.append(client_id)
...
class RealtimePublishingSystem:
    ...
    async def publish_prediction(
        ...
        try:
            ...
            if (PublishingChannel.MQTT in self.enabled_channels and 
                self.prediction_publisher is not None):
                try:
            ...
            if PublishingChannel.WEBSOCKET in self.enabled_channels:
                try:
                    ws_sent = await self.websocket_manager.broadcast_to_room(room_id, event)
                    results['websocket'] = {
                        'success': True,
            ...
            logger.debug(f"Published prediction for {room_id} across {len(results)} channels")
    ...
    async def create_sse_stream(self, room_id: Optional[str] = None) -> StreamingResponse:
    ...
    def _format_prediction_data(
        ...
        # Get room name
        room_name = self.rooms.get(room_id, {}).get('name', room_id.replace('_', ' ').title())
            ...
            if message_type == 'subscribe':
                ...
                if room_id and room_id in self.rooms:
                    ...
                    response = RealtimePredictionEvent(
                        ...
                        timestamp=datetime.utcnow(),
                        room_id=room_id,
                        data={'message': f'Subscribed to {room_id}', 'status': 'success'}
                    )
                ...
                room_id = message_data.get('room_id')
                if room_id:
                    await self.websocket_manager.unsubscribe_from_room(client_id, room_id)
                    ...
                    response = RealtimePredictionEvent(
                        ...
                        timestamp=datetime.utcnow(),
                        room_id=room_id,
                        data={'message': f'Unsubscribed from {room_id}', 'status': 'success'}
                    )
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                stale_ws_connections = [
                    client_id for client_id, client_meta in self.websocket_manager.client_metadata.items()
                ...
                stale_sse_connections = [
                    client_id for client_id, client_meta in self.sse_manager.client_metadata.items()
                    if client_meta.last_activity < cutoff_time
                ]
