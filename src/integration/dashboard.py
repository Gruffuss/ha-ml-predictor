...
from ..adaptation.validator import AccuracyMetrics, AccuracyLevel
from ..adaptation.tracker import AccuracyTracker, RealTimeMetrics, AccuracyAlert, AlertSeverity, TrendDirection
from ..adaptation.drift_detector import ConceptDriftDetector, DriftMetrics, DriftSeverity, DriftType
from ..adaptation.retrainer import AdaptiveRetrainer, RetrainingRequest, RetrainingStatus, RetrainingTrigger
...
class WebSocketManager:
    ...
    def __init__(self, max_connections: int = 50):
        ...
        logger.info(f"Initialized WebSocketManager with max {max_connections} connections")
    ...
    async def connect(self, websocket: WebSocket, client_info: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"WebSocket connection accepted: {len(self.active_connections)} active connections")
            return True
        ...
        try:
            if websocket in self.active_connections:
                ...
                if websocket in self.connection_metadata:
                    ...
                    self.connection_metadata[websocket]['last_message_at'] = datetime.utcnow()
    ...
    async def broadcast(self, message: Dict[str, Any]) -> int:
        ...
        if successful_sends > 0:
            logger.debug(f"Broadcast message to {successful_sends} WebSocket connections")
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'connections_available': self.max_connections - len(self.active_connections)
            }
...
class PerformanceDashboard:
    ...
    def _register_routes(self, app: FastAPI) -> None:
        ...
        async def get_accuracy_metrics(
            ...
            try:
                metrics = await self._get_accuracy_dashboard_data(room_id, model_type, hours_back)
                return metrics
            except Exception as e:
        ...
        if self.config.enable_retraining_controls and self.config.mode != DashboardMode.READONLY:
        ...
        if self.config.enable_alert_management and self.config.mode != DashboardMode.READONLY:
        ...
        if self.config.websocket_enabled:
            ...
            async def websocket_endpoint(websocket: WebSocket):
                ...
                try:
                    ...
                    await self.websocket_manager.send_personal_message(initial_data, websocket)
            ...
            while self._running:
                try:
                    if self.websocket_manager and self.websocket_manager.active_connections:
                        ...
                        if sent_count > 0:
                            logger.debug(f"Sent dashboard update to {sent_count} WebSocket clients")
    ...
    async def _get_system_overview(self) -> SystemOverview:
        ...
        try:
            ...
            if tracking_status:
                overview.system_health_score = tracking_status.get('overall_health_score', 0.0)
                overview.system_status = tracking_status.get('status', 'unknown')
                
                # Prediction metrics
                accuracy_summary = tracking_status.get('accuracy_summary', {})
                overview.total_predictions_24h = accuracy_summary.get('total_predictions_24h', 0)
                overview.accuracy_rate_24h = accuracy_summary.get('accuracy_rate_24h', 0.0)
                overview.mean_error_minutes_24h = accuracy_summary.get('mean_error_minutes_24h', 0.0)
                
                # Activity metrics
                overview.active_rooms = len(tracking_status.get('tracked_rooms', []))
                overview.active_models = len(tracking_status.get('tracked_models', []))
                overview.predictions_per_hour = accuracy_summary.get('predictions_per_hour', 0.0)
                ...
                overview.active_retraining_tasks = retraining_summary.get('active_tasks', 0)
                overview.completed_retraining_24h = retraining_summary.get('completed_24h', 0)
                
                # Performance indicators
                performance = tracking_status.get('performance_metrics', {})
                overview.avg_prediction_latency_ms = performance.get('avg_prediction_latency_ms', 0.0)
                overview.avg_validation_lag_minutes = performance.get('avg_validation_lag_minutes', 0.0)
                overview.cache_hit_rate = performance.get('cache_hit_rate', 0.0)
            
            # Dashboard-specific metrics
            overview.uptime_hours = (datetime.utcnow() - self._dashboard_start_time).total_seconds() / 3600
    ...
    async def _get_accuracy_dashboard_data(
        ...
        try:
            ...
            real_time_metrics = await self.tracking_manager.get_real_time_metrics(room_id, model_type)
            
            # Get detailed accuracy metrics from validator
            if hasattr(self.tracking_manager, 'validator') and self.tracking_manager.validator:
                accuracy_metrics = await self.tracking_manager.validator.get_accuracy_metrics(
                    room_id=room_id,
            ...
            if real_time_metrics:
                if isinstance(real_time_metrics, list):
                    ...
                    dashboard_data['real_time_metrics'] = [
                        real_time_metrics.to_dict() if hasattr(real_time_metrics, 'to_dict') else real_time_metrics
                    ]
            
            # Process accuracy metrics
            if accuracy_metrics:
                accuracy_dict = accuracy_metrics.to_dict() if hasattr(accuracy_metrics, 'to_dict') else accuracy_metrics
                
                dashboard_data['accuracy_summary'] = {
                    'total_predictions': accuracy_dict.get('total_predictions', 0),
                    'validated_predictions': accuracy_dict.get('validated_predictions', 0),
                    'accuracy_rate': accuracy_dict.get('accuracy_rate', 0.0),
                    'mean_error_minutes': accuracy_dict.get('mean_error_minutes', 0.0),
                    'median_error_minutes': accuracy_dict.get('median_error_minutes', 0.0),
                    'validation_rate': accuracy_dict.get('validation_rate', 0.0)
                }
                
                dashboard_data['error_distribution'] = accuracy_dict.get('error_percentiles', {})
                dashboard_data['confidence_analysis'] = {
                    'mean_confidence': accuracy_dict.get('mean_confidence', 0.0),
                    'confidence_calibration_score': accuracy_dict.get('confidence_calibration_score', 0.0),
                    'overconfidence_rate': accuracy_dict.get('overconfidence_rate', 0.0)
                }
        ...
        try:
            ...
            if drift_status:
                ...
                if room_id and room_id in room_drift_data:
                    dashboard_data['drift_by_room'] = {room_id: room_drift_data[room_id]}
                else:
                    dashboard_data['drift_by_room'] = room_drift_data
                
                # Drift types summary
                dashboard_data['drift_types_detected'] = drift_status.get('drift_types_summary', {})
                
                # Recent drift events
                dashboard_data['recent_drift_events'] = drift_status.get('recent_events', [])
                
                # Configuration
                dashboard_data['drift_configuration'] = drift_status.get('configuration', {})
    ...
    async def _get_retraining_dashboard_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if retraining_status:
                ...
                # Active tasks
                dashboard_data['active_retraining_tasks'] = retraining_status.get('active_tasks', [])
                
                # Recent completions
                dashboard_data['recent_completions'] = retraining_status.get('recent_completions', [])
                
                # History
                dashboard_data['retraining_history'] = retraining_status.get('history', [])
                
                # Performance improvements
                dashboard_data['performance_improvements'] = retraining_status.get('performance_improvements', {})
                
                # Configuration
                dashboard_data['retrainer_configuration'] = retraining_status.get('configuration', {})
        ...
        try:
            ...
            health_data = {
                'overall_health': {
                    'score': 0.0,
                    'status': 'unknown',
                    'uptime_hours': (datetime.utcnow() - self._dashboard_start_time).total_seconds() / 3600
                },
                'component_health': {
            ...
            if self.websocket_manager:
                ...
                health_data['resource_usage']['websocket_connections'] = ws_stats['active_connections']
                health_data['resource_usage']['websocket_capacity'] = ws_stats['max_connections']
    ...
    async def _get_alerts_dashboard_data(
        ...
        try:
            ...
            active_alerts = await self.tracking_manager.get_active_alerts(room_id, severity)
            ...
            if active_alerts:
                # Process alerts
                alerts_list = active_alerts if isinstance(active_alerts, list) else [active_alerts]
    ...
    def _get_dashboard_stats(self) -> Dict[str, Any]:
        """Get dashboard system statistics."""
        try:
            uptime_seconds = (datetime.utcnow() - self._dashboard_start_time).total_seconds()
    ...
    async def _get_websocket_update_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_alerts = await self._get_alerts_dashboard_data(severity=None, room_id=None)
    ...
    async def _handle_websocket_message(self, message: str, websocket: WebSocket) -> None:
    ...
    async def _trigger_manual_retraining(self, request_data: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Manual retraining requested for {room_id}/{model_type}: {result}")
            return {
                'success': True,
