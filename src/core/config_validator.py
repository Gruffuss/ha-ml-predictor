...
class HomeAssistantConfigValidator:
    ...
    def validate(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        if websocket_timeout < 10:
            result.add_warning("WebSocket timeout is very low, may cause connection issues")
        elif websocket_timeout > 300:
            result.add_warning("WebSocket timeout is very high, may cause slow failure detection")
    ...
    def test_connection(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                result.add_error(f"❌ API connection failed: HTTP {response.status_code}")
...
class DatabaseConfigValidator:
    ...
    def validate(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        if "timescaledb" not in conn_str and "TimescaleDB" not in conn_str:
            result.add_warning("TimescaleDB extension is recommended for time-series data")
        ...
        if pool_size < 2:
            ...
            result.add_warning("Database pool size is very high, may consume too many connections")
    ...
    def test_connection(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            async def test_db():
                try:
                    ...
                    if conn_str.startswith("postgresql+asyncpg://"):
                        asyncpg_url = conn_str.replace("postgresql+asyncpg://", "postgresql://")
                    else:
                    ...
                    try:
                        ...
                        if ts_version:
                            result.add_info(f"✅ TimescaleDB extension found: {ts_version}")
                        else:
                    ...
                    result.add_error("❌ Authentication failed: Invalid username/password")
            ...
            result.add_warning("⚠️  Cannot test database connection: asyncpg not available")
        except Exception as e:
...
class MQTTConfigValidator:
    ...
    def validate(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        if not topic_prefix:
            result.add_warning("Topic prefix is empty, messages will be published to root topic")
        ...
        if mqtt_config.get("discovery_enabled", True):
            ...
            if not discovery_prefix:
                result.add_error("Discovery prefix is required when discovery is enabled")
    ...
    def test_connection(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            if connection_result["error"]:
                result.add_error(f"❌ MQTT connection failed: {connection_result['error']}")
...
class RoomsConfigValidator:
    ...
    def validate(self, rooms_config: Dict[str, Any]) -> ValidationResult:
        ...
        if total_sensors < 5:
            result.add_warning("Very few sensors configured, predictions may be less accurate")
        
        return result

    def _validate_room(self, room_id: str, room_data: Dict[str, Any]) -> ValidationResult:
        ...
        if missing_essential:
            result.add_warning(f"Room {room_id} missing essential sensor types: {missing_essential}")
        
        # Check for duplicate entity IDs (across all rooms - would need global context)
        result.add_info(f"Room {room_id}: {len(sensor_types)} sensor types, {len(entity_ids)} entities")
...
class SystemRequirementsValidator:
    ...
    def validate(self, config: Dict[str, Any]) -> ValidationResult:
        ...
        required_packages = [
            "asyncio", "aiohttp", "asyncpg", "sqlalchemy", "pydantic",
            "fastapi", "paho.mqtt", "numpy", "pandas", "scikit-learn"
        ]
        ...
        try:
            ...
            result.add_info(f"Total memory: {memory_gb:.1f} GB, Available: {available_gb:.1f} GB")
...
class ConfigurationValidator:
    ...
    def validate_configuration(self, 
        ...
        if strict_mode:
            ...
            if result.warnings:
                result.add_error(f"Strict mode: {len(result.warnings)} warnings treated as errors")
        ...
        if result.is_valid:
            ...
            result.add_info(f"💥 Configuration validation failed with {len(result.errors)} errors")
        
        logger.info(f"Configuration validation completed: {'PASSED' if result.is_valid else 'FAILED'}")
    ...
    def validate_config_files(self, 
                            ...
                            test_connections: bool = False) -> ValidationResult:
