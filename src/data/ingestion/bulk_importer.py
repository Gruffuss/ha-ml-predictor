...
class BulkImporter:
    ...
    async def import_historical_data(
        ...
        if start_date is None:
            start_date = end_date - timedelta(days=30 * self.import_config.months_to_import)
        
        # Get entity list
        if entity_ids is None:
            entity_ids = self.config.get_all_entity_ids()
        
        logger.info(f"Starting bulk import of {len(entity_ids)} entities from {start_date} to {end_date}")
        ...
        logger.info(f"Bulk import completed. Processed {self.progress.processed_events} events")
    ...
    async def _load_resume_data(self):
        ...
        try:
            ...
            self._completed_entities = set(self._resume_data.get('completed_entities', []))
            
            logger.info(f"Loaded resume data: {len(self._completed_entities)} entities already processed")
    ...
    async def _estimate_total_events(
        ...
        if sample_events > 0:
            ...
            avg_events_per_entity_per_day = sample_events / (sample_size * 1)  # 1 day sample
            estimated_total = int(avg_events_per_entity_per_day * len(entity_ids) * sample_days)
    ...
    async def _process_entities_batch(
        ...
        remaining_entities = [e for e in entity_ids if e not in self._completed_entities]
        ...
        # Process in batches
        for i in range(0, len(remaining_entities), self.import_config.entity_batch_size):
            ...
            tasks = [
                self._process_entity_with_semaphore(semaphore, entity_id, start_date, end_date)
    ...
    async def _process_history_chunk(
        ...
        try:
            ...
            if self.import_config.validate_events:
                processed_events = await self.event_processor.process_event_batch(ha_events)
            else:
                # Skip validation for faster processing
                processed_events = await self._convert_ha_events_to_sensor_events(ha_events)
            ...
            logger.error(f"Error processing history chunk for {entity_id}: {e}")
            self.stats['database_errors'] += 1
            return 0
    
    def _convert_history_record_to_ha_event(self, record: Dict[str, Any]) -> Optional[HAEvent]:
    ...
    async def _generate_import_report(self):
        ...
        report = {
            ...
            'data_quality': {
                'success_rate': (self.stats['events_imported'] / max(self.progress.processed_events, 1)) * 100,
                'error_rate': (len(self.progress.errors) / max(self.progress.processed_events, 1)) * 100
            }
        }
        ...
        if hasattr(self.import_config, 'report_file') and self.import_config.report_file:
            try:
    ...
    async def validate_data_sufficiency(
        ...
        try:
            async with get_db_session() as session:
                ...
                avg_events_per_day = sum(row.event_count for row in daily_counts) / total_days
                ...
                return {
                    ...
                    'recommendation': self._generate_sufficiency_recommendation(
                        sufficient_days, sufficient_events, total_days, avg_events_per_day
                    )
                }
