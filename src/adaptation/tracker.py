...
from .validator import PredictionValidator, AccuracyMetrics, ValidationRecord, AccuracyLevel
...
class RealTimeMetrics:
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.overall_health_score >= 70 and
            self.window_6h_accuracy >= 60 and
            self.accuracy_trend != TrendDirection.DEGRADING
        )
...
class AccuracyAlert:
    ...
    def resolve(self) -> None:
        ...
        logger.info(f"Alert {self.alert_id} resolved after {self.age_minutes:.1f} minutes")
...
class AccuracyTracker:
    ...
    def __init__(
        ...
        self._accuracy_history: Dict[str, deque] = defaultdict(lambda: deque(maxlen=trend_analysis_points))
        ...
        try:
            with self._lock:
                if room_id and model_type:
                    ...
                    return self._metrics_by_room.get(key) or self._metrics_by_model.get(key)
            ...
            with self._lock:
                return {
                    ...
                    'trend_tracking': {
                        ...
                        'total_data_points': sum(len(h) for h in self._accuracy_history.values())
                    },
                    'configuration': {
    ...
    async def _update_real_time_metrics(self) -> None:
        ...
        try:
            ...
            for room_id, model_type in entities:
                metrics = await self._calculate_real_time_metrics(room_id, model_type, current_time)
                if metrics:
    ...
    async def _calculate_real_time_metrics(
        ...
        try:
            ...
            real_time_metrics = RealTimeMetrics(
                ...
                confidence_calibration=window_metrics['24h'].confidence_calibration_score,
            ...
            with self._lock:
                for alert in self._active_alerts.values():
                    if (alert.room_id == room_id or room_id is None) and \
                       (alert.model_type == model_type or model_type is None) and \
                       not alert.resolved:
            ...
            logger.error(f"Failed to calculate real-time metrics for {room_id}, {model_type}: {e}")
            return None
    ...
    def _analyze_trend(self, data_points: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            values = [point['accuracy_6h'] for point in data_points if 'accuracy_6h' in point]
    ...
    def _calculate_global_trend(self, individual_trends: Dict[str, Any]) -> Dict[str, Any]:
            ...
            with self.validator._lock:
                for record in self.validator._validation_records.values():
                    if record.validation_time and record.validation_time >= cutoff_time:
                        ...
                        lag = (record.validation_time - record.prediction_time).total_seconds() / 60
                        lags.append(lag)
    ...
    async def _check_entity_alerts(self, entity_key: str, metrics: RealTimeMetrics) -> None:
        ...
        try:
            ...
            if metrics.window_6h_predictions >= 5:  # Only alert if enough predictions
                if metrics.window_6h_accuracy < self.alert_thresholds['accuracy_critical']:
                ...
                elif metrics.window_6h_accuracy < self.alert_thresholds['accuracy_warning']:
                    alerts_to_create.append({
            ...
            if (metrics.accuracy_trend == TrendDirection.DEGRADING and 
                metrics.trend_confidence > 0.5 and
                metrics.trend_slope < self.alert_thresholds['trend_degrading']):
                alerts_to_create.append({
            ...
            if metrics.validation_lag_minutes > self.alert_thresholds['validation_lag_critical']:
                alerts_to_create.append({
            ...
            elif metrics.validation_lag_minutes > self.alert_thresholds['validation_lag_warning']:
                alerts_to_create.append({
            ...
            for alert_spec in alerts_to_create:
                ...
                existing_alert = None
                for alert in self._active_alerts.values():
                    if (alert.room_id == metrics.room_id and
                        alert.model_type == metrics.model_type and
                        alert.trigger_condition == alert_spec['condition'] and
                        not alert.resolved):
    ...
    async def _cleanup_resolved_alerts(self) -> None:
        ...
        try:
            with self._lock:
                ...
                if alerts_to_remove:
                    logger.debug(f"Cleaned up {len(alerts_to_remove)} old resolved alerts")
    ...
    async def _should_auto_resolve_alert(self, alert: AccuracyAlert) -> bool:
        ...
        try:
            ...
            logger.error(f"Failed to check auto-resolution for alert {alert.alert_id}: {e}")
            return False
    
    async def _notify_alert_callbacks(self, alert: AccuracyAlert, escalation: bool = False) -> None:
