...
class MonitoringEnhancedTrackingManager:
    ...
    async def _monitored_record_prediction(
        self,
        room_id: str,
        ...
        **kwargs,
        ...
        async with self.monitoring_integration.track_prediction_operation(
            room_id=room_id, prediction_type=prediction_type, model_type=model_type_str
        ):
            ...
            if (
                hasattr(prediction_result, "confidence")
                and prediction_result.confidence is not None
            ):
    ...
    async def _monitored_validate_prediction(
        self, room_id: str, actual_time: datetime, **kwargs
    ) -> Dict[str, Any]:
    ...
    async def track_model_training(
        self, room_id: str, model_type: str, training_type: str = "retraining"
    ):
...
def get_enhanced_tracking_manager(
    config: TrackingConfig, **kwargs
) -> MonitoringEnhancedTrackingManager:
