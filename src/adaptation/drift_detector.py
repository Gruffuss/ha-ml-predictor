...
from ..data.storage.models import SensorEvent, Prediction, FeatureStore
...
class DriftMetrics:
    ...
    def _generate_recommendations(self) -> None:
        ...
        self.immediate_attention_required = (
            self.drift_severity == DriftSeverity.CRITICAL or
            self.accuracy_degradation > 25 or
            self.ph_drift_detected
        )
        
        # Retraining recommended for moderate+ drift or significant degradation
        self.retraining_recommended = (
            self.drift_severity in [DriftSeverity.MODERATE, DriftSeverity.MAJOR, DriftSeverity.CRITICAL] or
            self.accuracy_degradation > 15 or
            self.overall_drift_score > 0.5
        )
...
class ConceptDriftDetector:
    ...
    async def _analyze_prediction_drift(
        ...
        try:
            ...
            if baseline_metrics and current_metrics:
                ...
                baseline_errors = [r.error_minutes for r in baseline_metrics.recent_records]
                current_errors = [r.error_minutes for r in current_metrics.recent_records]
                ...
                drift_metrics.confidence_calibration_drift = abs(current_conf - baseline_conf)
    ...
    async def _analyze_feature_drift(
        ...
        try:
            ...
            if baseline_features is not None and current_features is not None:
                ...
                common_features = set(baseline_features.columns) & set(current_features.columns)
                ...
                if len(drifting_features) > 0 or drift_metrics.psi_score > self.psi_threshold:
    ...
    async def _test_categorical_drift(
        ...
        try:
            ...
            # Chi-square test
            chi2_stat, chi2_p, _, _ = stats.chi2_contingency([baseline_aligned, current_aligned])
            
            # Calculate basic statistics
            baseline_stats = {
                'mode': baseline.mode().iloc[0] if len(baseline.mode()) > 0 else None,
                'unique_count': baseline.nunique(),
                'most_frequent_pct': baseline_counts.iloc[0] / len(baseline) if len(baseline_counts) > 0 else 0
            }
            
            current_stats = {
                'mode': current.mode().iloc[0] if len(current.mode()) > 0 else None,
                'unique_count': current.nunique(),
                'most_frequent_pct': current_counts.iloc[0] / len(current) if len(current_counts) > 0 else 0
            }
    ...
    def _calculate_numerical_psi(
        ...
        try:
            ...
            baseline_dist = pd.cut(baseline, bins=bin_edges).value_counts(normalize=True).sort_index()
            current_dist = pd.cut(current, bins=bin_edges).value_counts(normalize=True).sort_index()
            
            # Align distributions
            baseline_aligned = baseline_dist.reindex(baseline_dist.index, fill_value=0.001)
            current_aligned = current_dist.reindex(baseline_dist.index, fill_value=0.001)
            
            # Calculate PSI
            psi = np.sum((current_aligned - baseline_aligned) * np.log(current_aligned / baseline_aligned))
    ...
    def _calculate_categorical_psi(
        ...
        try:
            ...
            baseline_aligned = pd.Series([baseline_dist.get(cat, 0.001) for cat in all_categories], index=all_categories)
            current_aligned = pd.Series([current_dist.get(cat, 0.001) for cat in all_categories], index=all_categories)
    ...
    async def _run_page_hinkley_test(
        ...
        try:
            ...
            errors = await self._get_recent_prediction_errors(room_id, days=self.current_days)
            ...
            if len(errors) > 10:
                ...
                for error in errors:
                    ...
                    # Update minimum
                    self._ph_min[room_id] = min(self._ph_min[room_id], self._ph_sum[room_id])
    ...
    def _calculate_statistical_confidence(self, drift_metrics: DriftMetrics) -> None:
        ...
        try:
            ...
            sample_size_factor = min(
                (drift_metrics.sample_size_baseline + drift_metrics.sample_size_current) / 200,
                1.0
            )
    ...
    async def _get_feature_data(
        ...
        try:
            ...
            logger.debug(f"Getting feature data for {room_id} from {start_time} to {end_time}")
            return None
        except Exception as e:
            ...
            async with get_db_session() as session:
                ...
                for pred in predictions:
                    if pred.actual_time and pred.predicted_time:
                        error_minutes = abs(
                            (pred.actual_time - pred.predicted_time).total_seconds() / 60
                        )
...
class FeatureDriftDetector:
    ...
    async def detect_feature_drift(
        ...
        if len(comparison_data) < self.min_samples_per_window or \
           len(monitor_data) < self.min_samples_per_window:
            logger.warning(
    ...
    async def _test_categorical_feature_drift(
        ...
        try:
            ...
            # Chi-square test
            chi2_stat, chi2_p, dof, expected = stats.chi2_contingency([baseline_aligned, current_aligned])
            
            # Calculate statistics
            baseline_stats = {
                'mode': baseline.mode().iloc[0] if len(baseline.mode()) > 0 else None,
                'unique_values': baseline.nunique(),
                'entropy': -sum(p * np.log2(p) for p in baseline.value_counts(normalize=True) if p > 0)
            }
            
            current_stats = {
                'mode': current.mode().iloc[0] if len(current.mode()) > 0 else None,
                'unique_values': current.nunique(),
                'entropy': -sum(p * np.log2(p) for p in current.value_counts(normalize=True) if p > 0)
            }
        ...
        while self._monitoring_active:
            try:
                for room_id in room_ids:
                    ...
                    if feature_data is not None and len(feature_data) > self.min_samples_per_window * 2:
                        # Detect drift
                        drift_results = await self.detect_feature_drift(room_id, feature_data)
                        
                        # Log significant drift
                        significant_drifts = [r for r in drift_results if r.is_significant()]
    ...
    def add_drift_callback(self, callback: Callable[[str, FeatureDriftResult], None]) -> None:
        """Add callback for drift notifications."""
        self._drift_callbacks.append(callback)
    
    def remove_drift_callback(self, callback: Callable[[str, FeatureDriftResult], None]) -> None:
        """Remove drift notification callback."""
        if callback in self._drift_callbacks:
            self._drift_callbacks.remove(callback)
    
    async def _notify_drift_callbacks(self, room_id: str, drift_result: FeatureDriftResult) -> None:
