...
class ModelOptimizer:
    ...
    def __init__(
        ...
        logger.info(f"Initialized ModelOptimizer with strategy: {config.strategy.value}")
    ...
    async def optimize_model_parameters(
        ...
        try:
            if not self.config.enabled:
                logger.info(f"Optimization disabled, using default parameters for {model_type}")
            ...
            if self.config.strategy == OptimizationStrategy.BAYESIAN:
                result = await self._bayesian_optimization(objective_func, param_space, model_type)
            elif self.config.strategy == OptimizationStrategy.GRID_SEARCH:
                result = await self._grid_search_optimization(objective_func, param_space, model_type)
            elif self.config.strategy == OptimizationStrategy.RANDOM_SEARCH:
                result = await self._random_search_optimization(objective_func, param_space, model_type)
            elif self.config.strategy == OptimizationStrategy.PERFORMANCE_ADAPTIVE:
                result = await self._performance_adaptive_optimization(
                    objective_func, param_space, model_type, room_id, performance_context
                )
            else:
                result = await self._bayesian_optimization(objective_func, param_space, model_type)
            ...
            with self._lock:
                ...
                if result.success and result.improvement_over_default > self.config.min_improvement_threshold:
    ...
    def get_cached_parameters(self, model_type: str, room_id: str) -> Optional[Dict[str, Any]]:
    ...
    def get_optimization_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            success_rate = (self._successful_optimizations / max(1, self._total_optimizations)) * 100
            ...
            recent_improvements = [r.improvement_over_default for r in recent_results if r.success]
            avg_recent_improvement = np.mean(recent_improvements) if recent_improvements else 0.0
            ...
            return {
                ...
                'optimization_history_length': sum(len(results) for results in self._optimization_history.values()),
                'configuration': {
        ...
        try:
            ...
            if accuracy_metrics and hasattr(accuracy_metrics, 'accuracy_rate'):
                if accuracy_metrics.accuracy_rate < 70.0:  # Poor performance
                    logger.info(f"Optimization needed due to poor accuracy: {accuracy_metrics.accuracy_rate:.1f}%")
                    return True
            ...
            if drift_metrics and hasattr(drift_metrics, 'overall_drift_score'):
                if drift_metrics.overall_drift_score > 0.3:  # Significant drift
                    logger.info(f"Optimization needed due to drift: {drift_metrics.overall_drift_score:.3f}")
                    return True
            ...
            with self._lock:
                if model_type in self._optimization_history:
                    ...
                    if recent_results and all(r.improvement_over_default < 0.005 for r in recent_results):
                        logger.info(f"Skipping optimization - recent attempts showed minimal improvement")
                        return False
        ...
        def objective(params):
            try:
                ...
                if self.config.objective == OptimizationObjective.ACCURACY:
                    score = training_result.validation_score or training_result.training_score or 0.0
                    ...
                    accuracy = training_result.validation_score or training_result.training_score or 0.0
                    ...
                    if hasattr(model_copy, 'last_prediction_time_ms'):
                        time_penalty = min(model_copy.last_prediction_time_ms / 1000.0, 0.1)
    ...
    def _run_bayesian_optimization(self, objective_func: Callable, param_space: List) -> OptimizationResult:
                ...
                for param in param_space:
                    if 'name' in param:
                        ...
                        if param['type'] == 'continuous':
                            ...
                            params[name] = np.random.randint(param['low'], param['high'] + 1)
                        elif param['type'] == 'categorical':
        ...
        try:
            ...
            if len(recent_performance) > 5:
                ...
                if trend < -0.05:  # Declining performance
                    ...
                    logger.info("Using aggressive optimization due to declining performance")
    ...
    def _initialize_parameter_spaces(self) -> Dict[str, List]:
        ...
        return {
            'lstm': [
                ...
                {'name': 'learning_rate', 'type': 'continuous', 'low': 0.0001, 'high': 0.01},
                {'name': 'batch_size', 'type': 'categorical', 'categories': [16, 32, 64, 128]}
            ],
            'xgboost': [
                ...
                {'name': 'colsample_bytree', 'type': 'continuous', 'low': 0.6, 'high': 1.0},
            ...
            'hmm': [
                ...
                {'name': 'covariance_type', 'type': 'categorical', 'categories': ['spherical', 'diag', 'full']},
                {'name': 'n_iter', 'type': 'integer', 'low': 50, 'high': 200},
                {'name': 'tol', 'type': 'continuous', 'low': 1e-6, 'high': 1e-2}
            ],
            'gaussian_process': [
                {'name': 'kernel', 'type': 'categorical', 'categories': ['rbf', 'matern', 'rational_quadratic']},
                {'name': 'alpha', 'type': 'continuous', 'low': 1e-12, 'high': 1e-1},
                {'name': 'n_restarts_optimizer', 'type': 'integer', 'low': 0, 'high': 10}
            ]
        }
