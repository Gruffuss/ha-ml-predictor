...
from ..core.exceptions import DatabaseError, OccupancyPredictionError, ErrorSeverity
...
class ValidationRecord:
    ...
    def validate_against_actual(self, actual_time: datetime, threshold_minutes: int = 15) -> bool:
        """
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'prediction_interval': (
                [self.prediction_interval[0].isoformat(), self.prediction_interval[1].isoformat()]
            ...
            'accuracy_level': self.accuracy_level.value if self.accuracy_level else None,
...
class AccuracyMetrics:
    ...
    # Error distribution
    error_percentiles: Dict[int, float] = field(default_factory=dict)  # 25th, 75th, 90th, 95th
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'measurement_period_start': (
                ...
                if self.measurement_period_start else None
            ),
            'measurement_period_end': (
                self.measurement_period_end.isoformat() 
                if self.measurement_period_end else None
            ),
...
class PredictionValidator:
    ...
    async def validate_prediction(
        ...
        try:
            ...
            candidates = await self._find_predictions_for_validation(
                room_id, actual_transition_time, transition_type, max_time_window_minutes
            )
            ...
            raise ValidationError("Failed to validate predictions against actual", cause=e)
    ...
    async def get_room_accuracy(self, room_id: str, hours_back: int = 24) -> AccuracyMetrics:
    ...
    async def get_model_accuracy(self, model_type: str, hours_back: int = 24) -> AccuracyMetrics:
        """Get accuracy metrics for specific model across all rooms."""
        return await self.get_accuracy_metrics(model_type=model_type, hours_back=hours_back)
            ...
            with self._lock:
                for record in self._validation_records.values():
                    ...
                    if expired_only:
                        ...
                        if (record.status == ValidationStatus.PENDING and 
                            record.predicted_time < cutoff_time):
    ...
    async def export_validation_data(
        ...
        try:
            ...
            logger.info(f"Exported {len(export_records)} validation records to {output_path}")
    ...
    def get_validation_stats(self) -> Dict[str, Any]:
        ...
        try:
            with self._lock:
                ...
                room_counts = {room: len(predictions) for room, predictions in self._records_by_room.items()}
                model_counts = {model: len(predictions) for model, predictions in self._records_by_model.items()}
    ...
    def cleanup_old_records(self, days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            with self._lock:
                ...
                records_to_remove = [
                    prediction_id for prediction_id, record in self._validation_records.items()
                    if record.prediction_time < cutoff_time
                ]
                ...
                self._records_by_room = {
                    room: predictions for room, predictions in self._records_by_room.items()
                    if predictions
                }
                self._records_by_model = {
                    model: predictions for model, predictions in self._records_by_model.items()
                    if predictions
                }
                ...
                for record in records:
                    ...
                    if db_prediction:
                        ...
                        db_prediction.is_accurate = (
                            ...
                            if record.error_minutes is not None else None
                        )
    ...
    def _calculate_metrics_from_records(
        ...
        validated_records = [r for r in records if r.status == ValidationStatus.VALIDATED]
        ...
        errors = [r.error_minutes for r in validated_records if r.error_minutes is not None]
        biases = [
            (r.actual_time - r.predicted_time).total_seconds() / 60
            for r in validated_records if r.actual_time
        ]
        ...
        if errors:
            ...
            metrics.std_error_minutes = statistics.stdev(errors) if len(errors) > 1 else 0.0
        ...
        if biases:
            ...
            metrics.bias_std_minutes = statistics.stdev(biases) if len(biases) > 1 else 0.0
        ...
        if confidences:
            ...
            if errors and len(errors) == len(confidences):
                ...
                metrics.confidence_accuracy_correlation = correlation_matrix[0, 1] if not np.isnan(correlation_matrix[0, 1]) else 0.0
                ...
                high_conf_wrong = sum(
                    1 for conf, err in zip(confidences, errors)
        ...
        if records:
            ...
            period_hours = (metrics.measurement_period_end - metrics.measurement_period_start).total_seconds() / 3600
            if period_hours > 0:
    ...
    async def _export_to_csv(self, records: List[ValidationRecord], output_path: Path) -> None:
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as csvfile:
            ...
            fieldnames = [
                'prediction_id', 'room_id', 'model_type', 'model_version',
                'predicted_time', 'transition_type', 'confidence_score',
                'actual_time', 'error_minutes', 'accuracy_level', 'status',
                'prediction_time', 'validation_time', 'expiration_time'
            ]
            ...
            for record in records:
                row = {
                    ...
                    'transition_type': record.transition_type,
                    'confidence_score': record.confidence_score,
                    'actual_time': record.actual_time.isoformat() if record.actual_time else '',
                    'error_minutes': record.error_minutes,
                    'accuracy_level': record.accuracy_level.value if record.accuracy_level else '',
                    'status': record.status.value,
                    'prediction_time': record.prediction_time.isoformat(),
                    'validation_time': record.validation_time.isoformat() if record.validation_time else '',
                    'expiration_time': record.expiration_time.isoformat() if record.expiration_time else ''
                }
                writer.writerow(row)
    
    async def _export_to_json(self, records: List[ValidationRecord], output_path: Path) -> None:
