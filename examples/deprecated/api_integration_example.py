...
async def run_integrated_system():
    ...
    try:
        ...
        if api_server:
            ...
            logger.info(
                f"📊 API Server running at: http://{config.api.host}:{config.api.port}"
            )
            logger.info(
                f"📋 API Documentation: http://{config.api.host}:{config.api.port}/docs"
            )
            ...
            logger.info(
                "  GET  /accuracy?room_id={id}&hours={h} - Get accuracy metrics"
            )
