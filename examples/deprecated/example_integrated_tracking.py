    ...
    return pd.DataFrame(
        {
            ...
            "transition_type": np.random.choice(
                ["vacant_to_occupied", "occupied_to_vacant"], num_samples
            ),
        }
    )
...
async def demonstrate_integrated_tracking():
    ...
    try:
        ...
        logger.info(
            f"Tracking configuration: enabled={config.tracking.enabled}, "
        ...
        tracking_manager = TrackingManager(
            ...
            notification_callbacks=[
                lambda alert: logger.warning(
                    f"ALERT: {alert.description} (severity: {alert.severity.value})"
                )
            ],
        )
        ...
        logger.info(
            f"Predictions recorded: {status['performance']['total_predictions_recorded']}"
        )
        logger.info(
            f"Validations performed: {status['performance']['total_validations_performed']}"
        )
        ...
        if alerts:
            ...
            for alert in alerts:
                logger.info(
                    f"  - {alert.description} (severity: {alert.severity.value})"
                )
        else:
        ...
        logger.info(
            f"Final predictions recorded: {final_status['performance']['total_predictions_recorded']}"
        )
        logger.info(
            f"Final validations performed: {final_status['performance']['total_validations_performed']}"
        )
