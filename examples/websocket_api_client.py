...
class WebSocketAPIClient:
    ...
    async def subscribe_to_endpoint(self, endpoint: str, room_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"Subscription request sent for {endpoint}" + (f" (room: {room_id})" if room_id else ""))
            return True
        except Exception as e:
            logger.error(f"Failed to subscribe to {endpoint}: {e}")
            return False
    
    async def unsubscribe_from_endpoint(self, endpoint: str, room_id: Optional[str] = None) -> bool:
    ...
    async def _handle_connection(self, message: Dict):
        ...
        logger.info(f"Supported message types: {data.get('supported_message_types', [])}")
    ...
    async def _handle_subscription_status(self, message: Dict):
        ...
        if success:
            if action == "subscribe":
                ...
                logger.info(f"Successfully subscribed to {endpoint}" + (f" (room: {room_id})" if room_id else ""))
            elif action == "unsubscribe":
                self.subscriptions.discard(endpoint)
                logger.info(f"Successfully unsubscribed from {endpoint}")
        else:
            logger.error(f"Subscription {action} failed for {endpoint}: {data.get('message')}")
    ...
    async def _handle_prediction_update(self, message: Dict):
        ...
        if alternatives:
            ...
            for i, alt in enumerate(alternatives[:3], 1):
                logger.info(f"     {i}. {alt['predicted_time']} (confidence: {alt['confidence']:.2%})")
    ...
    async def _handle_alert_notification(self, message: Dict):
        ...
        logger.warning(f"🚨 Alert Notification" + (f" - Room: {room_id}" if room_id else ""))
        ...
        if alert_type == "accuracy_degradation":
            logger.warning(f"   Current accuracy: {data.get('current_accuracy', 'Unknown')}")
            ...
            logger.warning(f"   Retraining recommended: {data.get('retraining_recommended', False)}")
    ...
    async def _handle_heartbeat(self, message: Dict):
        ...
        logger.debug(f"💓 Heartbeat - Server time: {server_time}, Uptime: {uptime:.1f}s")
    ...
    async def _handle_rate_limit_warning(self, message: Dict):
        ...
        logger.warning(f"   Max messages per minute: {data.get('max_messages_per_minute')}")
...
async def main():
    ...
    parser.add_argument("--client-name", default="ExampleClient", help="Client name for identification")
    parser.add_argument("--no-reconnect", action="store_true", help="Disable automatic reconnection")
