    ...
    try:
        ...
        if drift_metrics.retraining_recommended:
            ...
            if drift_metrics.immediate_attention_required:
                logger.error(f"Immediate attention required for {room_id} - critical drift detected!")
        
        # Export drift metrics for further analysis
        drift_data = drift_metrics.to_dict()
        logger.info(f"Drift analysis completed. Overall score: {drift_metrics.overall_drift_score:.3f}")
...
async def demonstrate_feature_drift_monitoring():
    ...
    async def drift_notification_callback(room_id: str, drift_result):
        ...
        if drift_result.drift_score > 0.5:
            logger.error(f"High drift score for {drift_result.feature_name} - immediate attention needed")
...
def print_drift_analysis(drift_metrics):
    ...
    print(f"Current Period: {drift_metrics.current_period[0]} to {drift_metrics.current_period[1]}")
    
    # Statistical test results
    print(f"\nSTATISTICAL TEST RESULTS:")
    print(f"Kolmogorov-Smirnov: statistic={drift_metrics.ks_statistic:.4f}, p={drift_metrics.ks_p_value:.6f}")
    print(f"Mann-Whitney U: statistic={drift_metrics.mw_statistic:.4f}, p={drift_metrics.mw_p_value:.6f}")
    print(f"Population Stability Index: {drift_metrics.psi_score:.4f}")
    print(f"Page-Hinkley: statistic={drift_metrics.ph_statistic:.2f}, threshold={drift_metrics.ph_threshold}")
    ...
    print(f"Confidence Calibration Drift: {drift_metrics.confidence_calibration_drift:.4f}")
    ...
    # Recommendations
    print(f"\nRECOMMENDATIONS:")
    print(f"Retraining Recommended: {'YES' if drift_metrics.retraining_recommended else 'NO'}")
    print(f"Immediate Attention Required: {'YES' if drift_metrics.immediate_attention_required else 'NO'}")
...
async def demonstrate_integrated_monitoring():
    ...
    try:
        ...
        real_time_metrics = await accuracy_tracker.get_real_time_metrics(room_id=room_id)
        
        if real_time_metrics and not real_time_metrics.is_healthy:
            logger.warning(f"Poor accuracy detected for {room_id} - triggering drift analysis")
            ...
            if drift_metrics.drift_severity.value in ['major', 'critical']:
                logger.error(f"Significant drift detected - this explains the accuracy degradation")
                
                # In production, this would trigger model retraining
                logger.info(f"Would trigger automatic retraining for {room_id}")
            else:
                logger.info(f"No significant drift detected - accuracy issues may be due to other factors")
        
        else:
            logger.info(f"Accuracy is healthy for {room_id} - no drift detection needed")
